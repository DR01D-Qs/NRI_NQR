...
function ElementPointOfNoReturn:on_executed(instigator)
    ...
    if self._values and self._values.tweak_id then
		...
		for i, k in pairs(self._values) do
            if string.find(i, "time_") then
				self._values[i] = self._values[i] * (lookup[self._values.tweak_id] or lookup[job] or (lookup_acc[job] and lookup_acc[job][self._values._editor_name]) or 1)
			end
        end
	end
