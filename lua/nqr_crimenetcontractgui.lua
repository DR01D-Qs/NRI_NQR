function CrimeNetContractGui:init(ws, fullscreen_ws, node)
	...
	local string_diff = managers.localization:to_upper_text(tweak_data.difficulty_name_ids[job_data.difficulty] or "")
	...
	local string = managers.localization:to_upper_text(menu_risk_id)
	local string_risk = string.sub(string, string.find(string, "%.") or 0)
	...
	local risk_text = self._contract_panel:text({
		...
		word_wrap = true,
		w = text_w - max_x,
		text = string_diff..string_risk .. " \n" .. managers.localization:to_upper_text("menu_stat_job_completed", {
			stat = tostring(stat)
		}) .. " ",
		font = font,
		font_size = font_size,
	...
	if self._node:item("difficulty") then
		if self._node:item("difficulty")._all_options then
			self._node:item("difficulty")._all_options[5] = nil
			self._node:item("difficulty")._all_options[1]._parameters.text_id = "menu_difficulty_easy"
			self._node:item("difficulty")._all_options[2]._parameters.text_id = "menu_difficulty_normal"
			self._node:item("difficulty")._all_options[3]._parameters.text_id = "menu_difficulty_hard"
		end
		if self._node:item("difficulty")._options then
			self._node:item("difficulty")._options[5] = nil
			self._node:item("difficulty")._options[1]._parameters.text_id = "menu_difficulty_easy"
			self._node:item("difficulty")._options[2]._parameters.text_id = "menu_difficulty_normal"
			self._node:item("difficulty")._options[3]._parameters.text_id = "menu_difficulty_hard"
		end
	end
end
...
function CrimeNetContractGui:set_difficulty_id(difficulty_id)
	...
	risk_text:set_text(string_diff..string_risk .. " \n" .. managers.localization:to_upper_text("menu_stat_job_completed", {
		stat = tostring(stat)
	}) .. " ")
