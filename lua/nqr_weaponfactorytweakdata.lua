...
--DELET
	...
	self.parts.wpn_fps_upg_i_burstfire.pcs = nil
	...
	local delete_ipsc = {
		...
		wpn_fps_pis_sparrow = true,
		wpn_fps_pis_lemming = true,
		wpn_fps_pis_legacy = true,
		wpn_fps_pis_stech = true,
		wpn_fps_pis_holt = true,
		wpn_fps_pis_packrat = true,
		wpn_fps_pis_rsh12 = true,
		wpn_fps_x_deagle = true,
		wpn_fps_pis_pmm = true,
	}
	for i, k in pairs(self) do
...
--DEFINITIONS
	...
	self.nqr.all_gadgets = table.with(self.nqr.all_pistol_gadgets, self.nqr.all_rifle_gadgets)
	for i, k in pairs(self.nqr.all_pistol_gadgets) do self.parts[k].a_fl = 1 end
	self.nqr.all_rifle_to_pistol_gadgets = {
		"wpn_fps_upg_fl_ass_smg_sho_peqbox",
		"wpn_fps_upg_fl_ass_laser",
		"wpn_fps_upg_fl_dbal_laser",
	}
...
--POS'S
	...
	self.parts.wpn_fps_gadgets_pos_a_fl_upper = {
		is_a_unlockable = true,
		dlc = "nqr_dlc",
		unit = fantom_unit,
		a_obj = "a_fl",
	...
	for i, k in pairs(self.nqr.all_gadgets) do
		self.parts.wpn_fps_gadgets_pos_a_fl_upper.override[k] = {
			parent = "upper_body", a_obj = "a_fl", a_fl = (self.parts[k].a_fl or 0),
		}
	end
	...
	self.parts.wpn_fps_gadgets_pos_a_o = {
		is_a_unlockable = true,
		dlc = "nqr_dlc",
		unit = fantom_unit,
		a_obj = "a_o",
		type = "gadget_pos",
		name_id = "bm_wp_gadgets_pos_a_o",
		pcs = {},
		adds = {  }, 
		forbids = {},
		override = {
			wpn_fps_extra2_lock_gadgets = { forbids = {} },
			wpn_fps_extra3_lock_gadgets = { forbids = {} }
		},
		stats = {},
	}
	for i, k in pairs(self.nqr.all_gadgets) do
	...
	self.parts.wpn_fps_gadgets_pos_a_vg = {
		is_a_unlockable = true,
		dlc = "nqr_dlc",
		unit = fantom_unit,
		a_obj = "a_vg",
		type = "gadget_pos",
		name_id = "bm_wp_gadgets_pos_a_vg",
		pcs = {},
		adds = { "wpn_fps_extra_lock_gadgets" }, 
		forbids = {},
		override = {},
		stats = {},
	}
	for i, k in pairs(self.nqr.all_gadgets) do
		self.parts.wpn_fps_gadgets_pos_a_vg.override[k] = {
			a_obj = "a_vg", a_fl = (self.parts[k].a_fl or 0)-1, forbids = deep_clone(self.nqr.all_vertical_grips),
		}
	end

	self.parts.wpn_fps_gadgets_pos_pistols_blank = {
		is_a_unlockable = true,
		dlc = "nqr_dlc",
		unit = fantom_unit,
		a_obj = "a_o",
		type = "gadget_pos",
		name_id = "bm_wp_gadgets_pos_pistols_blank",
		adds = {}, 
		forbids = table.without(self.nqr.all_rifle_gadgets, self.nqr.all_rifle_to_pistol_gadgets),
		override = {},
		stats = {},
	}
--
...
--OVERRIDE_THINGS
	...
	local overrides_all_sights_to_a_o_2 = {
		...
		wpn_fps_fold_ironsight = { a_obj = "a_o_2", parent = "foregrip" },
	}
	...
	for i, k in pairs(self.nqr.all_vertical_grips) do overrides_vertical_grip_and_gadget_thing[k] = {
		a_obj = "a_fl",
		--adds = table.with(self.parts[k] and self.parts[k].adds or {}, "wpn_fps_vertical_grip_lock_gadgets"),
		forbids = deep_clone(self.nqr.all_gadgets),
	} end
	for i, k in pairs(self.nqr.all_gadgets) do overrides_vertical_grip_and_gadget_thing[k] = {
		--adds = table.with(self.parts[k] and self.parts[k].adds or {}, "wpn_fps_gadgets_lock_vertical_grip"),
		forbids = deep_clone(self.nqr.all_vertical_grips),
	} end
	...
	local overrides_gadget_foregrip_parent_thing = { wpn_fps_addon_ris = { parent = "foregrip" } }
...
--
	...
	for i, k in pairs(self.parts) do
		if self.parts[i] and self.parts[i].stance_mod then
			if self.parts[i].stance_mod.wpn_fps_lmg_hcar then
			...
			if self.parts[i].stance_mod.wpn_fps_gre_ms3gl then
				self.parts[i].stance_mod.wpn_fps_gre_ms3gl.rotation = nil
			end
		end
	end
	...
	for i, k in pairs(self) do
		if k.uses_parts then
			...
			if table.contains(k.uses_parts, "wpn_fps_upg_o_specter") and not table.contains(k.uses_parts, "wpn_fps_upg_o_leupold") then
				table.delete(k.uses_parts, "wpn_fps_upg_o_bmg")
				table.addto(k.uses_parts, self.nqr.all_snoptics)
			end
			...
			if table.contains(k.uses_parts, "wpn_fps_upg_fl_ass_peq15") then
				table.addto(k.uses_parts, self.nqr.all_pistol_gadgets)
			end
			if table.contains(k.uses_parts, "wpn_fps_upg_fl_pis_tlr1") then
				table.addto(k.uses_parts, self.nqr.all_rifle_to_pistol_gadgets)
			end
		end
	end
--
...
--AMMO
	...
	self.parts.wpn_fps_upg_cal_40sw.forbids = { "wpn_fps_upg_a_subfmj" }
	...
	self.parts.wpn_fps_upg_cal_45acp.forbids = { "wpn_fps_upg_a_subfmj" }
	...
	table.addto(self.parts.wpn_fps_upg_cal_44mag.forbids, self.nqr.all_shotgun_ammotypes)
	...
	table.addto(self.parts.wpn_fps_upg_cal_357mag.forbids, self.nqr.all_shotgun_ammotypes)
	...
	self.parts.wpn_fps_upg_cal_410.shell_eject = Idstring("effects/payday2/particles/weapons/shells/shell_slug")
	self.parts.wpn_fps_upg_cal_410.forbids = { "wpn_fps_upg_a_subfmj" }
	...
	self.parts.wpn_fps_upg_cal_12g.shell_eject = Idstring("effects/payday2/particles/weapons/shells/shell_slug")
	...
	self.parts.wpn_fps_upg_cal_556x45.forbids = { "wpn_fps_upg_a_subfmj" }
	table.addto(self.parts.wpn_fps_upg_cal_556x45.forbids, self.nqr.all_shotgun_ammotypes)
	self.parts.wpn_fps_upg_cal_556x45.stats.caliber = "5.56x45"
	self.parts.wpn_fps_upg_cal_556x45.stats.value = 15

	self.parts.wpn_fps_upg_cal_300blk = deep_clone(self.parts.wpn_fps_upg_cal_556x45)
	self.parts.wpn_fps_upg_cal_300blk.name_id = "bm_wp_upg_cal_300blk"
	self.parts.wpn_fps_upg_cal_300blk.forbids = {}
	table.addto(self.parts.wpn_fps_upg_cal_300blk.forbids, self.nqr.all_shotgun_ammotypes)
	self.parts.wpn_fps_upg_cal_300blk.stats.caliber = ".300 BLK"

	self.parts.wpn_fps_upg_cal_762x39 = deep_clone(self.parts.wpn_fps_upg_cal_556x45)
	self.parts.wpn_fps_upg_cal_762x39.name_id = "bm_wp_upg_cal_762x39"
	self.parts.wpn_fps_upg_cal_762x39.forbids = {}
	table.addto(self.parts.wpn_fps_upg_cal_762x39.forbids, self.nqr.all_shotgun_ammotypes)
	self.parts.wpn_fps_upg_cal_762x39.stats.caliber = "7.62x39"

	self.parts.wpn_fps_upg_cal_545x39 = deep_clone(self.parts.wpn_fps_upg_cal_556x45)
	...
	table.addto(self.parts.wpn_fps_upg_cal_545x39.forbids, self.nqr.all_shotgun_ammotypes)
	self.parts.wpn_fps_upg_cal_545x39.stats.caliber = "5.45x39"

	self.parts.wpn_fps_upg_cal_9x39 = deep_clone(self.parts.wpn_fps_upg_cal_556x45)
	self.parts.wpn_fps_upg_cal_9x39.name_id = "bm_wp_upg_cal_9x39"
	self.parts.wpn_fps_upg_cal_9x39.stats.caliber = "9x39"

	self.parts.wpn_fps_upg_cal_50beo = deep_clone(self.parts.wpn_fps_upg_cal_556x45)
	self.parts.wpn_fps_upg_cal_50beo.name_id = "bm_wp_upg_cal_50beo"
	self.parts.wpn_fps_upg_cal_50beo.forbids = { "wpn_fps_upg_a_subfmj" }
	table.addto(self.parts.wpn_fps_upg_cal_50beo.forbids, self.nqr.all_shotgun_ammotypes)
	self.parts.wpn_fps_upg_cal_50beo.stats.caliber = ".50 Beo"

	self.parts.wpn_fps_upg_cal_762x51 = deep_clone(self.parts.wpn_fps_upg_cal_556x45)
	self.parts.wpn_fps_upg_cal_762x51.name_id = "bm_wp_upg_cal_762x51"
	self.parts.wpn_fps_upg_cal_762x51.stats.caliber = "7.62x51"
	self.parts.wpn_fps_upg_cal_762x51.stats.value = 20

	self.parts.wpn_fps_upg_cal_338lm = deep_clone(self.parts.wpn_fps_upg_cal_762x51)
	self.parts.wpn_fps_upg_cal_338lm.name_id = "bm_wp_upg_cal_338lm"
	self.parts.wpn_fps_upg_cal_338lm.stats.caliber = ".338 LM"
	self.parts.wpn_fps_upg_cal_338lm.stats.value = 25

	self.parts.wpn_fps_upg_cal_45lc = deep_clone(self.parts.wpn_fps_upg_cal_9x19)
	self.parts.wpn_fps_upg_cal_45lc.name_id = "bm_wp_upg_cal_45lc"
	table.addto(self.parts.wpn_fps_upg_cal_45lc.forbids, self.nqr.all_shotgun_ammotypes)
	self.parts.wpn_fps_upg_cal_45lc.stats.caliber = ".45 LC"
	self.parts.wpn_fps_upg_cal_45lc.stats.value = 1
	...
	self.parts.wpn_fps_upg_a_dragons_breath.bullet_trail = Idstring("effects/payday2/particles/weapons/shotgun/sho_tracer_dragons_breath")
	self.parts.wpn_fps_upg_a_dragons_breath.muzzleflash = Idstring("effects/payday2/particles/weapons/shotgun/sho_muzzleflash_dragons_breath")
	...
	self.parts.wpn_fps_upg_cal_default = deep_clone(self.parts.wpn_fps_upg_cal_9x19)
	self.parts.wpn_fps_upg_cal_default.name_id = "bm_wp_upg_cal_default"
	self.parts.wpn_fps_upg_cal_default.pcs = nil
	self.parts.wpn_fps_upg_cal_default.stats = {}

	self.parts.wpn_fps_upg_blankcal_45acp = deep_clone(self.parts.wpn_fps_upg_cal_9x19)
	self.parts.wpn_fps_upg_blankcal_45acp.pcs = nil
	self.parts.wpn_fps_upg_blankcal_45acp.forbids = { "wpn_fps_upg_a_subfmj" }
	self.parts.wpn_fps_upg_blankcal_45acp.stats = {}
--



----MUZZLE DEVICES
	self.parts.wpn_fps_upg_ns_ass_smg_firepig.stats = { concealment = 2, weight = 1, length = 2, md_code = {0,0,0,0,2} }
	self.parts.wpn_fps_upg_ns_ass_smg_tank.stats = { concealment = 4, weight = 2, length = 2, md_code = {0,0,0,3,0} }
	self.parts.wpn_fps_upg_ns_ass_smg_v6.stats = { concealment = 6, weight = 3, length = 3, md_code = {0,0,2,2,0} }
	self.parts.wpn_fps_upg_ass_ns_battle.stats = { concealment = 2, weight = 1, length = 2, md_code = {0,2,0,0,0} }
	self.parts.wpn_fps_upg_ass_ns_linear.stats = { concealment = 2, weight = 1, length = 2, md_code = {0,1,0,1,1} }
	...
	self.parts.wpn_fps_upg_ns_ass_smg_stubby.stats = { concealment = 4, weight = 1, length = 2, md_code = {0,3,0,0,0} }
	self.parts.wpn_fps_lmg_hk51b_ns_jcomp.stats = { concealment = 2, weight = 1, length = 2, md_code = {0,0,0,2,0} }
	self.parts.wpn_fps_lmg_kacchainsaw_ns_muzzle.stats = { concealment = 2, weight = 1, length = 2, md_code = {0,2,0,1,0} }
	...
	self.parts.wpn_fps_upg_ns_duck.stats = { concealment = 2, weight = 1, length = 3, md_code = {0,1,0,0,0} }
...
----SILENCERS
	...
	self.parts.wpn_fps_upg_ns_ass_filter.value = 1
...
----SIGHTS
	...
	self.parts.wpn_fps_upg_o_atibal_reddot.value = 8
	...
	self.parts.wpn_fps_upg_o_northtac_reddot.value = 7
	...
	self.parts.wpn_fps_upg_o_45iron.value = 1
	...
	self.parts.wpn_fps_upg_o_45steel.value = 1
	...
	self.parts.wpn_fps_upg_o_45rds.value = 14
	...
	self.parts.wpn_fps_upg_o_45rds_v2.value = 13
	...
	self.parts.wpn_fps_upg_o_xpsg33_magnifier.value = 10
	...
	self.parts.wpn_fps_upg_o_sig.value = 11
	...
	self.parts.wpn_fps_upg_o_rmr.stance_mod.wpn_fps_pis_g17.translation = Vector3(0,0,-1.0)
	self.parts.wpn_fps_upg_o_rmr.stance_mod.wpn_fps_pis_g18c.translation = self.parts.wpn_fps_upg_o_rmr.stance_mod.wpn_fps_pis_g17.translation
	self.parts.wpn_fps_upg_o_rmr.stance_mod.wpn_fps_pis_g22c.translation = self.parts.wpn_fps_upg_o_rmr.stance_mod.wpn_fps_pis_g17.translation
	self.parts.wpn_fps_upg_o_rmr.stance_mod.wpn_fps_pis_g26.translation = self.parts.wpn_fps_upg_o_rmr.stance_mod.wpn_fps_pis_g17.translation
	self.parts.wpn_fps_upg_o_rmr.stance_mod.wpn_fps_pis_breech.translation = Vector3(0,0,-0.4)
	self.parts.wpn_fps_upg_o_rmr.stance_mod.wpn_fps_pis_welrod.translation = Vector3(0,0,-0.37)
	self.parts.wpn_fps_upg_o_rmr.stance_mod.wpn_fps_pis_pmm.translation = Vector3(0,0,-1.3)
	...
	self.parts.wpn_fps_upg_o_rms.stance_mod.wpn_fps_pis_g17.translation = Vector3(0,0,-1.2)
	self.parts.wpn_fps_upg_o_rms.stance_mod.wpn_fps_pis_g18c.translation = self.parts.wpn_fps_upg_o_rms.stance_mod.wpn_fps_pis_g17.translation
	self.parts.wpn_fps_upg_o_rms.stance_mod.wpn_fps_pis_g22c.translation = self.parts.wpn_fps_upg_o_rms.stance_mod.wpn_fps_pis_g17.translation
	self.parts.wpn_fps_upg_o_rms.stance_mod.wpn_fps_pis_g26.translation = self.parts.wpn_fps_upg_o_rms.stance_mod.wpn_fps_pis_g17.translation
	self.parts.wpn_fps_upg_o_rms.stance_mod.wpn_fps_pis_breech.translation = Vector3(0,0,-0.6)
	self.parts.wpn_fps_upg_o_rms.stance_mod.wpn_fps_pis_welrod.translation = Vector3(0,0,-0.57)
	self.parts.wpn_fps_upg_o_rms.stance_mod.wpn_fps_pis_pmm.translation = Vector3(0,0,-1.5)
	self.parts.wpn_fps_upg_o_rms.stats = { concealment = 2, weight = 1, sightheight = 1.2, use_stance_mod = true }
	self.parts.wpn_fps_upg_o_rikt.parent = nil
	self.parts.wpn_fps_upg_o_rikt.value = 6
	self.parts.wpn_fps_upg_o_rikt.stance_mod.wpn_fps_pis_g17.translation = Vector3(0,0,-1.6)
	self.parts.wpn_fps_upg_o_rikt.stance_mod.wpn_fps_pis_g18c.translation = self.parts.wpn_fps_upg_o_rikt.stance_mod.wpn_fps_pis_g17.translation
	self.parts.wpn_fps_upg_o_rikt.stance_mod.wpn_fps_pis_g22c.translation = self.parts.wpn_fps_upg_o_rikt.stance_mod.wpn_fps_pis_g17.translation
	self.parts.wpn_fps_upg_o_rikt.stance_mod.wpn_fps_pis_g26.translation = self.parts.wpn_fps_upg_o_rikt.stance_mod.wpn_fps_pis_g17.translation
	self.parts.wpn_fps_upg_o_rikt.stance_mod.wpn_fps_pis_breech.translation = Vector3(0,0,-1.0)
	self.parts.wpn_fps_upg_o_rikt.stance_mod.wpn_fps_pis_welrod.translation = Vector3(0,0,-0.97)
	self.parts.wpn_fps_upg_o_rikt.stance_mod.wpn_fps_pis_pmm.translation = Vector3(0,0,-1.9)
	self.parts.wpn_fps_upg_o_rikt.stats = { concealment = 3, weight = 1, sightheight = 1.6, use_stance_mod = true } --sightheight = 1.7



----GADGETS
	self.parts.wpn_fps_upg_fl_ass_laser.stats = { concealment = 0, weight = 1, gadget_power = { laser = 1 } }
	self.parts.wpn_fps_upg_fl_ass_smg_sho_peqbox.stats = { concealment = 0, weight = 2, gadget_power = { laser = 2 } }
	self.parts.wpn_fps_upg_fl_ass_peq15.adds = {}
	self.parts.wpn_fps_upg_fl_ass_peq15.stats = { concealment = 0, weight = 3, gadget_power = { laser = 2, flashlight = 0 } }
	self.parts.wpn_fps_upg_fl_ass_smg_sho_surefire.stats = { concealment = 0, weight = 2, gadget_power = { flashlight = 2 } }
	self.parts.wpn_fps_upg_fl_ass_utg.stats = { concealment = 0, weight = 3, gadget_power = { laser = 1, flashlight = 2 } }

	self.parts.wpn_fps_upg_fl_pis_laser.stats = { concealment = 0, weight = 1, gadget_power = { laser = 1 } }
	self.parts.wpn_fps_upg_fl_pis_crimson.stats = { concealment = 0, weight = 1, gadget_power = { laser = 1 } }
	self.parts.wpn_fps_upg_fl_pis_tlr1.stats = { concealment = 0, weight = 2, gadget_power = { flashlight = 2 } }
	self.parts.wpn_fps_upg_fl_pis_m3x.stats = { concealment = 0, weight = 2, gadget_power = { flashlight = 2 } }
	self.parts.wpn_fps_upg_fl_pis_perst.stats = { concealment = 0, weight = 3, gadget_power = { laser = 3 } }
	...
	self.parts.wpn_fps_upg_fl_dbal_laser.stats = { concealment = 0, weight = 2, gadget_power = { laser = 1, flashlight = 1 } }
	self.parts.wpn_fps_upg_fl_pis_x400v.stats = { concealment = 0, weight = 3, gadget_power = { laser = 1, flashlight = 2 } }
...
--------M4
	...
	local forbids_b_os1 = {
		"wpn_fps_m4_uupg_b_sd", "wpn_fps_snp_victor_b_standard",

		"wpn_fps_snp_tti_fg_standard", "wpn_fps_snp_victor_fg_standard", "wpn_fps_m4_upg_fg_mk12",
		"wpn_fps_upg_ass_m16_fg_stag",
		--"wpn_fps_m16_fg_standard", "wpn_fps_m16_fg_vietnam",
		"wpn_fps_upg_fg_jp", "wpn_fps_upg_fg_smr", "wpn_fps_m16_fg_railed",
		"wpn_fps_upg_ass_m4_fg_lvoa", "wpn_fps_snp_victor_fg_hera", "wpn_fps_uupg_fg_radian",
	...
	self.parts.wpn_fps_m4_uupg_b_long.stats = { concealment = 0, weight = 10, barrel_length = 20, md_code = {0,2,0,0,0}, md_bulk = {1,1} }
	...
	self.parts.wpn_fps_m4_uupg_b_medium.stats = { concealment = 0, weight = 7, barrel_length = 14.5, md_code = {0,2,0,0,0}, md_bulk = {1,1} }
	...
	self.parts.wpn_fps_para_b_medium = deep_clone(self.parts.wpn_fps_m4_uupg_b_short)
	self.parts.wpn_fps_para_b_medium.name_id = "bm_wp_para_b_medium"
	self.parts.wpn_fps_para_b_medium.forbids = deep_clone(forbids_fg_3)
	self.parts.wpn_fps_para_b_medium.override = {}
	self.parts.wpn_fps_para_b_medium.override.wpn_fps_ass_m16_os_frontsight = { forbids = deep_clone(forbids_b_os1) }
	self.parts.wpn_fps_para_b_medium.override.wpn_fps_ass_m16_os_frontsight2 = { forbids = deep_clone(forbids_b_os1) }
	self.parts.wpn_fps_para_b_medium.override.wpn_fps_ass_m16_os_frontsight3 = { forbids = deep_clone(forbids_b_os1) }
	self.parts.wpn_fps_para_b_medium.stats = { concealment = 0, weight = 6, barrel_length = 11.5, md_code = {0,2,0,0,0}, md_bulk = {1,1} }
	...
	table.addto(self.parts.wpn_fps_para_b_medium_os3.forbids, {"wpn_fps_amcar_uupg_fg_amcar", "wpn_fps_m4_uupg_fg_lr300"})
	self.parts.wpn_fps_m4_uupg_b_short.unit = "units/payday2/weapons/wpn_fps_ass_m4_pts/wpn_fps_para_b_medium"
	self.parts.wpn_fps_m4_uupg_b_short.forbids = deep_clone(forbids_fg_4)
	...
	self.parts.wpn_fps_m4_uupg_b_short.stats = { concealment = 0, weight = 5, barrel_length = 10.5, md_code = {0,2,0,0,0}, md_bulk = {1,1} }
	...
	table.addto(self.parts.wpn_fps_m4_uupg_b_short_os3.forbids, {"wpn_fps_amcar_uupg_fg_amcar", "wpn_fps_m4_uupg_fg_lr300"})
	self.parts.wpn_fps_para_b_short = deep_clone(self.parts.wpn_fps_para_b_medium)
	self.parts.wpn_fps_para_b_short.unit = "units/payday2/weapons/wpn_fps_ass_m4_pts/wpn_fps_para_b_short"
	self.parts.wpn_fps_para_b_short.name_id = "bm_wp_para_b_short"
	self.parts.wpn_fps_para_b_short.forbids = deep_clone(forbids_fg_1)
	self.parts.wpn_fps_para_b_short.override = {}
	self.parts.wpn_fps_para_b_short.override.wpn_fps_ass_m16_os_frontsight = { forbids = deep_clone(forbids_b_os3) }
	self.parts.wpn_fps_para_b_short.override.wpn_fps_ass_m16_os_frontsight = { forbids = deep_clone(forbids_b_os3) }
	self.parts.wpn_fps_para_b_short.override.wpn_fps_ass_m16_os_frontsight = { forbids = deep_clone(forbids_b_os3) }
	self.parts.wpn_fps_para_b_short.stats = { concealment = 0, weight = 3, barrel_length = 6.5, md_code = {0,2,0,0,0}, md_bulk = {1,1} }
	self.parts.wpn_fps_m4_uupg_b_sd.forbids = {}
	table.addto(self.parts.wpn_fps_m4_uupg_b_sd.forbids, forbids_fg_sd)
	table.addto(self.parts.wpn_fps_m4_uupg_b_sd.forbids, self.nqr.all_bxs)
	self.parts.wpn_fps_m4_uupg_b_sd.override = { wpn_fps_m4_uupg_fg_rail_ext = { unit = fantom_unit } }
	self.parts.wpn_fps_m4_uupg_b_sd.stats = { concealment = 0, weight = 12, barrel_length = 7, length = 5, md_code = {5,0,0,0,0} } --not_sure barrel
	self.parts.wpn_fps_upg_ass_m4_b_beowulf.sound_switch = nil
	self.parts.wpn_fps_upg_ass_m4_b_beowulf.override = {}
	self.parts.wpn_fps_upg_ass_m4_b_beowulf.override.wpn_fps_ass_m16_os_frontsight = { forbids = deep_clone(forbids_b_os2) }
	self.parts.wpn_fps_upg_ass_m4_b_beowulf.override.wpn_fps_ass_m16_os_frontsight2 = { forbids = deep_clone(forbids_b_os2) }
	self.parts.wpn_fps_upg_ass_m4_b_beowulf.override.wpn_fps_ass_m16_os_frontsight3 = { forbids = deep_clone(forbids_b_os2) }
	self.parts.wpn_fps_upg_ass_m4_b_beowulf.stats = { concealment = 0, weight = 17, barrel_length = 22 } --roughly
	...
	table.addto(self.parts.wpn_fps_ass_m16_o_handle_sight.forbids, table.without(self.nqr.all_sights, "wpn_fps_o_blank"))
	...
	self.parts.wpn_fps_ass_m16_os_frontsight.forbids = {
		...
		"wpn_fps_m4_uupg_fg_lr300", "wpn_fps_snp_tti_fg_standard", "wpn_fps_snp_victor_fg_standard", "wpn_fps_m4_upg_fg_mk12",
	}
	self.parts.wpn_fps_ass_m16_os_frontsight.override = {
		wpn_fps_m4_uupg_fg_rail = { forbids = { "wpn_fps_para_b_medium_os3", "wpn_fps_m4_uupg_b_short_os3", }, },
		wpn_fps_m4_uupg_fg_lr300 = { forbids = { "wpn_fps_para_b_medium_os3", "wpn_fps_m4_uupg_b_short_os3", }, },
		wpn_fps_upg_ass_m4_fg_moe = { forbids = { "wpn_fps_para_b_medium_os3", "wpn_fps_m4_uupg_b_short_os3", }, },
	...
	self.parts.wpn_fps_m4_uupg_draghandle_radian = deep_clone(self.parts.wpn_fps_m4_uupg_draghandle)
	self.parts.wpn_fps_m4_uupg_draghandle_radian.unit = "units/pd2_dlc_mxm/weapons/wpn_fps_upg_mxm_m4/wpn_fps_m4_uupg_draghandle_radian"
	self.parts.wpn_fps_m4_uupg_draghandle_radian.third_unit = "units/payday2/weapons/wpn_third_ass_m4_pts/wpn_third_m4_uupg_draghandle"
	self.parts.wpn_fps_m4_uupg_draghandle_radian.texture_bundle_folder = "mxm"
	self.parts.wpn_fps_m4_uupg_draghandle_radian.dlc = "mxm"
	self.parts.wpn_fps_m4_uupg_draghandle_radian.name_id = "bm_wp_m4_uupg_draghandle_radian"
	self.parts.wpn_fps_m4_uupg_draghandle_radian.stats = { concealment = 0, weight = 0 }

	local stanag_clip = { ["5.56x45"] = 30, [".300 BLK"] = 30, [".50 Beo"] = 10, ["5.45x39"] = 30, ["7.62x39"] = 25, [".410 bore"] = 10 }
	...
	self.parts.wpn_fps_upg_m4_m_drum.stats = { concealment = 48, weight = 10, mag_amount = { 1, 1, 2 }, CLIP_AMMO_MAX = { ["5.56x45"] = 100, [".300 BLK"] = 100, [".50 Beo"] = 8, ["5.45x39"] = 100, ["7.62x39"] = 17, [".410 bore"] = 8 }, retention = false }
	self.parts.wpn_fps_upg_m4_m_l5.stats = { concealment = 8, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = deep_clone(stanag_clip) }
	self.parts.wpn_fps_upg_m4_m_pmag.stats = { concealment = 8, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = deep_clone(stanag_clip) }
	self.parts.wpn_fps_upg_m4_m_quad.stats = { concealment = 18, weight = 3, mag_amount = { 1, 2, 3 }, CLIP_AMMO_MAX = { ["5.56x45"] = 60, [".300 BLK"] = 60, [".50 Beo"] = 5, ["5.45x39"] = 60, ["7.62x39"] = 45, [".410 bore"] = 5 }, retention = false }
	self.parts.wpn_fps_upg_m4_m_straight.stats = { concealment = 6, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = { ["5.56x45"] = 20, [".300 BLK"] = 20, [".50 Beo"] = 7, ["5.45x39"] = 20, ["7.62x39"] = 15, [".410 bore"] = 7 } }
	self.parts.wpn_fps_m4_uupg_m_std.stats = { concealment = 8, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = deep_clone(stanag_clip) }
	self.parts.wpn_fps_m4_uupg_m_strike.stats = { concealment = 11, weight = 2, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = {  ["5.56x45"] = 37, [".300 BLK"] = 37,[".50 Beo"] = 12, ["5.45x39"] = 37, ["7.62x39"] = 31, [".410 bore"] = 12 } }
	self.parts.wpn_fps_m4_upg_m_quick.stats = { concealment = 8, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = deep_clone(stanag_clip) }
	...
	self.parts.wpn_fps_m4_uupg_fg_rail.override = {
	...
	self.parts.wpn_fps_m4_uupg_fg_rail.stats = { concealment = 0, weight = 4 }
	self.parts.wpn_fps_m4_uupg_fg_lr300.adds = {}
	self.parts.wpn_fps_m4_uupg_fg_lr300.forbids = { "wpn_fps_gadgets_pos_a_fl3", "wpn_fps_m4_uupg_b_long", "wpn_fps_para_b_medium_os3", "wpn_fps_m4_uupg_b_short_os3", "wpn_fps_para_b_short", }
	self.parts.wpn_fps_m4_uupg_fg_lr300.override = {
		wpn_fps_m4_uupg_fg_rail_ext = { unit = fantom_unit },
		wpn_fps_fold_ironsight = {
			unit = "units/pd2_dlc_savi/weapons/wpn_fps_snp_victor_pts/wpn_fps_snp_victor_o_down_hera",
			third_unit = "units/pd2_dlc_savi/weapons/wpn_third_snp_victor_pts/wpn_third_snp_victor_o_hera",
			stats = { concealment = 2, weight = 0 },
	...
	self.parts.wpn_fps_upg_ass_m4_fg_moe.stats = { concealment = 0, weight = 3 }
	...
	self.parts.wpn_fps_upg_ass_m4_fg_lvoa.stats = { concealment = 0, weight = 6 }
	self.parts.wpn_fps_upg_ass_m4_fg_lvoa.override = {
		wpn_fps_fold_ironsight = {
			unit = "units/pd2_dlc_savi/weapons/wpn_fps_snp_victor_pts/wpn_fps_snp_victor_o_down_hera",
			third_unit = "units/pd2_dlc_savi/weapons/wpn_third_snp_victor_pts/wpn_third_snp_victor_o_hera",
			stats = { concealment = 2, weight = 0 },
	...
	self.parts.wpn_fps_upg_fg_jp.forbids = { "wpn_fps_m4_uupg_b_short", "wpn_fps_m4_uupg_b_short_os3", "wpn_fps_snp_victor_b_sbr", "wpn_fps_para_b_short", }
	...
	self.parts.wpn_fps_upg_fg_jp.stats = { concealment = 0, weight = 4 }
	...
	self.parts.wpn_fps_upg_fg_smr.stats = { concealment = 0, weight = 4 }
	...
	self.parts.wpn_fps_m4_upg_fg_mk12.forbids = { "wpn_fps_para_b_medium", "wpn_fps_para_b_medium_os3", "wpn_fps_m4_uupg_b_short", "wpn_fps_m4_uupg_b_short_os3", "wpn_fps_snp_victor_b_sbr", "wpn_fps_para_b_short", "wpn_fps_m4_uupg_b_sd", }
	...
	self.parts.wpn_fps_m4_upg_fg_mk12.stats = { concealment = 0, weight = 7, sightheight = 1.76 }
	...
	self.parts.wpn_fps_upg_m4_s_adapter.stats = { concealment = 0, weight = 1, length = 6 }
	...
	self.parts.wpn_fps_upg_m4_s_pts.stats = { concealment = 10, weight = 2, length = 9.06, shouldered = true }
	...
	self.parts.wpn_fps_upg_m4_s_standard.forbids = {}
	...
	self.parts.wpn_fps_upg_m4_s_ubr.stats = { concealment = 20, weight = 1+5, length = 10.08, shouldered = true }
	self.parts.wpn_fps_m4_uupg_s_zulu.stats = { concealment = 7, weight = 1+5, length = 9.08, shouldered = true }
	...
	local m16_legend_forbids = { "wpn_fps_addon_ris" }
	table.addto(m16_legend_forbids, table.without(self.nqr.all_m4_barrels, {"wpn_fps_m4_uupg_b_medium_os2"}))
	table.addto(m16_legend_forbids, table.without(self.nqr.all_m4_foregrips, {"wpn_fps_m16_fg_standard"}))
	table.addto(m16_legend_forbids, self.nqr.all_bxs)
	table.addto(m16_legend_forbids, table.with(self.nqr.all_big_snoptics, {"wpn_fps_upg_o_cs"}))
	self.nqr.m4_collection = {
		"wpn_fps_addon_ris",
		"wpn_nqr_extra3_rail",
		...
		"wpn_fps_upg_cal_545x39",
		"wpn_fps_upg_cal_762x39",
		"wpn_fps_upg_cal_410",

		"wpn_fps_ass_m16_fg_legend",
		"wpn_fps_ass_m16_s_legend",
	}
	table.addto(self.nqr.m4_collection, self.nqr.all_m4_stocks)
	table.addto(self.nqr.m4_collection, self.nqr.all_m4_foregrips)
	table.addto(self.nqr.m4_collection, self.nqr.all_m4_lowers)
	table.addto(self.nqr.m4_collection, self.nqr.all_m4_mags)
	table.addto(self.nqr.m4_collection, self.nqr.all_m4_grips)
	table.addto(self.nqr.m4_collection, self.nqr.all_m4_draghandles)
	table.addto(self.nqr.m4_collection, self.nqr.all_shotgun_ammotypes)
	self.nqr.m4_family = {
		"wpn_fps_ass_m4",
		"wpn_fps_ass_amcar",
		"wpn_fps_ass_m16",
	...
	for i, k in pairs(self.nqr.m4_family) do table.addto(self[k].uses_parts, self.nqr.m4_collection) end
	...
	self.wpn_fps_ass_m4.override = {
		...
		--	"wpn_fps_para_b_medium", "wpn_fps_para_b_medium_os3", "wpn_fps_m4_uupg_b_short", "wpn_fps_m4_uupg_b_short_os3", "wpn_fps_snp_victor_b_sbr", "wpn_fps_para_b_short",
		--} },
		wpn_fps_m16_fg_standard = { forbids = table.without(self.parts.wpn_fps_m16_fg_standard.forbids, {"wpn_fps_m4_uupg_b_medium"}) },
		--wpn_fps_m16_fg_vietnam = { forbids = {
		--	"wpn_fps_para_b_medium", "wpn_fps_para_b_medium_os3", "wpn_fps_m4_uupg_b_short", "wpn_fps_m4_uupg_b_short_os3", "wpn_fps_snp_victor_b_sbr", "wpn_fps_para_b_short", "wpn_fps_m4_uupg_b_sd",
		--} },
		wpn_fps_m16_fg_vietnam = { forbids = table.without(self.parts.wpn_fps_m16_fg_vietnam.forbids, {"wpn_fps_m4_uupg_b_medium"}) },
		wpn_fps_ass_m16_fg_legend = { forbids = table.without(m16_legend_forbids, {"wpn_fps_m4_uupg_b_medium", "wpn_fps_m4_uupg_fg_rail"}) },
		wpn_fps_m4_uupg_b_medium = { forbids = table.with(self.parts.wpn_fps_m4_uupg_b_medium.forbids, {"wpn_fps_ass_m16_fg_legend"}) },
	...
	self.parts.wpn_fps_amcar_uupg_fg_amcar.forbids = { "wpn_fps_gadgets_pos_a_fl3", "wpn_fps_m4_uupg_b_long", "wpn_fps_m4_uupg_b_medium_os2", "wpn_fps_para_b_medium_os3", "wpn_fps_m4_uupg_b_short_os3", "wpn_fps_para_b_short", }
	...
	self.wpn_fps_ass_amcar.override = {
		...
		wpn_fps_m16_fg_standard = { forbids = table.without(self.parts.wpn_fps_m16_fg_standard.forbids, {"wpn_fps_para_b_medium"}) },
		wpn_fps_m16_fg_vietnam = { forbids = table.without(self.parts.wpn_fps_m16_fg_vietnam.forbids, {"wpn_fps_para_b_medium"}) },
		wpn_fps_ass_m16_fg_legend = { forbids = table.without(m16_legend_forbids, {"wpn_fps_para_b_medium", "wpn_fps_amcar_uupg_fg_amcar"}) },
		wpn_fps_para_b_medium = { forbids = table.with(self.parts.wpn_fps_para_b_medium.forbids, {"wpn_fps_ass_m16_fg_legend"}) },
	}
	...
	for i, k in pairs(amcar_overrides) do
		...
		self.wpn_fps_ass_amcar.override[k] = self.wpn_fps_ass_amcar.override[k] or {}
		self.wpn_fps_ass_amcar.override[k].forbids = self.wpn_fps_ass_amcar.override[k].forbids or deep_clone(self.parts[k] and (self.parts[k].forbids or {}) or {})
		table.delete(self.wpn_fps_ass_amcar.override[k].forbids, {"wpn_fps_amcar_uupg_fg_amcar"})
	end
	table.addto_dict(self.wpn_fps_ass_amcar.override, overrides_gadget_foregrip_parent_thing)
	self.wpn_fps_ass_amcar.adds = {}
	table.swap(self.wpn_fps_ass_amcar.default_blueprint, "wpn_fps_m4_uupg_b_medium", "wpn_fps_para_b_medium")
	...
	table.insert(self.wpn_fps_ass_amcar.uses_parts, "wpn_fps_remove_fg")

	self.parts.wpn_fps_ass_m16_b_legend.type = "barrel_ext"
	self.parts.wpn_fps_ass_m16_b_legend.forbids = {}
	self.parts.wpn_fps_ass_m16_b_legend.override = {}
	...
	self.parts.wpn_fps_m16_s_solid.stats = { concealment = 0, weight = 1+3, length = 9, shouldered = true }
	self.parts.wpn_fps_upg_m4_g_mgrip_legend = deep_clone(self.parts.wpn_fps_upg_m4_g_mgrip)
	self.parts.wpn_fps_upg_m4_g_mgrip_legend.stats = {}
	self.parts.wpn_fps_ass_m16_s_legend.is_a_unlockable = true
	self.parts.wpn_fps_ass_m16_s_legend.pcs = {}
	self.parts.wpn_fps_ass_m16_s_legend.adds = { "wpn_fps_upg_m4_g_mgrip_legend" }
	table.addto(self.parts.wpn_fps_ass_m16_s_legend.forbids, table.without(self.nqr.all_m4_grips, { "wpn_fps_upg_m4_g_standard" }))
	self.parts.wpn_fps_ass_m16_s_legend.override = { wpn_fps_upg_m4_g_standard = { unit = fantom_unit } }
	self.parts.wpn_fps_ass_m16_s_legend.stance_mod = nil
	self.parts.wpn_fps_ass_m16_s_legend.stats = { concealment = 0, weight = 6, length = 11.09, shouldered = true }
	self.parts.wpn_fps_m16_fg_standard.pcs = {}
	self.parts.wpn_fps_m16_fg_standard.adds = {}
	self.parts.wpn_fps_m16_fg_standard.forbids = { "wpn_fps_m4_uupg_b_medium", "wpn_fps_snp_victor_b_standard", "wpn_fps_para_b_medium", "wpn_fps_para_b_medium_os3", "wpn_fps_m4_uupg_b_short", "wpn_fps_m4_uupg_b_short_os3", "wpn_fps_snp_victor_b_sbr", "wpn_fps_para_b_short" }
	self.parts.wpn_fps_m16_fg_standard.name_id = "bm_wp_fps_m16_fg_standard"
	self.parts.wpn_fps_m16_fg_standard.stats = { concealment = 0, weight = 3 }
	self.parts.wpn_fps_m16_fg_vietnam.adds = {}
	self.parts.wpn_fps_m16_fg_vietnam.forbids = { "wpn_fps_m4_uupg_b_medium", "wpn_fps_snp_victor_b_standard", "wpn_fps_para_b_medium", "wpn_fps_para_b_medium_os3", "wpn_fps_m4_uupg_b_short", "wpn_fps_m4_uupg_b_short_os3", "wpn_fps_snp_victor_b_sbr", "wpn_fps_para_b_short", "wpn_fps_m4_uupg_b_sd", }
	self.parts.wpn_fps_m16_fg_vietnam.stats = { concealment = 0, weight = 3 }
	self.parts.wpn_fps_m16_fg_railed.rails = { "top", "side", "bottom" }
	self.parts.wpn_fps_m16_fg_railed.adds = {}
	self.parts.wpn_fps_m16_fg_railed.forbids = { "wpn_fps_m4_uupg_b_short", "wpn_fps_m4_uupg_b_short_os3", "wpn_fps_snp_victor_b_sbr", "wpn_fps_para_b_short" }
	...
	self.parts.wpn_fps_m16_fg_railed.stats = { concealment = 0, weight = 6 }
	self.parts.wpn_fps_upg_ass_m16_fg_stag.rails = { "top" }
	self.parts.wpn_fps_upg_ass_m16_fg_stag.adds = {}
	self.parts.wpn_fps_upg_ass_m16_fg_stag.forbids = { "wpn_fps_para_b_medium", "wpn_fps_para_b_medium_os3", "wpn_fps_m4_uupg_b_short", "wpn_fps_m4_uupg_b_short_os3", "wpn_fps_snp_victor_b_sbr", "wpn_fps_para_b_short", "wpn_fps_m4_uupg_b_sd", }
	self.parts.wpn_fps_upg_ass_m16_fg_stag.override = nil
	self.parts.wpn_fps_upg_ass_m16_fg_stag.stats = { concealment = 0, weight = 5 }
	self.parts.wpn_fps_ass_m16_fg_legend.is_a_unlockable = true
	self.parts.wpn_fps_ass_m16_fg_legend.pcs = {}
	self.parts.wpn_fps_ass_m16_fg_legend.type = "exclusive_set"
	self.parts.wpn_fps_ass_m16_fg_legend.adds = { "wpn_fps_ass_m16_b_legend" }
	self.parts.wpn_fps_ass_m16_fg_legend.forbids = {}
	table.addto(self.parts.wpn_fps_ass_m16_fg_legend.forbids, table.without(m16_legend_forbids, {"wpn_fps_m4_uupg_b_long", "wpn_fps_m16_fg_standard"}))
	self.parts.wpn_fps_ass_m16_fg_legend.override = {
		wpn_fps_m4_uupg_b_medium_os2 = { unit = fantom_unit, stats = table.copy_append(self.parts.wpn_fps_m4_uupg_b_medium_os2.stats, { length = 7, md_code = {1,0,0,1,0}, md_bulk = {0,0} }) },
		wpn_fps_m16_fg_standard = { unit = fantom_unit, stats = {} },
		wpn_fps_m4_uupg_fg_rail = { unit = fantom_unit, stats = {} },
		wpn_fps_amcar_uupg_fg_amcar = { unit = fantom_unit, stats = {} },
		wpn_fps_smg_olympic_fg_olympic = { unit = fantom_unit, stats = {} },
		wpn_fps_snp_victor_fg_standard = { unit = fantom_unit, stats = {} },
		wpn_fps_m4_uupg_fg_rail_ext = { unit = fantom_unit },
	}
	self.parts.wpn_fps_ass_m16_fg_legend.stance_mod = nil
	self.parts.wpn_fps_ass_m16_fg_legend.stats = { concealment = 0, weight = 15, gadget_power = { laser = 1 } }
	...
	self.wpn_fps_ass_m16.override = {
		wpn_fps_snp_victor_s_hera = { forbids = table.without(self.nqr.all_m4_grips, "wpn_fps_upg_m4_g_standard") },
		wpn_fps_m4_uupg_b_long = { forbids = table.with(self.parts.wpn_fps_m4_uupg_b_long.forbids, {"wpn_fps_ass_m16_fg_legend"}) },
	}
	...
	table.insert(self.wpn_fps_ass_m16.uses_parts, "wpn_fps_remove_fg")
	...
	self.parts.wpn_fps_smg_olympic_s_adjust.stats = { concealment = 0, weight = 1+2, length = 7, shouldered = true }
	...
	self.parts.wpn_fps_smg_olympic_s_short.stats = { concealment = 0, weight = 1, length = 6 }
	...
	self.parts.wpn_fps_smg_olympic_fg_olympic.stats = { concealment = 0, weight = 2 }
	...
	self.parts.wpn_fps_smg_olympic_fg_railed.stats = { concealment = 0, weight = 3 }
	...
	self.parts.wpn_fps_upg_smg_olympic_fg_lr300.stats = { concealment = 0, weight = 3 }
	...
	self.wpn_fps_smg_olympic.override = {
		...
		wpn_fps_m16_fg_standard = { forbids = table.without(self.parts.wpn_fps_m16_fg_standard.forbids, {"wpn_fps_para_b_short"}) },
		wpn_fps_m16_fg_vietnam = { forbids = table.without(self.parts.wpn_fps_m16_fg_vietnam.forbids, {"wpn_fps_para_b_short"}) },
		wpn_fps_ass_m16_fg_legend = { forbids = table.without(m16_legend_forbids, {"wpn_fps_para_b_short", "wpn_fps_smg_olympic_fg_olympic"}) },
		wpn_fps_para_b_short = { forbids = table.with(self.parts.wpn_fps_para_b_short.forbids, {"wpn_fps_m4_uupg_fg_rail", "wpn_fps_ass_m16_fg_legend"}) },
		wpn_fps_ass_m16_o_handle_sight = { forbids = table.without(self.parts.wpn_fps_ass_m16_o_handle_sight.forbids, {"wpn_fps_amcar_uupg_body_upperreciever"}) },
		wpn_fps_amcar_uupg_body_upperreciever = { forbids = table.with(self.parts.wpn_fps_amcar_uupg_body_upperreciever.forbids, {"wpn_fps_ass_m16_o_handle_sight"}) },
	...
	table.swap(self.wpn_fps_smg_olympic.default_blueprint, "wpn_fps_m4_upper_reciever_round", "wpn_fps_amcar_uupg_body_upperreciever")
	table.swap(self.wpn_fps_smg_olympic.default_blueprint, "wpn_fps_m4_uupg_draghandle", "wpn_fps_ass_amcar_dh")
	table.delete(self.wpn_fps_smg_olympic.default_blueprint, "wpn_fps_ass_m16_o_handle_sight")
	...
	self.parts.wpn_fps_snp_victor_b_standard.pcs = {}
	self.parts.wpn_fps_snp_victor_b_standard.dlc = "savi"
	self.parts.wpn_fps_snp_victor_b_standard.forbids = { "wpn_fps_m16_fg_standard", "wpn_fps_m16_fg_vietnam", "wpn_fps_ass_m16_os_frontsight", }
	self.parts.wpn_fps_snp_victor_b_standard.override = { wpn_fps_m4_uupg_fg_rail_ext = { unit = fantom_unit } }
	self.parts.wpn_fps_snp_victor_b_standard.stats = { concealment = 0, weight = 7, barrel_length = 14.5 }
	self.parts.wpn_fps_snp_victor_body_receiver_lower.texture_bundle_folder = "savi"
	self.parts.wpn_fps_snp_victor_body_receiver_lower.dlc = "savi"
	...
	self.parts.wpn_fps_snp_victor_body_receiver_upper.dlc = "savi"
	...
	self.parts.wpn_fps_snp_victor_bolt_standard.dlc = "savi"
	...
	self.parts.wpn_fps_snp_victor_ns_standard.dlc = "savi"
	...
	self.parts.wpn_fps_snp_victor_s_adapter.dlc = "savi"
	self.parts.wpn_fps_snp_victor_s_adapter.pcs = {}
	self.parts.wpn_fps_snp_victor_s_adapter.type = "stock"
	self.parts.wpn_fps_snp_victor_s_adapter.stats = { concealment = 0, weight = 1, length = 6 }
	...
	self.parts.wpn_fps_snp_victor_fg_standard.dlc = "savi"
	...
	self.parts.wpn_fps_snp_victor_fg_standard.forbids = { "wpn_fps_m4_uupg_b_sd", "wpn_fps_snp_victor_b_sbr", "wpn_fps_ass_m16_os_frontsight", }
	self.parts.wpn_fps_snp_victor_fg_standard.stats = { concealment = 0, weight = 4 }
	self.parts.wpn_fps_snp_victor_vg_hera.texture_bundle_folder = "savi"
	self.parts.wpn_fps_snp_victor_vg_hera.dlc = "victor_mods_pack_2"
	...
	self.parts.wpn_fps_snp_victor_m_1.dlc = "savi"
	self.parts.wpn_fps_snp_victor_m_1.stats = { concealment = 6, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = { ["5.56x45"] = 20, [".300 BLK"] = 20, [".50 Beo"] = 7, ["5.45x39"] = 20, ["7.62x39"] = 15, [".410 bore"] = 7 } }
	...
	self.parts.wpn_fps_snp_victor_fg_hera = {
		...
		adds = {},
		forbids = { "wpn_fps_gadgets_pos_a_fl3", "wpn_fps_para_b_short", "wpn_fps_m4_uupg_b_sd", "wpn_fps_ass_m16_os_frontsight", },
		override = {
			wpn_fps_fold_ironsight = {
				unit = "units/pd2_dlc_savi/weapons/wpn_fps_snp_victor_pts/wpn_fps_snp_victor_o_down_hera",
				third_unit = "units/pd2_dlc_savi/weapons/wpn_third_snp_victor_pts/wpn_third_snp_victor_o_hera",
	...
	self.parts.wpn_fps_snp_victor_b_sbr.dlc = "victor_mods_pack_2"
	...
	self.parts.wpn_fps_snp_victor_b_sbr.forbids = { "wpn_fps_ass_m16_os_frontsight", }
	table.addto(self.parts.wpn_fps_snp_victor_b_sbr.forbids, forbids_fg_4)
	self.parts.wpn_fps_snp_victor_b_sbr.stats = { concealment = 0, weight = 5, barrel_length = 10.5 }
	...
	self.parts.wpn_fps_snp_victor_ns_hera_supp.dlc = "victor_mods_pack_2"
	...
	self.parts.wpn_fps_snp_victor_ns_hera_muzzle.stats = { concealment = 2, weight = 1, length = 2, md_code = {0,2,0,1,0} }
	...
	self.wpn_fps_snp_victor.override = {
		...
		wpn_fps_ass_m16_os_frontsight = { forbids = table.without(self.parts.wpn_fps_ass_m16_os_frontsight.forbids, {"wpn_fps_snp_victor_b_standard", "wpn_fps_snp_victor_fg_standard"}) },
		wpn_fps_ass_m16_fg_legend = { forbids = table.without(m16_legend_forbids, {"wpn_fps_snp_victor_b_standard", "wpn_fps_amcar_uupg_fg_amcar"}) },
		wpn_fps_snp_victor_b_standard = { forbids = table.with(self.parts.wpn_fps_snp_victor_b_standard.forbids, {"wpn_fps_ass_m16_fg_legend"}) },
	}
	...
	for i, k in pairs(victor_overrides) do
		self.wpn_fps_snp_victor.override[k] = { override = deep_clone(self.parts[k] and (self.parts[k].override or {})) }
		local csc = self.wpn_fps_snp_victor.override[k].override and self.wpn_fps_snp_victor.override[k].override.wpn_fps_ass_m16_os_frontsight
		if csc then self.wpn_fps_snp_victor.override[k].override.wpn_fps_ass_m16_os_frontsight = table.without(csc.forbids or {}, {"wpn_fps_snp_victor_fg_standard"}) end
	end
	...
	table.insert(self.wpn_fps_snp_victor.default_blueprint, "wpn_fps_m4_uupg_fg_rail_ext")
	table.insert(self.wpn_fps_snp_victor.uses_parts, "wpn_fps_m4_uupg_fg_rail_ext")
	...
	table.insert(self.wpn_fps_snp_victor.uses_parts, "wpn_fps_ass_m16_os_frontsight")
...
--------AK
	...
	self.nqr.all_ak_sightmounts = {
		"wpn_fps_upg_o_ak_scopemount",
		"wpn_fps_smg_coal_o_scopemount_standard",
	}
	self.nqr.all_ak_calibers = {
		"wpn_fps_upg_cal_762x39",
		"wpn_fps_upg_cal_545x39",
		"wpn_fps_upg_cal_556x45",
		"wpn_fps_upg_cal_300blk",

		"wpn_fps_upg_a_subfmj",
	}
	self.nqr.ak_collection = {}
	table.addto(self.nqr.ak_collection, self.nqr.all_ak_barrels)
	table.addto(self.nqr.ak_collection, self.nqr.all_ak_uppers)
	table.addto(self.nqr.ak_collection, self.nqr.all_ak_mags)
	table.addto(self.nqr.ak_collection, self.nqr.all_ak_grips)
	table.addto(self.nqr.ak_collection, self.nqr.all_ak_stocks)
	table.addto(self.nqr.ak_collection, self.nqr.all_ak_foregrips)
	table.addto(self.nqr.ak_collection, self.nqr.all_ak_sightmounts)
	table.addto(self.nqr.ak_collection, self.nqr.all_ak_calibers)
	self.nqr.ak_family = {
		"wpn_fps_ass_74",
		"wpn_fps_ass_akm",
	...
	for i, k in pairs(self.nqr.ak_family) do table.addto(self[k].uses_parts, self.nqr.ak_collection) end
	table.addto(self.wpn_fps_smg_akmsu.uses_parts, table.without(self.nqr.ak_collection, table.combine(self.nqr.all_ak_barrels, self.nqr.all_ak_uppers, self.nqr.all_ak_foregrips)))
	table.addto(self.wpn_fps_ass_flint.uses_parts, table.combine(self.nqr.all_ak_mags, self.nqr.all_ak_grips, self.nqr.all_ak_stocks, self.nqr.all_ak_calibers))
	table.addto(self.wpn_fps_smg_coal.uses_parts, table.combine(self.nqr.all_ak_grips, self.nqr.all_ak_stocks, self.nqr.all_ak_sightmounts))
	--table.insert(self.wpn_fps_smg_coal.uses_parts, "wpn_fps_upg_o_ak_scopemount") --todo
	table.addto(self.wpn_fps_smg_vityaz.uses_parts, table.combine(self.nqr.all_ak_grips))
	--table.addto(self.wpn_fps_smg_vityaz.uses_parts, table.combine(self.nqr.all_ak_stocks, self.nqr.all_ak_foregrips)) --todo
	table.addto(self.wpn_fps_ass_groza.uses_parts, table.combine(self.nqr.all_ak_grips, self.nqr.all_ak_mags))
	table.addto(self.wpn_fps_ass_tkb.uses_parts, self.nqr.all_ak_grips)
	...
	self.parts.wpn_fps_ass_74_b_standard.stats = { concealment = 0, weight = 8, barrel_length = 16.3, md_code = {0,1,0,1,0}, md_bulk = {2,2} }
	self.parts.wpn_fps_upg_ak_b_draco.override = {}
	self.parts.wpn_fps_upg_ak_b_draco.stats = { concealment = 0, weight = 6, barrel_length = 12.2 }
	self.parts.wpn_fps_upg_ak_b_ak105.stats = { concealment = 0, weight = 6, barrel_length = 12.4, md_code = {0,1,0,0,0}, md_bulk = {1,1} }
	self.parts.wpn_fps_upg_ass_ak_b_zastava.stats = { concealment = 0, weight = 12, barrel_length = 23.6, md_code = {0,2,0,0,0}, md_bulk = {2,2} } --not_sure
	self.parts.wpn_fps_ass_74_ns_legend = deep_clone(self.parts.wpn_fps_ass_74_b_legend)
	self.parts.wpn_fps_ass_74_ns_legend.name_id = "bm_wp_upg_ass_74_ns_legend"
	self.parts.wpn_fps_ass_74_ns_legend.is_a_unlockable = true
	self.parts.wpn_fps_ass_74_ns_legend.pcs = {}
	self.parts.wpn_fps_ass_74_ns_legend.type = "barrel_ext"
	self.parts.wpn_fps_ass_74_ns_legend.visibility = { { objects = { g_standard = false, g_fl = false } } }
	self.parts.wpn_fps_ass_74_ns_legend.stats = { concealment = 2, weight = 1, length = 1 }
	self.parts.wpn_upg_ak_fl_legend_dummy = deep_clone(self.parts.wpn_fps_ass_74_b_legend)
	self.parts.wpn_upg_ak_fl_legend_dummy.type = "gadget_dummy"
	self.parts.wpn_upg_ak_fl_legend_dummy.visibility = { { objects = { g_standard = false, g_ns = false } } }
	self.parts.wpn_upg_ak_fl_legend_dummy.stats = {}
	self.parts.wpn_upg_ak_fl_legend.is_a_unlockable = true
	self.parts.wpn_upg_ak_fl_legend.pcs = {}
	self.parts.wpn_upg_ak_fl_legend.adds = { "wpn_upg_ak_fl_legend_dummy" }
	self.parts.wpn_upg_ak_fl_legend.type = "wep_cos"
	self.parts.wpn_upg_ak_fl_legend.stats = { concealment = 0, weight = 4, gadget_power = { laser = 3 } }
	self.parts.wpn_fps_ass_74_b_legend.is_a_unlockable = true
	self.parts.wpn_fps_ass_74_b_legend.pcs = {}
	self.parts.wpn_fps_ass_74_b_legend.visibility = { { objects = { g_fl = false, g_ns = false } } }
	self.parts.wpn_fps_ass_74_b_legend.stats = { concealment = 0, weight = 8, barrel_length = 16.3 }
	self.parts.wpn_fps_upg_ak_ns_ak105.pcs = {}
	self.parts.wpn_fps_upg_ak_ns_ak105.stats = { concealment = 2, weight = 1, length = 2, md_code = {0,1,0,0,1} }
	...
	self.parts.wpn_fps_upg_ak_ns_jmac.stats = { concealment = 2, weight = 1, length = 2, md_code = {0,1,1,1,0} }
	...
	local akmag_clip = { ["5.45x39"] = 30, ["7.62x39"] = 30, ["5.56x45"] = 30, [".300 BLK"] = 30 }
	self.parts.wpn_fps_ass_74_m_standard.pcs = {}
	self.parts.wpn_fps_ass_74_m_standard.name_id = "bm_wp_fps_ass_74_m_standard"
	self.parts.wpn_fps_ass_74_m_standard.stats = { concealment = 8, weight = 2, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = deep_clone(akmag_clip) }
	...
	self.parts.wpn_fps_upg_ak_m_quad.stats = { concealment = 17, weight = 3, mag_amount = { 1, 2, 3 }, CLIP_AMMO_MAX = { ["5.45x39"] = 60, ["7.62x39"] = 60, ["5.56x45"] = 60, [".300 BLK"] = 60 }, retention = false }
	self.parts.wpn_fps_upg_ak_m_quick.stats = { concealment = 8, weight = 2, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = deep_clone(akmag_clip) }
	self.parts.wpn_fps_upg_ak_m_uspalm.stats = { concealment = 9, weight = 2, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = deep_clone(akmag_clip) }
	...
	self.parts.wpn_upg_ak_g_legend.is_a_unlockable = true
	self.parts.wpn_upg_ak_g_legend.pcs = {}
	self.parts.wpn_upg_ak_g_legend.stats = { concealment = 0, weight = 2 }
	...
	self.parts.wpn_upg_ak_fg_legend.is_a_unlockable = true
	self.parts.wpn_upg_ak_fg_legend.pcs = {}
	self.parts.wpn_upg_ak_fg_legend.visibility = { { objects = { g_upperhandguard = false } } }
	self.parts.wpn_upg_ak_fg_legend.stats = { concealment = 0, weight = 3 }
	...
	self.parts.wpn_fps_upg_ak_fg_trax.forbids = { "wpn_upg_ak_fl_legend" }
	...
	self.parts.wpn_fps_upg_ak_fg_zenitco.forbids = { "wpn_fps_upg_ak_b_draco", "wpn_upg_ak_fl_legend" }
	...
	self.parts.wpn_upg_ak_s_adapter.stats = { concealment = 0, weight = 1, length = 7 }
	...
	self.parts.wpn_upg_ak_s_legend.is_a_unlockable = true
	self.parts.wpn_upg_ak_s_legend.pcs = {}
	self.parts.wpn_upg_ak_s_legend.stats = { concealment = 0, weight = 3, length = 8, shouldered = true }
	...
	self.parts.wpn_fps_upg_blankcal_545 = deep_clone(self.parts.wpn_fps_upg_cal_9x19)
	self.parts.wpn_fps_upg_blankcal_545.pcs = nil
	self.parts.wpn_fps_upg_blankcal_545.forbids = { "wpn_fps_upg_a_subfmj" }
	self.parts.wpn_fps_upg_blankcal_545.stats = {}
	...
	self.wpn_fps_ass_74.override = {
		...
		--wpn_fps_o_blank = { override = {} },
		wpn_fps_upg_ns_pis_large = { flip_a = 2 },
	}
	for i, k in pairs({} or self.nqr.all_second_sights) do
		if self.parts[k].parent then
	...
	table.delete(self.wpn_fps_ass_74.uses_parts, "wpn_fps_upg_cal_545x39")
	table.insert(self.wpn_fps_ass_74.default_blueprint, "wpn_fps_upg_blankcal_545")
	table.insert(self.wpn_fps_ass_74.uses_parts, "wpn_fps_upg_blankcal_545")
	...
	self.parts.wpn_fps_ass_akm_b_standard.stats = { concealment = 0, weight = 8, barrel_length = 16.3, md_code = {0,0,1,0,0}, md_bulk = {1,1} }
	self.parts.wpn_upg_ak_m_akm.pcs = {}
	self.parts.wpn_upg_ak_m_akm.name_id = "bm_wp_upg_ak_m_akm"
	self.parts.wpn_upg_ak_m_akm.stats = { concealment = 8, weight = 3, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = deep_clone(akmag_clip) }
	...
	table.delete(self.wpn_fps_ass_akm.uses_parts, "wpn_fps_upg_cal_762x39")
	...
	table.delete(self.wpn_fps_ass_akm_gold.uses_parts, "wpn_fps_upg_cal_762x39")
	...
	self.parts.wpn_fps_lmg_rpk_b_standard.stats = { concealment = 0, weight = 17, barrel_length = 23.2, md_code = {0,2,0,0,0}, md_bulk = {1,1} }
	...
	self.parts.wpn_lmg_rpk_m_drum.stats = { concealment = 39, weight = 9, mag_amount = { 1, 1, 2 }, CLIP_AMMO_MAX = { ["5.45x39"] = 75, ["7.62x39"] = 75, ["5.56x45"] = 75, [".300 BLK"] = 75 }, retention = false }
	...
	self.parts.wpn_lmg_rpk_m_standard.stats = { concealment = 12, weight = 3, mag_amount = { 2, 3, 4 }, CLIP_AMMO_MAX = { ["5.45x39"] = 45, ["7.62x39"] = 45, ["5.56x45"] = 45, [".300 BLK"] = 45 } }
	...
	table.delete(self.wpn_fps_lmg_rpk.uses_parts, "wpn_fps_upg_cal_762x39")
	...
	self.parts.wpn_fps_smg_akmsu_b_standard.stats = { concealment = 0, weight = 4, barrel_length = 8.1, md_code = {0,1,0,0,0}, md_bulk = {1,1} }
	...
	table.delete(self.wpn_fps_smg_akmsu.uses_parts, "wpn_fps_upg_cal_545x39")
	table.insert(self.wpn_fps_smg_akmsu.default_blueprint, "wpn_fps_upg_blankcal_545")
	table.insert(self.wpn_fps_smg_akmsu.uses_parts, "wpn_fps_upg_blankcal_545")
...
--------REST
	...
	self.parts.wpn_fps_ass_flint_b_standard.stats = { concealment = 0, weight = 8, barrel_length = 16.3, md_code = {0,1,0,1,0}, md_bulk = {2,2} }
	...
	self.parts.wpn_fps_ass_flint_m_standard.stats = { concealment = 9, weight = 2, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = deep_clone(akmag_clip) }
	...
	self.parts.wpn_fps_snp_flint_s_adapter.stats = { concealment = 0, weight = 1, length = 6, shoulderable = true }
	self.wpn_fps_ass_flint.override.wpn_fps_upg_ak_dh_zenitco = { a_obj = "a_dh" }
	self.wpn_fps_ass_flint.override.wpn_fps_upg_ns_pis_large = { flip_a = 2 }
	...
	--table.insert(self.wpn_fps_ass_flint.uses_parts, "wpn_fps_upg_ak_dh_zenitco") --todo
	...
	table.delete(self.wpn_fps_ass_flint.uses_parts, "wpn_fps_upg_cal_545x39")
	table.insert(self.wpn_fps_ass_flint.default_blueprint, "wpn_fps_upg_blankcal_545")
	table.insert(self.wpn_fps_ass_flint.uses_parts, "wpn_fps_upg_blankcal_545")
	...
	self.parts.wpn_fps_upg_blankcal_9x39 = deep_clone(self.parts.wpn_fps_upg_cal_9x19)
	self.parts.wpn_fps_upg_blankcal_9x39.pcs = nil
	self.parts.wpn_fps_upg_blankcal_9x39.forbids = { "wpn_fps_upg_a_subfmj" }
	self.parts.wpn_fps_upg_blankcal_9x39.stats = {}
	self.parts.wpn_fps_ass_groza_b_standard.stats = { concealment = 0, weight = 5, barrel_length = 9.4 }
	...
	table.insert(self.wpn_fps_ass_groza.default_blueprint, "wpn_fps_upg_blankcal_9x39")
	table.insert(self.wpn_fps_ass_groza.uses_parts, "wpn_fps_upg_blankcal_9x39")
	table.addto(self.wpn_fps_ass_groza.uses_parts, self.nqr.all_second_sights)

	self.wpn_fps_ass_asval.sightheight_mod = 0
	self.parts.wpn_fps_ass_asval_b_proto.value_mod = 0.7
	self.parts.wpn_fps_ass_asval_b_proto.sub_type = "ironsight"
	self.parts.wpn_fps_ass_asval_b_proto.stats = { concealment = 0, weight = 4+7, barrel_length = 7.9, md_code = {1,0,0,0,0} }
	self.parts.wpn_fps_ass_asval_b_standard.sub_type = "ironsight"
	self.parts.wpn_fps_ass_asval_b_standard.stats = { concealment = 0, weight = 4+6, length = 8, barrel_length = 7.9, md_code = {5,0,0,0,0} }
	...
	self.parts.wpn_fps_ass_ak5_b_short.stats = { concealment = 0, weight = 7, barrel_length = 13.8, md_code = {0,2,0,0,0}, md_bulk = {1,1} }
	self.parts.wpn_fps_ass_ak5_b_std.stats = { concealment = 0, weight = 9, barrel_length = 17.7, md_code = {0,2,0,0,0}, md_bulk = {1,1} }
	...
	self.parts.wpn_fps_aug_b_long.stats = { concealment = 0, weight = 12, barrel_length = 24.4, md_code = {0,2,0,0,0}, md_bulk = {1,1} }
	self.parts.wpn_fps_aug_b_medium.stats = { concealment = 0, weight = 10, barrel_length = 19, md_code = {0,2,0,0,0}, md_bulk = {1,1} }
	self.parts.wpn_fps_aug_b_short.stats = { concealment = 0, weight = 7, barrel_length = 13.8, md_code = {0,2,0,0,0}, md_bulk = {1,1} }
	...
	table.insert(self.wpn_fps_ass_aug.default_blueprint, "wpn_fps_upg_blankcal_556")
	table.insert(self.wpn_fps_ass_aug.uses_parts, "wpn_fps_upg_blankcal_556")
	table.insert(self.wpn_fps_ass_aug.uses_parts, "wpn_fps_upg_cal_300blk")
	table.insert(self.wpn_fps_ass_aug.uses_parts, "wpn_fps_upg_a_subfmj")
	...
	self.parts.wpn_fps_ass_corgi_b_long.stats = { concealment = 0, weight = 10, barrel_length = 20 } --roughly
	self.parts.wpn_fps_ass_corgi_b_short.texture_bundle_folder = "rvd"
	self.parts.wpn_fps_ass_corgi_b_short.dlc = "rvd"
	self.parts.wpn_fps_ass_corgi_b_short.stats = { concealment = 0, weight = 8, barrel_length = 16 }
	...
	self.parts.wpn_fps_ass_famas_b_short.value_mod = 0.6
	self.parts.wpn_fps_ass_famas_b_short.override.wpn_fps_ass_famas_b_suppressed = { a_obj = "a_b", parent = false }
	self.parts.wpn_fps_ass_famas_b_short.stats = { concealment = 0, weight = 9, barrel_length = 17.7, md_code = {0,2,0,0,0}, md_bulk = {1,1} }
	self.parts.wpn_fps_ass_famas_b_standard.value_mod = 0.6
	self.parts.wpn_fps_ass_famas_b_standard.override.wpn_fps_ass_famas_b_suppressed = { a_obj = "a_ns_n" }
	self.parts.wpn_fps_ass_famas_b_standard.stats = { concealment = 0, weight = 10, barrel_length = 19.2, md_code = {0,2,0,0,0}, md_bulk = {1,1} }
	self.parts.wpn_fps_ass_famas_b_long.value_mod = 0.6
	self.parts.wpn_fps_ass_famas_b_long.override.wpn_fps_ass_famas_b_suppressed = { a_obj = "a_ns_n" }
	self.parts.wpn_fps_ass_famas_b_long.stats = { concealment = 0, weight = 12, barrel_length = 23.6, md_code = {0,2,0,0,0}, md_bulk = {1,1} }
	self.parts.wpn_fps_ass_famas_b_sniper.value_mod = 0.6
	...
	self.parts.wpn_fps_ass_famas_b_sniper.stats = { concealment = 0, weight = 19, barrel_length = 25.6 }
	...
	self.parts.wpn_fps_ass_famas_m_standard.stats = { concealment = 7, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = { ["5.56x45"] = 25 } }
	...
	self.parts.wpn_fps_ass_g36_b_short.stats = { concealment = 0, weight = 5, barrel_length = 9, md_code = {0,2,0,0,0}, md_bulk = {1,1} }
	self.parts.wpn_fps_ass_g36_b_long.stats = { concealment = 0, weight = 6, barrel_length = 12.5, md_code = {0,2,0,0,0}, md_bulk = {1,1} }
	...
	self.parts.wpn_fps_upg_g36_fg_long.stats = { concealment = 0, weight = 9+3+2, barrel_length = 18.9, md_code = {0,2,0,0,0}, md_bulk = {1,1}, value = 4 }
	...
	self.parts.wpn_fps_ass_g36_m_standard.stats = { concealment = 9, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = { ["5.56x45"] = 30 } }
	self.parts.wpn_fps_ass_g36_m_quick.stats = { concealment = 9, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = { ["5.56x45"] = 30 } }
	...
	self.wpn_fps_ass_g36.override.wpn_fps_upg_ns_pis_large = { flip_a = 2 }
	...
	self.parts.wpn_fps_ass_komodo_b_standard.stats = { concealment = 0, weight = 7, barrel_length = 13 }
	...
	self.parts.wpn_fps_ass_l85a2_b_short.value_mod = 0.7
	self.parts.wpn_fps_ass_l85a2_b_short.stats = { concealment = 0, weight = 9, barrel_length = 18 } --not_sure
	self.parts.wpn_fps_ass_l85a2_b_medium.value_mod = 0.7
	self.parts.wpn_fps_ass_l85a2_b_medium.stats = { concealment = 0, weight = 10, barrel_length = 20.4 }
	self.parts.wpn_fps_ass_l85a2_b_long.value_mod = 0.7
	self.parts.wpn_fps_ass_l85a2_b_long.stats = { concealment = 0, weight = 12, barrel_length = 23 } --roughly
	...
	self.parts.wpn_fps_ass_l85a2_m_emag.stats = { concealment = 8, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = deep_clone(stanag_clip) }
	...
	self.parts.wpn_fps_ass_l85a2_o_standard.type = "ironsight"
	self.parts.wpn_fps_ass_l85a2_o_standard.forbids = {}
	table.addto(self.parts.wpn_fps_ass_l85a2_o_standard.forbids, table.without(self.nqr.all_sights, "wpn_fps_o_blank"))
	...
	table.insert(self.wpn_fps_ass_l85a2.uses_parts, "wpn_fps_remove_ironsight")
	...
	self.parts.wpn_fps_ass_s552_b_standard.stats = { concealment = 0, weight = 4, barrel_length = 8.9 }
	...
	self.parts.wpn_fps_ass_s552_b_long.stats = { concealment = 0, weight = 7, barrel_length = 14.3 }
	...
	self.parts.wpn_fps_ass_s552_m_standard.a_obj = "a_m1"
	self.parts.wpn_fps_ass_s552_m_standard.bullet_objects.amount = 27
	self.parts.wpn_fps_ass_s552_m_standard.animations = nil
	self.parts.wpn_fps_ass_s552_m_standard.stats = { concealment = 8, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = { ["5.56x45"] = 30 } }
	...
	--[[self.wpn_fps_ass_s552.animations = {
		reload_not_empty = "reload_not_empty",
		reload = "reload",
	}]]
	...
	for i, k in pairs(self.nqr.all_ak_mags) do self.wpn_fps_ass_s552.override[k] = { a_obj = "a_m1" } end
	...
	table.insert(self.wpn_fps_ass_s552.default_blueprint, "wpn_fps_upg_blankcal_556")
	table.insert(self.wpn_fps_ass_s552.uses_parts, "wpn_fps_upg_blankcal_556")
	table.insert(self.wpn_fps_ass_s552.uses_parts, "wpn_fps_upg_cal_762x39")
	table.insert(self.wpn_fps_ass_s552.uses_parts, "wpn_fps_upg_a_subfmj")
	table.addto(self.wpn_fps_ass_s552.uses_parts, self.nqr.all_tube_stocks)
	table.addto(self.wpn_fps_ass_s552.uses_parts, self.nqr.all_ak_mags)

	self.parts.wpn_fps_ass_tecci_b_standard.forbids = { "wpn_fps_ass_contraband_fg_standard" }
	self.parts.wpn_fps_ass_tecci_b_standard.stats = { concealment = 0, weight = 5, barrel_length = 9 }
	self.parts.wpn_fps_ass_tecci_b_long.stats = { concealment = 0, weight = 7, barrel_length = 14.5 }
	self.parts.wpn_fps_ass_tecci_b_legend.stats = {}
	...
	self.parts.wpn_fps_ass_tecci_fg_standard.rails = { "side", "bottom" }
	self.parts.wpn_fps_ass_tecci_fg_standard.override = {
		wpn_fps_ass_contraband_o_standard = { unit = self.parts.wpn_fps_ass_tecci_o_standard.unit }
	}
	self.parts.wpn_fps_ass_tecci_fg_standard.stats = { concealment = 0, weight = 4 }
	self.parts.wpn_fps_ass_tecci_fg_legend.is_a_unlockable = true
	self.parts.wpn_fps_ass_tecci_fg_legend.pcs = {}
	self.parts.wpn_fps_ass_tecci_fg_legend.type = "exclusive_set"
	self.parts.wpn_fps_ass_tecci_fg_legend.rails = { "bottom" }
	self.parts.wpn_fps_ass_tecci_fg_legend.adds = { "wpn_fps_ass_tecci_b_legend" }
	self.parts.wpn_fps_ass_tecci_fg_legend.forbids = { "wpn_fps_ass_tecci_b_long", "wpn_fps_ass_contraband_fg_standard" }
	table.addto(self.parts.wpn_fps_ass_tecci_fg_legend.forbids, table.without(self.nqr.all_bxs, "wpn_fps_ass_tecci_ns_standard"))
	self.parts.wpn_fps_ass_tecci_fg_legend.override = {
		wpn_fps_ass_tecci_b_standard = { unit = fantom_unit },
		wpn_fps_ass_tecci_fg_standard = { unit = fantom_unit, override = {} },
		wpn_fps_ass_tecci_ns_standard = { unit = fantom_unit, stats = {} },
	}
	self.parts.wpn_fps_ass_tecci_fg_legend.stats = { concealment = 0, weight = -1, length = 2, md_code = {0,0,0,1,0}, gadget_power = { laser = 1 } }
	...
	self.parts.wpn_fps_ass_tecci_m_drum.stats = { concealment = 48, weight = 10, mag_amount = { 1, 1, 2 }, CLIP_AMMO_MAX = self.parts.wpn_fps_upg_m4_m_drum.stats.CLIP_AMMO_MAX, retention = false }
	...
	self.parts.wpn_fps_ass_tecci_s_legend.is_a_unlockable = true
	self.parts.wpn_fps_ass_tecci_s_legend.pcs = {}
	self.parts.wpn_fps_ass_tecci_s_legend.desc_id = "bm_wp_not_a_stock_desc"
	self.parts.wpn_fps_ass_tecci_s_legend.stats = { concealment = 0, weight = 3, length = 5 }
	...
	table.insert(self.wpn_fps_ass_tecci.default_blueprint, 1, "wpn_fps_foregrip_lock_gadgets")
	table.insert(self.wpn_fps_ass_tecci.uses_parts, "wpn_fps_foregrip_lock_gadgets")
	table.insert(self.wpn_fps_ass_tecci.default_blueprint, 1, "wpn_fps_foregrip_lock_vertical_grips")
	table.insert(self.wpn_fps_ass_tecci.uses_parts, "wpn_fps_foregrip_lock_vertical_grips")
	...
	table.insert(self.wpn_fps_ass_tecci.default_blueprint, "wpn_fps_upg_blankcal_556")
	table.insert(self.wpn_fps_ass_tecci.uses_parts, "wpn_fps_upg_blankcal_556")
	table.insert(self.wpn_fps_ass_tecci.uses_parts, "wpn_fps_upg_cal_300blk")
	table.insert(self.wpn_fps_ass_tecci.uses_parts, "wpn_fps_upg_a_subfmj")

	self.parts.wpn_fps_ass_vhs_b_short.value_mod = 0.8
	self.parts.wpn_fps_ass_vhs_b_short.override = { wpn_fps_ass_vhs_b_silenced = { a_obj = "a_b", parent = false } }
	self.parts.wpn_fps_ass_vhs_b_short.stats = { concealment = 0, weight = 8, barrel_length = 16.1 }
	self.parts.wpn_fps_ass_vhs_b_standard.value_mod = 0.8
	self.parts.wpn_fps_ass_vhs_b_standard.stats = { concealment = 0, weight = 10, barrel_length = 19.7 }
	self.parts.wpn_fps_ass_vhs_b_sniper.value_mod = 0.8
	self.parts.wpn_fps_ass_vhs_b_sniper.override = {}
	self.parts.wpn_fps_ass_vhs_b_sniper.stats = { concealment = 0, weight = 12, barrel_length = 24 } --roughly
	...
	self.parts.wpn_fps_ass_vhs_m.stats = { concealment = 9, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = { ["5.56x45"] = 30 } }
	...
	self.parts.wpn_fps_smg_hajk_b_short.stats = { concealment = 0, weight = 5, barrel_length = 10.9 }
	self.parts.wpn_fps_smg_hajk_b_medium.stats = { concealment = 0, weight = 6, barrel_length = 12.5 }
	self.parts.wpn_fps_smg_hajk_b_standard.stats = { concealment = 0, weight = 7, barrel_length = 14 } --roughly
	...
	table.insert(self.wpn_fps_smg_hajk.default_blueprint, "wpn_fps_upg_blankcal_556")
	table.insert(self.wpn_fps_smg_hajk.uses_parts, "wpn_fps_upg_blankcal_556")
	table.insert(self.wpn_fps_smg_hajk.uses_parts, "wpn_fps_upg_cal_762x39")
	table.insert(self.wpn_fps_smg_hajk.uses_parts, "wpn_fps_upg_cal_300blk")
	table.insert(self.wpn_fps_smg_hajk.uses_parts, "wpn_fps_upg_a_subfmj")
...
----DMR
	self.parts.wpn_fps_ass_scar_b_long.value_mod = 1.3
	self.parts.wpn_fps_ass_scar_b_long.stats = { concealment = 4, weight = 10, barrel_length = 20 }
	self.parts.wpn_fps_ass_scar_b_medium.value_mod = 1.3
	self.parts.wpn_fps_ass_scar_b_medium.stats = { concealment = 0, weight = 8, barrel_length = 16 }
	self.parts.wpn_fps_ass_scar_b_short.value_mod = 1.3
	...
	self.parts.wpn_fps_ass_scar_b_short.stats = { concealment = -2, weight = 7, barrel_length = 13 }
	...
	self.parts.wpn_fps_ass_shak12_b_dummy.stats = { concealment = 0, weight = 11, barrel_length = 14.7 } --roughly
	...
	self.parts.wpn_fps_ass_m14_b_standard.stats = { concealment = 0, weight = 8, barrel_length = 16.3, md_code = {0,1,0,0,0}, md_bulk = {1,1} }
	self.parts.wpn_fps_ass_m14_b_ruger.stats = {}
	self.parts.wpn_fps_ass_m14_b_legendary.stats = {}
	self.parts.wpn_fps_ass_m14_b_legendary_laser = deep_clone(self.parts.wpn_fps_ass_m14_b_legendary)
	self.parts.wpn_fps_ass_m14_b_legendary_laser.type = "gadget_dummy"
	self.parts.wpn_fps_ass_m14_b_legendary_laser.visibility = { { objects = { g_barrel = false } }, gadget_power = { laser = 1 } }
	...
	self.parts.wpn_fps_ass_m14_body_dmr.forbids = { "wpn_fps_ass_m14_body_ruger_rail", "wpn_fps_ass_m14_body_legendary_stock" }
	...
	self.parts.wpn_fps_ass_m14_body_ebr.forbids = { "wpn_fps_ass_m14_body_ruger_rail", "wpn_fps_ass_m14_body_ruger_rail2", "wpn_fps_ass_m14_body_legendary_stock" }
	...
	self.parts.wpn_fps_ass_m14_body_jae.forbids = { "wpn_fps_ass_m14_body_ruger_rail", "wpn_fps_ass_m14_body_ruger_rail2", "wpn_fps_ass_m14_body_legendary_stock" }
	...
	self.parts.wpn_fps_ass_m14_body_legendary_stock = deep_clone(self.parts.wpn_fps_ass_m14_body_legendary)
	self.parts.wpn_fps_ass_m14_body_legendary_stock.name_id = "bm_wp_m14_body_legend_stock"
	self.parts.wpn_fps_ass_m14_body_legendary_stock.is_a_unlockable = true
	self.parts.wpn_fps_ass_m14_body_legendary_stock.pcs = {}
	self.parts.wpn_fps_ass_m14_body_legendary_stock.type = "stock_addon"
	self.parts.wpn_fps_ass_m14_body_legendary_stock.visibility = { { objects = { g_body = false } } }
	self.parts.wpn_fps_ass_m14_body_legendary_stock.stats = { concealment = 0, weight = 3, length = 10, shouldered = true, shoulderable = true }
	self.parts.wpn_fps_ass_m14_body_legendary.is_a_unlockable = true
	self.parts.wpn_fps_ass_m14_body_legendary.pcs = {}
	self.parts.wpn_fps_ass_m14_body_legendary.type = "exclusive_set"
	self.parts.wpn_fps_ass_m14_body_legendary.adds = { "wpn_fps_ass_m14_b_legendary_laser" }
	self.parts.wpn_fps_ass_m14_body_legendary.forbids = { "wpn_fps_ass_m14_body_ebr", "wpn_fps_ass_m14_body_jae" }
	self.parts.wpn_fps_ass_m14_body_legendary.override.wpn_fps_ass_m14_body_dmr = { unit = fantom_unit, forbids = {}, override = {}, stats = {} }
	self.parts.wpn_fps_ass_m14_body_legendary.override.wpn_fps_ass_m14_b_standard = { unit = self.parts.wpn_fps_ass_m14_b_legendary.unit, stats = {} }
	self.parts.wpn_fps_ass_m14_body_legendary.override.wpn_fps_ass_m14_body_upper = { unit = self.parts.wpn_fps_ass_m14_body_upper_legendary.unit }
	self.parts.wpn_fps_ass_m14_body_legendary.override.wpn_fps_ass_m14_body_lower = { unit = self.parts.wpn_fps_ass_m14_body_lower_legendary.unit }
	self.parts.wpn_fps_ass_m14_body_legendary.visibility = { { objects = { g_stock = false } } }
	self.parts.wpn_fps_ass_m14_body_legendary.stats = { concealment = 0, weight = -2, barrel_length = 14, length = 9 }
	self.parts.wpn_fps_ass_m14_body_ruger.type = "exclusive_set"
	self.parts.wpn_fps_ass_m14_body_ruger.forbids = { "wpn_fps_ass_m14_body_ebr", "wpn_fps_ass_m14_body_jae", "wpn_fps_ass_m14_body_legendary_stock" }
	self.parts.wpn_fps_ass_m14_body_ruger.override.wpn_fps_ass_m14_body_dmr = { unit = fantom_unit, forbids = {}, override = {}, stats = {} }
	...
	self.parts.wpn_fps_ass_m14_body_ruger.stats = { concealment = 0, weight = -7, length = 20.010, shouldered = true, shoulderable = true, barrel_length = 12, value = 6 }
	...
	self.parts.wpn_fps_ass_m14_body_lower_legendary.stats = {}
	self.parts.wpn_fps_ass_m14_body_upper.stats = { concealment = 0, weight = 0 }
	self.parts.wpn_fps_ass_m14_body_upper_legendary.stats = {}
	...
	table.insert(self.wpn_fps_ass_m14.uses_parts, "wpn_fps_ass_m14_body_legendary_stock")

	self.parts.wpn_fps_ass_g3_b_short.custom_stats = nil
	self.parts.wpn_fps_ass_g3_b_short.stats = { concealment = 0, weight = 6, barrel_length = 12.4, md_code = {0,2,0,0,0}, md_bulk = {1,1} }
	self.parts.wpn_fps_ass_g3_b_long.stats = { concealment = 0, weight = 9, barrel_length = 17.7, md_code = {0,2,0,0,0}, md_bulk = {1,1} }
	self.parts.wpn_fps_ass_g3_b_sniper.adds = {}
	self.parts.wpn_fps_ass_g3_b_sniper.override = nil
	self.parts.wpn_fps_ass_g3_b_sniper.stats = { concealment = 0, weight = 19, barrel_length = 25.6 }
	...
	self.parts.wpn_fps_ass_g3_s_sniper.stats = { concealment = 0, weight = 5, length = 12, shouldered = true, cheek = 1 }
	self.parts.wpn_fps_ass_g3_s_wood.stats = { concealment = 0, weight = 3, length = 10, shouldered = true }
	...
	self.parts.wpn_fps_ass_galil_b_standard.value_mod = 1.1
	...
	self.parts.wpn_fps_ass_galil_b_standard.stats = { concealment = 0, weight = 9, barrel_length = 18.1, md_code = {0,2,0,0,0}, md_bulk = {1,1} }
	...
	self.parts.wpn_fps_ass_galil_fg_mar.value_mod = 1.1
	...
	self.parts.wpn_fps_ass_galil_fg_mar.stats = { concealment = 0, weight = 4, sightheight = -1, barrel_length = 8.26, md_code = {0,2,0,0,0}, md_bulk = {1,1}, value = 4 }
	...
	self.parts.wpn_fps_ass_galil_b_sar.stats = { concealment = 0, weight = 7, barrel_length = 13.1, md_code = {0,2,0,0,0}, md_bulk = {1,1} }
	...
	self.parts.wpn_fps_ass_galil_b_sniper.value_mod = 1.1
	...
	self.parts.wpn_fps_ass_galil_b_sniper.stats = { concealment = 0, weight = 9, barrel_length = 18.1, md_code = {0,1,2,0,0}, md_bulk = {2,2} }
	...
	self.parts.wpn_fps_ass_galil_s_standard.stats = { concealment = 0, weight = 6, length = 10, shouldered = true, shoulderable = true }
	self.parts.wpn_fps_ass_galil_s_fab.forbids = { "wpn_fps_ass_galil_s_wood", "wpn_fps_ass_galil_s_plastic" }
	self.parts.wpn_fps_ass_galil_s_fab.stats = { concealment = 0, weight = 6, length = 10, shouldered = true, shoulderable = true, cheek = 1.5 }
	self.parts.wpn_fps_ass_galil_s_light.forbids = { "wpn_fps_ass_galil_s_wood", "wpn_fps_ass_galil_s_plastic" }
	self.parts.wpn_fps_ass_galil_s_light.stats = { concealment = 0, weight = 6, length = 11, shouldered = true, shoulderable = true, cheek = 1 }
	self.parts.wpn_fps_ass_galil_s_skeletal.forbids = { "wpn_fps_ass_galil_s_wood", "wpn_fps_ass_galil_s_plastic" }
	self.parts.wpn_fps_ass_galil_s_skeletal.stats = { concealment = 0, weight = 5, length = 11, shouldered = true, shoulderable = true }
	self.parts.wpn_fps_ass_galil_s_sniper.forbids = { "wpn_fps_ass_galil_s_wood", "wpn_fps_ass_galil_s_plastic" }
	self.parts.wpn_fps_ass_galil_s_sniper.stats = { concealment = 0, weight = 5, length = 10, shouldered = true, shoulderable = true, cheek = 1 }
	self.parts.wpn_fps_ass_galil_s_wood.type = "stock_addon" --todo
	self.parts.wpn_fps_ass_galil_s_wood.stats = { concealment = 0, weight = 1, cheek = 1 }
	self.parts.wpn_fps_ass_galil_s_plastic.type = "stock_addon" --todo
	self.parts.wpn_fps_ass_galil_s_plastic.stats = { concealment = 0, weight = 1, cheek = 1 }
	...
	self.parts.wpn_fps_ass_fal_b_standard.value_mod = 1.1
	...
	self.parts.wpn_fps_ass_fal_b_standard.stats = { concealment = 0, weight = 9, barrel_length = 18, md_code = {0,2,0,0,0}, md_bulk = {2,2} }
	...
	self.parts.wpn_fps_ass_fal_fg_01.value_mod = 1.1
	...
	self.parts.wpn_fps_ass_fal_fg_01.stats = { concealment = 0, weight = 6, barrel_length = 11, md_code = {0,2,0,0,0}, md_bulk = {1,1}, value = 4 }
	self.parts.wpn_fps_ass_fal_b_01 = deep_clone(self.parts.wpn_fps_ass_fal_fg_01)
	self.parts.wpn_fps_ass_fal_b_01.type = "barrel"
	self.parts.wpn_fps_ass_fal_b_01.value_mod = 1.1
	self.parts.wpn_fps_ass_fal_b_01.visibility = { { objects = { g_frontgrip_lod0 = false } } }
	self.parts.wpn_fps_ass_fal_b_01.stats = { concealment = 0, weight = 6, barrel_length = 11, md_code = {0,2,0,0,0}, md_bulk = {1,1} }
	...
	self.parts.wpn_fps_ass_fal_b_03.value_mod = 1.1
	self.parts.wpn_fps_ass_fal_b_03.visibility = { { objects = { g_frontgrip03_lod0 = false } } }
	self.parts.wpn_fps_ass_fal_b_03.stats = { concealment = 0, weight = 11, barrel_length = 21, md_code = {0,2,0,0,0}, md_bulk = {2,2} }
	...
	self.parts.wpn_fps_ass_fal_s_01.stats = { concealment = 0, weight = 5, length = 10, shouldered = true, shoulderable = true }
	self.parts.wpn_fps_ass_fal_s_03.stats = { concealment = 0, weight = 5, length = 11.010, shouldered = true, cheek = 1 }
	self.parts.wpn_fps_ass_fal_s_standard.stats = { concealment = 0, weight = 3, length = 10, shouldered = true }
	self.parts.wpn_fps_ass_fal_s_wood.stats = { concealment = 0, weight = 4, length = 10, shouldered = true }
	...
	self.parts.wpn_fps_ass_contraband_b_standard.stats = { concealment = 0, weight = 8, barrel_length = 16.5 }
	...
	self.parts.wpn_fps_ass_contraband_fg_standard.override = {
		wpn_fps_ass_tecci_o_standard = { unit = self.parts.wpn_fps_ass_contraband_o_standard.unit }
	}
	...
	self.parts.wpn_fps_ass_ching_b_standard.value_mod = 0.6
	self.parts.wpn_fps_ass_ching_b_standard.stats = { concealment = 0, weight = 12, barrel_length = 24 }
	self.parts.wpn_fps_ass_ching_b_short.value_mod = 0.6
	self.parts.wpn_fps_ass_ching_b_short.stats = { concealment = 0, weight = 9, barrel_length = 18 }
	...
	self.parts.wpn_fps_snp_siltstone_b_silenced.value_mod = 0.5
	...
	self.parts.wpn_fps_snp_siltstone_b_silenced.stats = { concealment = 0, weight = 10, barrel_length = 20.4, md_code = {3,0,0,0,0}, value = 6 } --todo conc and weight instead of md_bulk
	...
	self.parts.wpn_fps_snp_siltstone_b_standard.stats = { concealment = 0, weight = 12, barrel_length = 24.4 }
	...
	self.parts.wpn_fps_snp_siltstone_s_polymer.stats = { concealment = 0, weight = 3, shouldered = true }
	self.parts.wpn_fps_snp_siltstone_s_wood.stats = { concealment = 0, weight = 4, shouldered = true }
	...
	self.parts.wpn_fps_snp_siltstone_ns_variation_b.stats = { concealment = 2, weight = 2, length = 2, md_code = {0,2,0,0,0} }
	...
	self.parts.wpn_fps_snp_tti_b_standard.stats = { concealment = 0, weight = 7, barrel_length = 14.5 }
	...
	self.parts.wpn_fps_snp_tti_ns_standard.stats = { concealment = 2, weight = 1, length = 2, md_code = {0,0,0,2,0} }
	...
	self.parts.wpn_fps_snp_qbu88_b_short.value_mod = 0.7
	self.parts.wpn_fps_snp_qbu88_b_short.stats = { concealment = 0, weight = 10, barrel_length = 20 } --roughly
	self.parts.wpn_fps_snp_qbu88_b_standard.value_mod = 0.7
	self.parts.wpn_fps_snp_qbu88_b_standard.stats = { concealment = 0, weight = 13, barrel_length = 25.2 }
	self.parts.wpn_fps_snp_qbu88_b_long.value_mod = 0.7
	self.parts.wpn_fps_snp_qbu88_b_long.stats = { concealment = 0, weight = 16, barrel_length = 32 } --roughly
	...
	self.wpn_fps_snp_qbu88.override = {
		wpn_fps_upg_ns_pis_large = { flip_a = 2 },
	}
	...
	self.parts.wpn_fps_snp_winchester_b_long.value_mod = 0.1
	self.parts.wpn_fps_snp_winchester_b_long.stats = { concealment = 5, weight = 12, barrel_length = 24, CLIP_AMMO_MAX = 14 } --not_sure
	self.parts.wpn_fps_snp_winchester_b_standard.value_mod = 0.1
	self.parts.wpn_fps_snp_winchester_b_standard.stats = { concealment = 0, weight = 10, barrel_length = 20, CLIP_AMMO_MAX = 10 } --not_sure
	self.parts.wpn_fps_snp_winchester_b_suppressed.value_mod = 0.1
	self.parts.wpn_fps_snp_winchester_b_suppressed.stats = { concealment = 8, weight = 14, length = 5, barrel_length = 20, CLIP_AMMO_MAX = 10, md_code = {1,0,0,0,0} }
	...
	self.parts.wpn_fps_snp_sbl_b_standard.value_mod = 0.2
	self.parts.wpn_fps_snp_sbl_b_standard.stats = { concealment = 0, weight = 10, barrel_length = 19.1, CLIP_AMMO_MAX = { [".45-70"] = 6, [".410 bore"] = 6, [".44 Mag"] = 9, [".357 Mag"] = 10 } }
	self.parts.wpn_fps_snp_sbl_b_long.type = "exclusive_set"
	self.parts.wpn_fps_snp_sbl_b_long.value_mod = 0.2
	self.parts.wpn_fps_snp_sbl_b_long.forbids = deep_clone(self.nqr.all_bxs)
	self.parts.wpn_fps_snp_sbl_b_long.override = { wpn_fps_snp_sbl_b_standard = { unit = self.parts.wpn_fps_snp_sbl_b_long.unit, stats = {} } }
	self.parts.wpn_fps_snp_sbl_b_long.visibility = { { objects = { g_b_long = false } } }
	self.parts.wpn_fps_snp_sbl_b_long.stats = { concealment = 0, weight = 12, barrel_length = 23, md_code = {0,0,0,3,0}, CLIP_AMMO_MAX = { [".45-70"] = 5, [".410 bore"] = 5, [".44 Mag"] = 7, [".357 Mag"] = 8 }, value = 4 }
	self.parts.wpn_fps_snp_sbl_b_short.type = "exclusive_set"
	self.parts.wpn_fps_snp_sbl_b_short.value_mod = 0.2
	self.parts.wpn_fps_snp_sbl_b_short.forbids = deep_clone(self.nqr.all_bxs)
	self.parts.wpn_fps_snp_sbl_b_short.override = { wpn_fps_snp_sbl_b_standard = { unit = self.parts.wpn_fps_snp_sbl_b_short.unit, stats = {} } }
	self.parts.wpn_fps_snp_sbl_b_short.visibility = { { objects = { g_b_short = false } } }
	self.parts.wpn_fps_snp_sbl_b_short.stats = { concealment = 0, weight = 7+5, length = 10, barrel_length = 13.25, md_code = {3,0,0,0,0}, CLIP_AMMO_MAX = { [".45-70"] = 4, [".410 bore"] = 4, [".44 Mag"] = 6, [".357 Mag"] = 7 }, value = 6 }
	...
	self.wpn_fps_snp_sbl.override = {
		...
		wpn_fps_upg_cal_default = { forbids = deep_clone(self.nqr.all_shotgun_ammotypes) },
		wpn_fps_upg_cal_44mag = { custom_stats = { fire_rate_multiplier = 1.5 }, stats = table.copy_append(self.parts.wpn_fps_upg_cal_44mag.stats, { totalammo = 24 }) },
		wpn_fps_upg_cal_357mag = { custom_stats = { fire_rate_multiplier = 1.5 }, stats = table.copy_append(self.parts.wpn_fps_upg_cal_357mag.stats, { totalammo = 24 }) },
	}
	for i, k in pairs(table.combine(self.nqr.all_sps3, self.nqr.all_sps4)) do
	...
	table.insert(self.wpn_fps_snp_sbl.default_blueprint, "wpn_fps_upg_cal_default")
	table.insert(self.wpn_fps_snp_sbl.uses_parts, "wpn_fps_upg_cal_default")
	...
	table.insert(self.wpn_fps_snp_sbl.uses_parts, "wpn_fps_upg_cal_44mag")
	table.insert(self.wpn_fps_snp_sbl.uses_parts, "wpn_fps_upg_cal_357mag")
	...
	self.parts.wpn_fps_lmg_hcar_barrel_short.value_mod = 1.1
	self.parts.wpn_fps_lmg_hcar_barrel_short.stats = { concealment = 0, weight = 7, barrel_length = 13 }
	self.parts.wpn_fps_lmg_hcar_barrel_standard.value_mod = 1.1
	self.parts.wpn_fps_lmg_hcar_barrel_standard.stats = { concealment = 0, weight = 8, barrel_length = 16 }
	self.parts.wpn_fps_lmg_hcar_barrel_dmr.value_mod = 1.1
	self.parts.wpn_fps_lmg_hcar_barrel_dmr.stats = { concealment = 0, weight = 18, barrel_length = 24 }
...
----SNIPER RIFLE
	self.parts.wpn_fps_snp_desertfox_b_short.value_mod = 0.7
	...
	self.parts.wpn_fps_snp_desertfox_b_short.stats = { concealment = 0, weight = 12, barrel_length = 16 }
	self.parts.wpn_fps_snp_desertfox_b_long.value_mod = 0.7
	self.parts.wpn_fps_snp_desertfox_b_long.visibility = { { objects = { g_comp = false, g_rail_long = false } } }
	self.parts.wpn_fps_snp_desertfox_b_long.stats = { concealment = 0, weight = 17, barrel_length = 22 }
	...
	self.parts.wpn_fps_snp_m95_barrel_short.stats = { concealment = 0, weight = 12, barrel_length = 16, md_code = {0,0,0,4,0}, length = 2, md_bulk = {2,2} } --roughly
	self.parts.wpn_fps_snp_m95_barrel_standard.stats = { concealment = 0, weight = 19, barrel_length = 29, md_code = {0,0,0,4,0}, length = 2, md_bulk = {2,2} }
	self.parts.wpn_fps_snp_m95_barrel_suppressed.stats = { concealment = 60, weight = 19+20, barrel_length = 29, length = 4, md_code = {5,0,0,0,0}, md_bulk = {60,20} }
	self.parts.wpn_fps_snp_m95_barrel_long.stats = { concealment = 0, weight = 24, barrel_length = 40, md_code = {0,0,0,4,0}, length = 2, md_bulk = {2,2} } --roughly
	...
	self.parts.wpn_fps_snp_model70_b_standard.stats = { concealment = 0, weight = 11, barrel_length = 22 }
	self.parts.wpn_fps_snp_model70_b_short = deep_clone(self.parts.wpn_fps_snp_model70_b_legend)
	self.parts.wpn_fps_snp_model70_b_short.name_id = "bm_wp_model70_b_short"
	self.parts.wpn_fps_snp_model70_b_short.pcs = {}
	self.parts.wpn_fps_snp_model70_b_short.value_mod = 0.4
	...
	self.parts.wpn_fps_snp_model70_b_short.stats = { concealment = 0, weight = 6, barrel_length = 12 }
	self.parts.wpn_fps_snp_model70_b_legend.name_id = "bm_wp_model70_b_legend"
	self.parts.wpn_fps_snp_model70_b_legend.is_a_unlockable = true
	self.parts.wpn_fps_snp_model70_b_legend.pcs = {}
	self.parts.wpn_fps_snp_model70_b_legend.type = "wep_cos"
	self.parts.wpn_fps_snp_model70_b_legend.visibility = { { objects = { g_b = false } } }
	self.parts.wpn_fps_snp_model70_b_legend.stats = { concealment = 7, weight = 2 }
	...
	self.parts.wpn_fps_snp_model70_s_legend.is_a_unlockable = true
	self.parts.wpn_fps_snp_model70_s_legend.pcs = {}
	self.parts.wpn_fps_snp_model70_s_legend.stats = { concealment = 0, weight = 6, length = 1, shouldered = true }
	...
	self.parts.wpn_fps_snp_mosin_b_short.value_mod = 0.1
	self.parts.wpn_fps_snp_mosin_b_short.stats = { concealment = 0, weight = 10, barrel_length = 20 }
	self.parts.wpn_fps_snp_mosin_b_medium.value_mod = 0.1
	self.parts.wpn_fps_snp_mosin_b_medium.stats = { concealment = 0, weight = 14, barrel_length = 28.7 }
	self.parts.wpn_fps_snp_mosin_b_sniper.value_mod = 0.1
	self.parts.wpn_fps_snp_mosin_b_sniper.forbids = deep_clone(self.nqr.all_bxs_sbr)
	self.parts.wpn_fps_snp_mosin_b_sniper.stats = { concealment = 0, weight = 14+5, barrel_length = 28.7, length = 8, md_code = {3,0,0,0,0} }
	self.parts.wpn_fps_snp_mosin_b_standard.value_mod = 0.1
	self.parts.wpn_fps_snp_mosin_b_standard.stats = { concealment = 0, weight = 16, barrel_length = 31.5 }
	...
	self.parts.wpn_fps_snp_msr_b_standard.value_mod = 0.7
	self.parts.wpn_fps_snp_msr_b_standard.stats = { concealment = 0, weight = 10, barrel_length = 20, md_code = {0,0,0,2,0}, md_bulk = {2,2} }
	self.parts.wpn_fps_snp_msr_b_long.value_mod = 0.7
	self.parts.wpn_fps_snp_msr_b_long.override = {}
	self.parts.wpn_fps_snp_msr_b_long.stats = { concealment = 0, weight = 14, barrel_length = 27, md_code = {0,0,0,2,0}, md_bulk = {2,2} }
	...
	self.parts.wpn_fps_snp_r700_b_short.value_mod = 0.4
	self.parts.wpn_fps_snp_r700_b_short.stats = { concealment = 0, weight = 8, barrel_length = 16 } --roughly
	self.parts.wpn_fps_snp_r700_b_standard.stats = { concealment = 0, weight = 12, barrel_length = 24 }
	...
	self.parts.wpn_fps_snp_r700_b_medium.stats = { concealment = 0, weight = 8+8, barrel_length = 16, length = 11, md_code = {5,0,0,0,0}, value = 6 } --roughly
	...
	self.parts.wpn_fps_snp_r93_b_short.value_mod = 0.7
	self.parts.wpn_fps_snp_r93_b_short.forbids = { "wpn_fps_snp_r93_b_suppressed" }
	self.parts.wpn_fps_snp_r93_b_short.stats = { concealment = 0, weight = 15, barrel_length = 20, md_code = {0,1,0,1,0}, md_bulk = {2,2} } --roughly
	self.parts.wpn_fps_snp_r93_b_standard.stats = { concealment = 0, weight = 18, barrel_length = 26.3, md_code = {0,1,0,1,0}, md_bulk = {2,2} }
	self.parts.wpn_fps_snp_r93_b_suppressed.value_mod = 0.7
	...
	self.parts.wpn_fps_snp_scout_b_standard.stats = { concealment = 0, weight = 10, barrel_length = 19 }
	...
	self.parts.wpn_fps_snp_wa2000_b_long.stats = { concealment = 0, weight = 19, barrel_length = 30 } --roughly
	self.parts.wpn_fps_snp_wa2000_b_standard.stats = { concealment = 0, weight = 17, barrel_length = 26 }
	self.parts.wpn_fps_snp_wa2000_b_suppressed.stats = { concealment = 30, weight = 17+10, barrel_length = 26, length = 11, md_code = {5,0,0,0,0} }
	...
	self.parts.wpn_fps_snp_awp_b_short.value_mod = 0.5
	self.parts.wpn_fps_snp_awp_b_short.override = {}
	self.parts.wpn_fps_snp_awp_b_short.stats = { concealment = 0, weight = 10, barrel_length = 16 } --roughly
	self.parts.wpn_fps_snp_awp_b_standard.stats = { concealment = 0, weight = 15, barrel_length = 20 }
	self.parts.wpn_fps_snp_awp_b_long.value_mod = 0.5
	self.parts.wpn_fps_snp_awp_b_long.override = {}
	self.parts.wpn_fps_snp_awp_b_long.stats = { concealment = 0, weight = 18, barrel_length = 24 }
	self.parts.wpn_fps_snp_awp_ns_muzzle.stats = { concealment = 0, weight = 3, length = 3, md_code = {0,0,0,3,0} }
	...
	self.wpn_fps_snp_awp.override.wpn_fps_remove_s = { forbids = { "wpn_fps_snp_awp_ext_shellrack" } }
	self.wpn_fps_snp_awp.override.wpn_fps_upg_cal_338lm = { custom_stats = { fire_rate_multiplier = 0.86 } }
	...
	table.insert(self.wpn_fps_snp_awp.uses_parts, "wpn_fps_remove_s")
...
----LMG
	...
	self.parts.wpn_fps_lmg_hk21_b_long.stats = { concealment = 0, weight = 17, barrel_length = 22, md_code = {0,2,0,0,0}, md_bulk = {1,1} }
	self.parts.wpn_fps_lmg_hk21_b_short.stats = { concealment = 0, weight = 13, barrel_length = 17.7, md_code = {0,2,0,0,0}, md_bulk = {1,1} }
	...
	self.parts.wpn_fps_lmg_hk21_fg_short.stats = { concealment = 0, weight = 9-2, barrel_length = 12, md_code = {0,2,0,0,0}, md_bulk = {1,1}, value = 4 }
	...
	self.parts.wpn_fps_lmg_hk21_s_standard.stats = { concealment = 0, weight = 3, length = 10, shouldered = true }
	...
	self.parts.wpn_fps_lmg_hk51b_b_standard.stats = { concealment = 0, weight = 5, barrel_length = 8.9 }
	self.parts.wpn_fps_lmg_hk51b_b_fluted.stats = { concealment = 0, weight = 7, barrel_length = 13 } --roughly
	...
	self.parts.wpn_fps_lmg_hk51b_s_collapsed.stats = { concealment = 0, weight = 3, length = 5, shouldered = true, shoulderable = true }
	self.parts.wpn_fps_lmg_hk51b_s_extended.is_a_unlockable = true
	self.parts.wpn_fps_lmg_hk51b_s_extended.for_free = true
	...
	self.parts.wpn_fps_lmg_hk51b_s_extended.stats = { concealment = 0, weight = 3, length = 1, shoulderable = true }
	...
	self.parts.wpn_fps_lmg_m249_b_short.stats = { concealment = 0, weight = 10, barrel_length = 13.7, md_code = {0,2,0,0,0}, md_bulk = {1,1} }
	self.parts.wpn_fps_lmg_m249_b_long.stats = { concealment = 0, weight = 12, barrel_length = 16.1, md_code = {0,2,0,0,0}, md_bulk = {1,1} }
	...
	self.parts.wpn_fps_lmg_m249_s_modern.stats = { concealment = 0, weight = 2, length = 7, shouldered = true }
	self.parts.wpn_fps_lmg_m249_s_para.desc_id = "bm_wp_stock_collapsable_desc"
	self.parts.wpn_fps_lmg_m249_s_para.stats = { concealment = 0, weight = 6, length = 10.04, shouldered = true, shoulderable = true }
	self.parts.wpn_fps_lmg_m249_s_solid.stats = { concealment = 0, weight = 5, length = 10, shouldered = true }
	...
	self.parts.wpn_fps_lmg_m60_b_short.stats = { concealment = 0, weight = 13, barrel_length = 17.3 }
	self.parts.wpn_fps_lmg_m60_b_standard.stats = { concealment = 0, weight = 17, barrel_length = 22 }
	...
	table.addto(self.parts.wpn_fps_lmg_m60_o_sight.forbids, table.without(self.nqr.all_sights, "wpn_fps_o_blank"))
	...
	self.parts.wpn_fps_lmg_mg42_b_mg42.stats = { concealment = 0, weight = 16, barrel_length = 20.9, sightheight = self.wpn_fps_lmg_mg42.sightheight_mod }
	self.parts.wpn_fps_lmg_mg42_b_mg34.sub_type = "ironsight"
	self.parts.wpn_fps_lmg_mg42_b_mg34.stats = { concealment = 0, weight = 16, barrel_length = 20.9, sightheight = self.wpn_fps_lmg_mg42.sightheight_mod } --todo sight
	...
	self.parts.wpn_fps_lmg_mg42_b_vg38.stats = { concealment = 0, weight = 16, barrel_length = 20.9 }
	...
	self.parts.wpn_fps_lmg_par_b_short.forbids = { "wpn_fps_lmg_svinet_b_standard" }
	self.parts.wpn_fps_lmg_par_b_short.stats = { concealment = 0, weight = 11, barrel_length = 15, md_code = {0,2,0,0,0}, md_bulk = {1,1} } --roughly
	self.parts.wpn_fps_lmg_par_b_standard.stats = { concealment = 0, weight = 16, barrel_length = 21.7, md_code = {0,2,0,0,0}, md_bulk = {1,1} }
	self.parts.wpn_fps_lmg_svinet_b_standard.is_a_unlockable = true
	self.parts.wpn_fps_lmg_svinet_b_standard.pcs = {}
	self.parts.wpn_fps_lmg_svinet_b_standard.type = "wep_cos"
	table.addto(self.parts.wpn_fps_lmg_svinet_b_standard.forbids, self.nqr.all_bxs)
	self.parts.wpn_fps_lmg_svinet_b_standard.visibility = { { objects = { g_b = false } } }
	self.parts.wpn_fps_lmg_svinet_b_standard.stats = { concealment = 0, weight = 7 }
	...
	self.parts.wpn_fps_lmg_par_s_plastic.stats = { concealment = 0, weight = 5, shouldered = true }
	self.parts.wpn_fps_lmg_par_s_standard.stats = { concealment = 0, weight = 6, shouldered = true }
	self.parts.wpn_fps_lmg_par_upper_reciever.stats = { concealment = 0, weight = 0 }
	self.parts.wpn_fps_lmg_svinet_s_legend.is_a_unlockable = true
	self.parts.wpn_fps_lmg_svinet_s_legend.pcs = {}
	self.parts.wpn_fps_lmg_svinet_s_legend.stats = { concealment = 0, weight = 9, length = 1, shouldered = true }
	...
	self.parts.wpn_fps_lmg_kacchainsaw_barrel_standard.stats = { concealment = 0, weight = 9, barrel_length = 12.5 }
	...
	self.parts.wpn_fps_lmg_kacchainsaw_b_short.stats = { concealment = 0, weight = 11, barrel_length = 15 }
	self.parts.wpn_fps_lmg_kacchainsaw_b_long.stats = { concealment = 0, weight = 15, barrel_length = 20 }
	...
	self.parts.wpn_fps_lmg_kacchainsaw_mag_a.value = 1
	self.parts.wpn_fps_lmg_kacchainsaw_mag_a.stats = self.parts.wpn_fps_lmg_m249_m_standard.stats
	self.parts.wpn_fps_lmg_kacchainsaw_mag_b.value = 1
...
----SMG
	...
	table.insert(self.wpn_fps_shot_saiga.uses_parts, "wpn_fps_smg_coal_o_scopemount_standard")
	self.parts.wpn_fps_smg_coal_b_standard.stats = { concealment = 0, weight = 5, barrel_length = 9.1 }
	...
	self.parts.wpn_fps_smg_coal_m_standard.stats = { concealment = 12, weight = 4, mag_amount = { 2, 3, 4 }, CLIP_AMMO_MAX = { ["9x18"] = 64, ["9x19"] = 53 }, retention = false } --roughly
	...
	self.parts.wpn_fps_smg_coal_s_standard.stats = { concealment = 0, weight = 3, length = 8, shouldered = true, shoulderable = true }
	...
	self.wpn_fps_smg_erma.sightheight_mod = -0.4
	self.parts.wpn_fps_smg_erma_b_standard.stats = { concealment = 0, weight = 5, barrel_length = 9.9 }
	...
	self.parts.wpn_fps_smg_erma_extra_rail.rails = { "top" }
	self.parts.wpn_fps_smg_erma_extra_rail.stats = { concealment = 4, weight = 1 }
	self.parts.wpn_fps_smg_erma_mag_standard.stats = { concealment = 4, weight = 2, mag_amount = { 4, 6, 8 }, CLIP_AMMO_MAX = 32, retention = false }
	...
	self.parts.wpn_fps_smg_erma_s_folded.is_a_unlockable = true
	self.parts.wpn_fps_smg_erma_s_folded.for_free = true
	self.parts.wpn_fps_smg_erma_s_folded.stats = { concealment = 0, weight = 4, shoulderable = true }
	self.parts.wpn_fps_smg_erma_s_unfolded.stats = { concealment = 0, weight = 4, length = 8, shouldered = true, shoulderable = true }
	...
	table.insert(self.wpn_fps_smg_erma.uses_parts, "wpn_fps_upg_a_subfmj")
	table.addto(self.wpn_fps_smg_erma.uses_parts, self.nqr.all_angled_sights)

	self.parts.wpn_fps_smg_m45_b_small.stats = { concealment = 0, weight = 2, barrel_length = 4.5 } --roughly
	self.parts.wpn_fps_smg_m45_b_standard.stats = { concealment = 0, weight = 4, barrel_length = 8.3 }
	self.parts.wpn_fps_smg_m45_b_green.stats = { concealment = 0, weight = 4, barrel_length = 8.3 }
	...
	self.parts.wpn_fps_smg_m45_m_extended.stats = { concealment = 6, weight = 2, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = 50 }
	self.parts.wpn_fps_smg_m45_m_mag.stats = { concealment = 4, weight = 1, mag_amount = { 4, 6, 8 }, CLIP_AMMO_MAX = 36 }
	self.parts.wpn_fps_smg_m45_s_folded.is_a_unlockable = true
	self.parts.wpn_fps_smg_m45_s_folded.for_free = true
	self.parts.wpn_fps_smg_m45_s_folded.stats = { concealment = 0, weight = 3, shoulderable = true }
	self.parts.wpn_fps_smg_m45_s_standard.stats = { concealment = 0, weight = 3, length = 10, shouldered = true, shoulderable = true }
	...
	table.insert(self.wpn_fps_smg_m45.uses_parts, "wpn_fps_upg_a_subfmj")
	table.addto(self.wpn_fps_smg_m45.uses_parts, self.nqr.all_angled_sights)

	self.parts.wpn_fps_smg_mp5_b_m5k.stats = { concealment = 0, weight = 2, barrel_length = 4.5 }
	self.parts.wpn_fps_smg_mp5_b_mp5a4.stats = { concealment = 0, weight = 4, barrel_length = 8.9 }
	self.parts.wpn_fps_smg_mp5_b_mp5a5.stats = { concealment = 0, weight = 4, barrel_length = 8.9 }
	self.parts.wpn_fps_smg_mp5_b_mp5sd.stats = { concealment = 0, weight = 0 }
	...
	self.parts.wpn_fps_smg_mp5_fg_flash.stats = { concealment = 0, weight = 3, gadget_power = { flashlight = 2 } }
	...
	self.parts.wpn_fps_smg_mp5_fg_m5k.override = { wpn_fps_smg_mp5_fg_mp5a4 = { unit = fantom_unit, stats = {} } }
	self.parts.wpn_fps_smg_mp5_fg_m5k.stats = { concealment = 0, weight = 2+3-1, barrel_length = 4.5, value = 4 }
	self.parts.wpn_fps_smg_mp5_fg_mp5a4.adds = { "wpn_fps_smg_mp5_b_mp5a5" }
	self.parts.wpn_fps_smg_mp5_fg_mp5a4.stats = { concealment = 0, weight = 4+1, barrel_length = 8.9 }
	self.parts.wpn_fps_smg_mp5_fg_mp5a5.rails = { "side", "bottom" }
	self.parts.wpn_fps_smg_mp5_fg_mp5a5.stats = { concealment = 0, weight = 5-1 }
	self.parts.wpn_fps_smg_mp5_fg_mp5sd.desc_id = "bm_wp_bleedoff"
	...
	self.parts.wpn_fps_smg_mp5_fg_mp5sd.override = { wpn_fps_smg_mp5_fg_mp5a4 = { unit = fantom_unit, stats = {} } }
	self.parts.wpn_fps_smg_mp5_fg_mp5sd.override.wpn_fps_smg_mp5_fg_mp5a5 = { override = {} }
	self.parts.wpn_fps_smg_mp5_fg_mp5sd.stats = { concealment = 0, weight = 3+1+5, barrel_length = 5.7, bleedoff = 0.85, length = 6, md_code = {4,0,0,0,0}, value = 6 }
	self.parts.wpn_fps_smg_mp5_m_std.stats = { concealment = 4, weight = 1, mag_amount = { 4, 6, 8 }, CLIP_AMMO_MAX = { ["9x19"] = 30 } }
	self.parts.wpn_fps_smg_mp5_m_straight.stats = { concealment = 5, weight = 1, mag_amount = { 4, 6, 8 }, CLIP_AMMO_MAX = { [".40 S&W"] = 30 } }
	self.parts.wpn_fps_smg_mp5_m_drum.pcs = {}
	self.parts.wpn_fps_smg_mp5_m_drum.name_id = "bm_wp_mp5_m_drum"
	self.parts.wpn_fps_smg_mp5_m_drum.stats = { concealment = 18, weight = 5, mag_amount = { 1, 2, 3 }, CLIP_AMMO_MAX = { ["9x19"] = 70 }, retention = false } --roughly
	self.parts.wpn_fps_smg_mp5_s_adjust.desc_id = "bm_wp_stock_collapsable_desc"
	self.parts.wpn_fps_smg_mp5_s_adjust.stats = { concealment = 0, weight = 3, length = 8, shouldered = true, shoulderable = true }
	self.parts.wpn_fps_smg_mp5_s_folding.stats = { concealment = 0, weight = 3, length = 10, shouldered = true, shoulderable = true }
	self.parts.wpn_fps_smg_mp5_s_ring.stats = { concealment = 0, weight = 0, shouldered = false }
	self.parts.wpn_fps_smg_mp5_s_solid.stats = { concealment = 0, weight = 3, length = 9, shouldered = true }
	...
	table.insert(self.wpn_fps_smg_mp5.uses_parts, "wpn_fps_upg_a_subfmj")
	table.addto(self.wpn_fps_smg_mp5.uses_parts, self.nqr.all_angled_sights)

	self.parts.wpn_fps_smg_mp7_b_standard.stats = { concealment = 0, weight = 4, barrel_length = 7.1, md_code = {0,2,0,0,0}, md_bulk = {1,1} }
	...
	self.parts.wpn_fps_smg_mp7_s_long.is_a_unlockable = true
	self.parts.wpn_fps_smg_mp7_s_long.for_free = true
	self.parts.wpn_fps_smg_mp7_s_long.desc_id = "bm_wp_stock_collapsable_desc"
	self.parts.wpn_fps_smg_mp7_s_long.stats = { concealment = 0, weight = 3, length = 10.01, shouldered = true, shoulderable = true }
	self.parts.wpn_fps_smg_mp7_s_standard.desc_id = "bm_wp_stock_collapsable_desc"
	self.parts.wpn_fps_smg_mp7_s_standard.stats = { concealment = 0, weight = 3, length = 1, shoulderable = true }
	...
	self.parts.wpn_fps_smg_p90_b_short.stats = { concealment = 0, weight = 5, barrel_length = 10.4, md_code = {0,0,1,0,0}, md_bulk = {1,1} }
	self.parts.wpn_fps_smg_p90_b_long.value_mod = 0.8
	self.parts.wpn_fps_smg_p90_b_long.forbids = { "wpn_fps_smg_p90_b_ninja" }
	self.parts.wpn_fps_smg_p90_b_long.stats = { concealment = 0, weight = 8, barrel_length = 16, md_code = {0,0,1,0,0}, md_bulk = {1,1} }
	self.parts.wpn_fps_smg_p90_b_civilian.value_mod = 0.8
	...
	self.parts.wpn_fps_smg_p90_b_civilian.stats = { concealment = 0, weight = 11, barrel_length = 18.5 }
	self.parts.wpn_fps_smg_p90_b_legend_dummy = deep_clone(self.parts.wpn_fps_smg_p90_b_legend)
	self.parts.wpn_fps_smg_p90_b_legend_dummy.stats = {}
	self.parts.wpn_fps_smg_p90_b_legend.is_a_unlockable = true
	self.parts.wpn_fps_smg_p90_b_legend.pcs = {}
	self.parts.wpn_fps_smg_p90_b_legend.type = "lower_reciever"
	self.parts.wpn_fps_smg_p90_b_legend.a_obj = "a_body"
	self.parts.wpn_fps_smg_p90_b_legend.forbids = { "wpn_fps_smg_p90_b_ninja", "wpn_fps_smg_p90_b_civilian" }
	self.parts.wpn_fps_smg_p90_b_legend.override = {
		wpn_fps_smg_p90_b_short = { unit = self.parts.wpn_fps_smg_p90_b_legend.unit },
		wpn_fps_smg_p90_b_long = { adds = { "wpn_fps_smg_p90_b_legend_dummy" } },
	}
	self.parts.wpn_fps_smg_p90_b_legend.unit = self.parts.wpn_fps_smg_p90_body_p90.unit
	self.parts.wpn_fps_smg_p90_b_legend.stats = { concealment = 0, weight = 11, length = 1 }
	...
	self.parts.wpn_fps_smg_p90_m_std.stats = { concealment = 10, weight = 3, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = 50 }
	self.parts.wpn_fps_smg_p90_m_strap.stats = { concealment = 10, weight = 3, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = 50 }
	...
	self.parts.wpn_fps_smg_polymer_barrel_standard.stats = { concealment = 0, weight = 3, barrel_length = 5.5 }
	...
	self.parts.wpn_fps_smg_polymer_barrel_precision.value_mod = 0.6
	...
	self.parts.wpn_fps_smg_polymer_barrel_precision.stats = { concealment = 0, weight = 8, barrel_length = 16 }
	...
	self.parts.wpn_fps_smg_polymer_m_standard.stats = { concealment = 5, weight = 2, mag_amount = { 4, 6, 8 }, CLIP_AMMO_MAX = { [".45 ACP"] = 25, ["9x19"] = 30 }, retention = false }
	...
	self.parts.wpn_fps_smg_polymer_s_adapter.stats = { concealment = 0, weight = 1, legnth = 7, shoulderable = true }
	self.parts.wpn_fps_smg_polymer_s_standard.stats = { concealment = 0, weight = 3, length = 8, shouldered = true, shoulderable = true, cheek = -1 }
	self.wpn_fps_smg_polymer.override = {
		wpn_fps_addon_ris = { unit = "units/payday2/weapons/wpn_fps_shot_r870_pts/wpn_fps_shot_r870_gadget_rail" },
		wpn_fps_smg_schakal_vg_surefire_flashlight = { a_obj = "a_fg" },
		wpn_fps_smg_polymer_s_adapter = { forbids = {} },
		wpn_fps_upg_m4_s_ubr = { adds = { "wpn_fps_smg_polymer_s_adapter" } },
		wpn_fps_upg_ns_pis_large = { flip_a = 2 },
	}
	...
	table.insert(self.wpn_fps_smg_polymer.default_blueprint, "wpn_fps_upg_blankcal_45acp")
	table.insert(self.wpn_fps_smg_polymer.uses_parts, "wpn_fps_upg_blankcal_45acp")
	table.insert(self.wpn_fps_smg_polymer.uses_parts, "wpn_fps_upg_cal_9x19")
	table.insert(self.wpn_fps_smg_polymer.uses_parts, "wpn_fps_upg_a_subfmj")
	table.addto(self.wpn_fps_smg_polymer.uses_parts, self.nqr.all_tube_stocks)
	table.insert(self.wpn_fps_smg_polymer.uses_parts, "wpn_fps_upg_m4_s_ubr")

	table.addto(self.wpn_fps_smg_schakal.uses_parts, self.nqr.all_angled_sights)
	self.parts.wpn_fps_smg_schakal_b_standard.stats = { concealment = 0, weight = 4, barrel_length = 7.9 }
	self.parts.wpn_fps_smg_schakal_b_civil.value = 14
	self.parts.wpn_fps_smg_schakal_b_civil.stats = { concealment = 0, weight = 8, barrel_length = 16 }
	...
	self.parts.wpn_fps_smg_schakal_m_short.stats = { concealment = 3, weight = 1, mag_amount = { 4, 6, 8 }, CLIP_AMMO_MAX = { [".45 ACP"] = 15, ["9x19"] = 20 } }
	self.parts.wpn_fps_smg_schakal_m_standard.stats = { concealment = 5, weight = 2, mag_amount = { 4, 6, 8 }, CLIP_AMMO_MAX = { [".45 ACP"] = 25, ["9x19"] = 30 } }
	self.parts.wpn_fps_smg_schakal_m_long.stats = { concealment = 8, weight = 3, mag_amount = { 2, 3, 4 }, CLIP_AMMO_MAX = { [".45 ACP"] = 40, ["9x19"] = 50 } }
	...
	self.parts.wpn_fps_smg_schakal_s_civil.stats = { concealment = 0, weight = 4, shouldered = true }
	self.parts.wpn_fps_smg_schakal_s_folded.is_a_unlockable = true
	self.parts.wpn_fps_smg_schakal_s_folded.for_free = true
	self.parts.wpn_fps_smg_schakal_s_folded.stats = { concealment = 0, weight = 3, shoulderable = true }
	self.parts.wpn_fps_smg_schakal_s_standard.stats = { concealment = 0, weight = 3, length = 10, shouldered = true, shoulderable = true }
	...
	--self.parts.wpn_fps_smg_schakal_vg_surefire.sub_type = "flashlight"
	self.parts.wpn_fps_smg_schakal_vg_surefire.perks = nil --"flashlight"
	self.parts.wpn_fps_smg_schakal_vg_surefire.stats = { concealment = 0, weight = 6, gadget_power = { flashlight = 2 } }
	self.parts.wpn_fps_smg_schakal_vg_surefire_flashlight.type = "gadget_extra"
	self.wpn_fps_smg_schakal.override = {
		wpn_fps_upg_ns_pis_large = { flip_a = 2 },
	}
	table.insert(self.wpn_fps_smg_schakal.uses_parts, "wpn_fps_remove_s")
	table.insert(self.wpn_fps_smg_schakal.default_blueprint, "wpn_fps_upg_blankcal_45acp")
	table.insert(self.wpn_fps_smg_schakal.uses_parts, "wpn_fps_upg_blankcal_45acp")
	table.insert(self.wpn_fps_smg_schakal.uses_parts, "wpn_fps_upg_a_subfmj")
	table.insert(self.wpn_fps_smg_schakal.uses_parts, "wpn_fps_upg_cal_9x19")

	self.parts.wpn_fps_smg_shepheard_b_standard.stats = { concealment = 0, weight = 4, barrel_length = 8 }
	...
	self.parts.wpn_fps_smg_shepheard_body_short.value = 4
	...
	self.parts.wpn_fps_smg_shepheard_s_standard.stats = { concealment = 0, weight = 3, length = 7.03, shouldered = true, shoulderable = true }
	...
	self.wpn_fps_smg_shepheard.override = {
		...
		wpn_fps_smg_shepheard_s_adapter = { forbids = {} },
		wpn_fps_upg_m4_s_ubr = { adds = { "wpn_fps_smg_shepheard_s_adapter" } },
	}
	...
	table.insert(self.wpn_fps_smg_shepheard.uses_parts, "wpn_fps_remove_s")
	...
	table.insert(self.wpn_fps_smg_shepheard.uses_parts, "wpn_fps_upg_m4_s_ubr")
	table.addto(self.wpn_fps_smg_shepheard.uses_parts, self.nqr.all_m4_grips)
	table.insert(self.wpn_fps_smg_shepheard.uses_parts, "wpn_fps_upg_cal_40sw")
	table.insert(self.wpn_fps_smg_shepheard.uses_parts, "wpn_fps_upg_a_subfmj")
	...
	self.parts.wpn_fps_smg_sterling_b_e11.value_mod = 0.5
	self.parts.wpn_fps_smg_sterling_b_e11.stats = { concealment = 0, weight = 4+1+2, barrel_length = 7.7 }
	self.parts.wpn_fps_smg_sterling_b_short.value = 3
	self.parts.wpn_fps_smg_sterling_b_short.stats = { concealment = 0, weight = 2+0, barrel_length = 4.5 }
	self.parts.wpn_fps_smg_sterling_b_standard.value_mod = 0.5
	self.parts.wpn_fps_smg_sterling_b_standard.stats = { concealment = 0, weight = 4+1, barrel_length = 7.7 }
	self.parts.wpn_fps_smg_sterling_b_suppressed.desc_id = "bm_wp_bleedoff"
	self.parts.wpn_fps_smg_sterling_b_suppressed.value_mod = 0.5
	self.parts.wpn_fps_smg_sterling_b_suppressed.stats = { concealment = 0, weight = 4+9+1, barrel_length = 7.8, length = 5, bleedoff = 0.75, md_code = {4,0,0,0,0} }
	self.parts.wpn_fps_smg_sterling_b_long.value_mod = 0.5
	self.parts.wpn_fps_smg_sterling_b_long.stats = { concealment = 0, weight = 8+2, barrel_length = 16.3 }
	self.parts.wpn_fps_smg_sterling_body_standard.stats = { concealment = 0, weight = 0, length = 11 }
	self.parts.wpn_fps_smg_sterling_m_short.stats = { concealment = 2, weight = 1, mag_amount = { 4, 6, 8 }, CLIP_AMMO_MAX = 10, retention = false }
	...
	self.parts.wpn_fps_smg_sterling_s_folded.is_a_unlockable = true
	self.parts.wpn_fps_smg_sterling_s_folded.for_free = true
	self.parts.wpn_fps_smg_sterling_s_folded.stats = { concealment = 0, weight = 4, shoulderable = true }
	self.parts.wpn_fps_smg_sterling_s_nostock.pcs = nil
	...
	self.parts.wpn_fps_smg_sterling_s_solid.stats = { concealment = 0, weight = 3, length = 11, shouldered = true }
	self.parts.wpn_fps_smg_sterling_s_standard.stats = { concealment = 0, weight = 4, length = 8, shouldered = true, shoulderable = true }
	...
	table.insert(self.wpn_fps_smg_sterling.uses_parts, "wpn_fps_remove_s")
	table.insert(self.wpn_fps_smg_sterling.uses_parts, "wpn_fps_upg_a_subfmj")
	...
	self.parts.wpn_fps_smg_thompson_barrel_short.stats = { concealment = 0, weight = 4, barrel_length = 6 }
	self.parts.wpn_fps_smg_thompson_barrel.stats = { concealment = 0, weight = 7, barrel_length = 10.5 }
	self.parts.wpn_fps_smg_thompson_barrel_long.value = 12
	self.parts.wpn_fps_smg_thompson_barrel_long.stats = { concealment = 0, weight = 10, barrel_length = 16.5 }
	...
	self.parts.wpn_fps_smg_thompson_stock_discrete.stats = { concealment = 0, weight = 4, shouldered = true }
	self.parts.wpn_fps_smg_thompson_stock_nostock.is_a_unlockable = true
	self.parts.wpn_fps_smg_thompson_stock_nostock.for_free = true
	...
	table.insert(self.wpn_fps_smg_thompson.uses_parts, "wpn_fps_remove_s")

	self.parts.wpn_fps_smg_uzi_b_standard.stats = { concealment = 0, weight = 5, barrel_length = 10.2 }
	...
	self.parts.wpn_fps_smg_uzi_body_standard.stats = { concealment = 0, weight = 0, length = 7 }
	...
	self.parts.wpn_fps_smg_uzi_m_standard.stats = { concealment = 4, weight = 1, mag_amount = { 4, 6, 8 }, CLIP_AMMO_MAX = { ["9x19"] = 32, [".45 ACP"] = 16 } }
	...
	self.parts.wpn_fps_smg_uzi_s_standard.is_a_unlockable = true
	self.parts.wpn_fps_smg_uzi_s_standard.for_free = true
	...
	table.insert(self.wpn_fps_smg_uzi.uses_parts, "wpn_fps_upg_a_subfmj")
	...
	self.wpn_fps_smg_vityaz.sightheight_mod = 0.5
	...
	self.parts.wpn_fps_smg_vityaz_b_standard.stats = { concealment = 0, weight = 5, barrel_length = 9.4 }
	self.parts.wpn_fps_smg_vityaz_b_long.value_mod = 0.7
	self.parts.wpn_fps_smg_vityaz_b_long.adds = {}
	self.parts.wpn_fps_smg_vityaz_b_long.stats = { concealment = 0, weight = 8, barrel_length = 16 }
	self.parts.wpn_fps_smg_vityaz_body_upper = deep_clone(self.parts.wpn_fps_smg_vityaz_body_standard)
	self.parts.wpn_fps_smg_vityaz_body_upper.type = "upper_reciever"
	self.parts.wpn_fps_smg_vityaz_body_upper.rails = { "top" }
	self.parts.wpn_fps_smg_vityaz_body_upper.forbids = { "wpn_fps_upg_o_ak_scopemount", "wpn_fps_smg_coal_o_scopemount_standard" }
	self.parts.wpn_fps_smg_vityaz_body_upper.visibility = { { objects = { g_body = false, g_rearsight = false } } }
	self.parts.wpn_fps_smg_vityaz_body_upper.stats = { concealment = 0, weight = 3 }
	self.parts.wpn_fps_smg_vityaz_body_standard.sub_type = "ironsight"
	self.parts.wpn_fps_smg_vityaz_body_standard.visibility = { { objects = { g_dustcover = false } } }
	...
	self.parts.wpn_fps_smg_vityaz_m_standard.bullet_objects = { amount = 1, prefix = "g_bullet" }
	self.parts.wpn_fps_smg_vityaz_m_standard.stats = { concealment = 5, weight = 2, mag_amount = { 4, 6, 8 }, CLIP_AMMO_MAX = 30 }
	...
	self.parts.wpn_fps_smg_vityaz_s_short.stats = { concealment = 0, weight = 1, length = 1, shouldered = false }
	self.parts.wpn_fps_smg_vityaz_s_standard.stats = { concealment = 0, weight = 3, length = 8, shouldered = true, shoulderable = true }
	self.wpn_fps_smg_vityaz.override = {
		wpn_fps_upg_o_ak_scopemount = { stats = table.copy_append(self.parts.wpn_fps_upg_o_ak_scopemount.stats, { sightheight = 0 }), },
		wpn_fps_smg_coal_o_scopemount_standard = { a_obj = "a_o", stats = table.copy_append(self.parts.wpn_fps_upg_o_ak_scopemount.stats, { sightheight = 0 }), },
		wpn_fps_upg_ns_pis_large = { flip_a = 2 },
	}
	for i, k in pairs(self.nqr.all_gadgets) do self.wpn_fps_smg_vityaz.override[k] = { parent = "foregrip" } end
	table.deletefrom(self.wpn_fps_smg_vityaz.uses_parts, self.nqr.all_snoptics)
	table.insert(self.wpn_fps_smg_vityaz.default_blueprint, "wpn_fps_smg_vityaz_body_upper")
	table.insert(self.wpn_fps_smg_vityaz.uses_parts, "wpn_fps_smg_vityaz_body_upper")
	table.insert(self.wpn_fps_smg_vityaz.default_blueprint, 1, "wpn_fps_upper_lock_sights")
	table.insert(self.wpn_fps_smg_vityaz.uses_parts, "wpn_fps_upper_lock_sights")
	...
	table.insert(self.wpn_fps_smg_vityaz.uses_parts, "wpn_fps_remove_upper")
	table.insert(self.wpn_fps_smg_vityaz.uses_parts, "wpn_fps_upg_o_ak_scopemount")
	table.insert(self.wpn_fps_smg_vityaz.uses_parts, "wpn_fps_smg_coal_o_scopemount_standard") --todo
	table.insert(self.wpn_fps_smg_vityaz.uses_parts, "wpn_fps_upg_a_subfmj")
	...
	self.parts.wpn_fps_ass_sub2000_b_std.stats = { concealment = 0, weight = 8+2, barrel_length = 16.3 }
	...
	self.parts.wpn_fps_ass_sub2000_fg_railed.stats = { concealment = 0, weight = 5-2, sightheight = -0.9 }
	...
	self.parts.wpn_fps_ass_sub2000_fg_suppressed.value = 12
	...
	self.parts.wpn_fps_ass_sub2000_fg_suppressed.stats = { concealment = 0, weight = 5+5, barrel_length = 9, sightheight = -0.15 }
	...
	table.insert(self.wpn_fps_ass_sub2000.uses_parts, "wpn_fps_upg_a_subfmj")
	...
	self.parts.wpn_fps_smg_speen_lower_body_standard.stats = { concealment = 0, weight = 0, length = 14.011, shouldered = true }
	self.parts.wpn_fps_smg_speen_upper_body_standard.stats = { concealment = 0, weight = 0 }
	self.parts.wpn_fps_smg_speen_magazine_standard.stats = {}
	self.parts.wpn_fps_smg_speen_magazine_extended.stats = {}
	self.parts.wpn_fps_smg_speen_extra_magazine.type = "stock_addon"
	self.parts.wpn_fps_smg_speen_extra_magazine.forbids = {}
	self.parts.wpn_fps_smg_speen_extra_magazine.stats = { concealment = 3, weight = 1, totalammo = 19 }
	self.parts.wpn_fps_smg_speen_barrel_standard.stats = { concealment = 0, weight = 8, barrel_length = 16 }
	self.parts.wpn_fps_smg_speen_barrel_dmr.stats = { concealment = 0, weight = 9, barrel_length = 18 }
	self.parts.wpn_fps_smg_speen_sights_front_standard.type = "ironsight"
	self.parts.wpn_fps_smg_speen_sights_front_standard.forbids = { "wpn_fps_gadgets_pos_a_fl_upper" }
	table.addto(self.parts.wpn_fps_smg_speen_sights_front_standard.forbids, table.combine(self.nqr.i_f_base_lpvo, self.nqr.i_f_spot, self.nqr.i_f_c96, self.nqr.i_f_acog))
	self.parts.wpn_fps_smg_speen_sights_front_standard.override = {}
	self.parts.wpn_fps_smg_speen_sights_front_standard.stats = { concealment = 2, weight = 2 }
	self.parts.wpn_fps_smg_speen_sights_rear_standard.stats = {}
	self.wpn_fps_smg_speen.override = {
		wpn_fps_addon_ris = { parent = "upper_body", a_obj = "a_fl_2" },
		wpn_nqr_extra3_rail = { parent = "lower_body" },
		wpn_fps_pis_g18c_m_mag_17rnd = {
			parent = "lower_body",
			unit = self.parts.wpn_fps_smg_speen_magazine_standard.unit,
			third_unit = self.parts.wpn_fps_smg_speen_magazine_standard.third_unit,
			stats = { concealment = 3, weight = 1, mag_amount = { 6, 9, 12 }, CLIP_AMMO_MAX = { ["9x19"] = 19, [".40 S&W"] = 16 } },
		},
		wpn_fps_pis_g18c_m_mag_33rnd = {
			parent = "lower_body",
			unit = self.parts.wpn_fps_smg_speen_magazine_extended.unit,
			third_unit = self.parts.wpn_fps_smg_speen_magazine_extended.third_unit,
			stats = { concealment = 6, weight = 2, mag_amount = { 4, 6, 8 }, CLIP_AMMO_MAX = { ["9x19"] = 33, [".40 S&W"] = 29 } },
		},
		wpn_fps_upg_ns_pis_large = { flip_a = 2 },
	}
	for i, k in pairs(self.nqr.all_sights) do if not self.parts[k].parent then self.wpn_fps_smg_speen.override[k] = { parent = "upper_body", a_obj = "a_o_optics" } end end
	--for i, k in pairs(table.with(self.nqr.all_vertical_grips)) do self.wpn_fps_smg_speen.override[k] = { parent = "lower_body" } end
	for i, k in pairs(self.nqr.all_gadgets) do self.wpn_fps_smg_speen.override[k] = { parent = "upper_body", a_obj = "a_fl_2" } end
	table.delete(self.wpn_fps_smg_speen.uses_parts, "wpn_fps_smg_speen_magazine_extended")
	table.deletefrom(self.wpn_fps_smg_speen.uses_parts, self.nqr.all_big_snoptics)
	table.swap(self.wpn_fps_smg_speen.default_blueprint, "wpn_fps_smg_speen_magazine_standard", "wpn_fps_pis_g18c_m_mag_17rnd")
	table.insert(self.wpn_fps_smg_speen.uses_parts, "wpn_fps_pis_g18c_m_mag_17rnd")
	table.insert(self.wpn_fps_smg_speen.default_blueprint, 1, "wpn_fps_extra2_lock_gadgets")
	table.insert(self.wpn_fps_smg_speen.uses_parts, "wpn_fps_extra2_lock_gadgets")
	table.insert(self.wpn_fps_smg_speen.uses_parts, "wpn_fps_pis_g18c_m_mag_33rnd")
	table.insert(self.wpn_fps_smg_speen.uses_parts, "wpn_fps_remove_ironsight")
	--table.insert(self.wpn_fps_smg_speen.uses_parts, "wpn_nqr_extra3_rail")
	table.insert(self.wpn_fps_smg_speen.uses_parts, "wpn_fps_gadgets_pos_a_fl_upper")
	table.addto(self.wpn_fps_smg_speen.uses_parts, self.nqr.all_angled_sights)



----MACHINE PISTOL
	self.wpn_fps_smg_fmg9.sightheight_mod = height_dflt+0.8
	self.parts.wpn_fps_smg_fmg9_b_dummy.stats = { concealment = 0, weight = 3, barrel_length = 7 }
	...
	self.parts.wpn_fps_smg_fmg9_m_speed.stats = { concealment = 4, weight = 2, mag_amount = { 4, 6, 8 }, CLIP_AMMO_MAX = 27 }
	self.parts.wpn_fps_smg_fmg9_m_standard.parent = "grip_base"
	self.parts.wpn_fps_smg_fmg9_m_standard.stats = { concealment = 4, weight = 2, mag_amount = { 4, 6, 8 }, CLIP_AMMO_MAX = 27 }
	...
	table.addto(self.parts.wpn_fps_smg_fmg9_o_sight.forbids, table.without(self.nqr.all_rifle_gadgets, self.nqr.all_rifle_to_pistol_gadgets))
	...
	table.addto(self.wpn_fps_smg_fmg9.uses_parts, self.nqr.all_rifle_gadgets)
	table.insert(self.wpn_fps_smg_fmg9.uses_parts, "wpn_fps_upg_cal_40sw")
	table.insert(self.wpn_fps_smg_fmg9.uses_parts, "wpn_fps_upg_a_subfmj")
	...
	self.parts.wpn_fps_smg_baka_b_standard.stats = { concealment = 1, weight = 2, barrel_length = 4.6 }
	self.parts.wpn_fps_smg_baka_b_comp.forbids = { "wpn_fps_smg_baka_b_dummy" }
	self.parts.wpn_fps_smg_baka_b_comp.stats = { concealment = 0, weight = 2, barrel_length = 4.6, md_code = {0,0,1,0,0}, md_bulk = {1,1} }
	...
	self.parts.wpn_fps_smg_baka_m_standard.stats = { concealment = 4, weight = 1, mag_amount = { 1, 2, 3 }, CLIP_AMMO_MAX = 32 }
	self.parts.wpn_fps_smg_baka_o_adapter.unit = self.parts.wpn_fps_smg_pm9_o_adapter.unit
	...
	self.parts.wpn_fps_smg_baka_s_unfolded.is_a_unlockable = true
	self.parts.wpn_fps_smg_baka_s_unfolded.for_free = true
	self.parts.wpn_fps_smg_baka_s_unfolded.stats = { concealment = 0, weight = 2, length = 9, shouldered = true, shoulderable = true }
	self.wpn_fps_smg_baka.a_fl = 1
	...
	table.insert(self.wpn_fps_smg_baka.uses_parts, "wpn_fps_remove_s")

	self.parts.wpn_fps_smg_tec9_b_long.stats = { concealment = 0, weight = 3+1, barrel_length = 5 }
	self.parts.wpn_fps_smg_tec9_b_standard.value = 4
	self.parts.wpn_fps_smg_tec9_b_standard.forbids = { "wpn_fps_smg_tec9_ns_ext" }
	self.parts.wpn_fps_smg_tec9_b_standard.stats = { concealment = 0, weight = 2, barrel_length = 3 }
	self.parts.wpn_fps_smg_tec9_body_standard.stats = { concealment = 0, weight = 0, length = 7 }
	self.parts.wpn_fps_smg_tec9_m_extended.stats = { concealment = 6, weight = 2, mag_amount = { 1, 2, 3 }, CLIP_AMMO_MAX = 50 }
	self.parts.wpn_fps_smg_tec9_m_standard.stats = { concealment = 4, weight = 1, mag_amount = { 1, 2, 3 }, CLIP_AMMO_MAX = 32 }
	...
	self.parts.wpn_fps_smg_tec9_s_unfolded.stats = { concealment = 0, weight = 2, length = 8, shouldered = true }
	...
	self.parts.wpn_fps_smg_sr2_b_standard.stats = { concealment = 0, weight = 3, barrel_length = 6.9 }
	...
	self.parts.wpn_fps_smg_sr2_m_mag.stats = { concealment = 4, weight = 1, mag_amount = { 4, 6, 8 }, CLIP_AMMO_MAX = 30 }
	self.parts.wpn_fps_smg_sr2_m_quick.stats = { concealment = 4, weight = 1, mag_amount = { 4, 6, 8 }, CLIP_AMMO_MAX = 30 }
	self.parts.wpn_fps_smg_sr2_ns_silencer.stats = { concealment = 16, weight = 6, length = 8, md_code = {4,0,0,0,0} }
	...
	self.parts.wpn_fps_smg_sr2_s_folded.stats = { concealment = 0, weight = 2, shoulderable = true }
	self.parts.wpn_fps_smg_sr2_s_unfolded.is_a_unlockable = true
	self.parts.wpn_fps_smg_sr2_s_unfolded.for_free = true
	self.parts.wpn_fps_smg_sr2_s_unfolded.stats = { concealment = 0, weight = 2, length = 8, shouldered = true, shoulderable = true }
	...
	self.parts.wpn_fps_smg_scorpion_b_standard.stats = { concealment = 0, weight = 2, barrel_length = 4.5 }
	...
	self.parts.wpn_fps_smg_scorpion_m_standard.stats = { concealment = 2, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = 20 }
	self.parts.wpn_fps_smg_scorpion_m_extended.pcs = nil
	self.parts.wpn_fps_smg_scorpion_m_extended.stats = { concealment = 4, weight = 2, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = 20 }
	self.parts.wpn_fps_smg_scorpion_s_nostock.pcs = nil
	...
	self.parts.wpn_fps_smg_scorpion_s_standard.stats = { concealment = 0, weight = 2, shoulderable = true }
	self.parts.wpn_fps_smg_scorpion_s_unfolded.is_a_unlockable = true
	self.parts.wpn_fps_smg_scorpion_s_unfolded.for_free = true
	self.parts.wpn_fps_smg_scorpion_s_unfolded.stats = { concealment = 0, weight = 2, length = 10, shouldered = true, shoulderable = true }
	self.wpn_fps_smg_scorpion.a_fl = 1
	...
	table.insert(self.wpn_fps_smg_scorpion.uses_parts, "wpn_fps_remove_s")

	self.wpn_fps_smg_pm9.sightheight_mod = -1.65
	self.parts.wpn_fps_smg_pm9_b_short.pcs = nil
	self.parts.wpn_fps_smg_pm9_b_short.stats = { concealment = 0, weight = 2, barrel_length = 4.7 }
	...
	self.parts.wpn_fps_smg_pm9_m_quick.stats = { concealment = 4, weight = 1, mag_amount = { 4, 6, 8 }, CLIP_AMMO_MAX = 25 }
	self.parts.wpn_fps_smg_pm9_m_standard.stats = { concealment = 4, weight = 1, mag_amount = { 4, 6, 8 }, CLIP_AMMO_MAX = 25 }
	...
	self.parts.wpn_fps_smg_pm9_s_tactical.stats = { concealment = 0, weight = 3, length = 7, shouldered = true }
	self.wpn_fps_smg_pm9.a_fl = 1
	self.wpn_fps_smg_pm9.adds = {}
	self.wpn_fps_smg_pm9.override = { wpn_fps_upg_ns_pis_large = { flip_a = 2 } }
	table.addto_dict(self.wpn_fps_smg_pm9.override, deep_clone(overrides_vertical_grip_and_gadget_thing))
	...
	table.deletefrom(self.wpn_fps_smg_pm9.uses_parts, {"wpn_fps_ass_sub2000_ns_supp","wpn_fps_upg_ns_ass_filter"})
	table.deletefrom(self.wpn_fps_smg_pm9.uses_parts, table.without(self.nqr.all_sps2, {"wpn_fps_snp_victor_ns_hera_supp","wpn_fps_upg_ak_ns_tgp"}))
	table.deletefrom(self.wpn_fps_smg_pm9.uses_parts, table.without(self.nqr.all_sps3, {"wpn_fps_snp_desertfox_b_silencer"}))
	table.deletefrom(self.wpn_fps_smg_pm9.uses_parts, self.nqr.all_sps4)
	--table.deletefrom(self.wpn_fps_smg_pm9.uses_parts, self.nqr.all_sps)
	--table.addto(self.wpn_fps_smg_pm9.uses_parts, table.combine(self.nqr.all_sps1, {"wpn_fps_snp_victor_ns_hera_supp","wpn_fps_upg_ak_ns_tgp","wpn_fps_snp_model70_ns_suppressor","wpn_fps_snp_victor_ns_hera_supp","wpn_fps_snp_desertfox_b_silencer"}))
	...
	self.parts.wpn_fps_smg_mp9_b_dummy.stats = { concealment = 0, weight = 3, barrel_length = 5.1 }
	...
	self.parts.wpn_fps_smg_mp9_m_short.stats = { concealment = 2, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = 15 }
	self.parts.wpn_fps_smg_mp9_m_extended.stats = { concealment = 4, weight = 1, mag_amount = { 1, 2, 3 }, CLIP_AMMO_MAX = 30 }
	self.parts.wpn_fps_smg_mp9_s_fold.stats = { concealment = 0, weight = 2, length = 9, shouldered = true, shoulderable = true }
	self.parts.wpn_fps_smg_mp9_s_skel.pcs = nil
	...
	self.parts.wpn_fps_smg_mac10_b_dummy.stats = { concealment = 0, weight = 3, barrel_length = 5.1 }
	...
	self.parts.wpn_fps_smg_mac10_body_ris.override = {
		wpn_fps_gadgets_pos_a_vg = { forbids = table.without(self.nqr.all_rifle_gadgets, self.nqr.all_rifle_to_pistol_gadgets), adds = {} },
	}
	...
	self.parts.wpn_fps_smg_mac10_m_short.stats = { concealment = 2, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = 16 }
	self.parts.wpn_fps_smg_mac10_m_quick.stats = { concealment = 4, weight = 1, mag_amount = { 1, 2, 3 }, CLIP_AMMO_MAX = 32 }
	self.parts.wpn_fps_smg_mac10_m_extended.stats = { concealment = 4, weight = 1, mag_amount = { 1, 2, 3 }, CLIP_AMMO_MAX = 32 }
	self.parts.wpn_fps_smg_mac10_s_fold.stats = { concealment = 0, weight = 3, length = 10.01, shouldered = true, shoulderable = true }
	self.parts.wpn_fps_smg_mac10_s_fold2.is_a_unlockable = true
	self.parts.wpn_fps_smg_mac10_s_fold2.for_free = true
	self.parts.wpn_fps_smg_mac10_s_fold2.stats = { concealment = 0, weight = 3, length = 1, shoulderable = true }
	self.parts.wpn_fps_smg_mac10_s_skel.pcs = nil
	self.parts.wpn_fps_smg_mac10_s_skel.stats = {}
	self.wpn_fps_smg_mac10.adds = {}
	--self.wpn_fps_smg_mac10.override.wpn_fps_addon_ris = { override = { wpn_fps_extra_lock_gadgets = { forbids = table.with(self.nqr.all_pistol_gadgets, self.nqr.all_vertical_grips) } } }
	self.wpn_fps_smg_mac10.override.wpn_fps_addon_ris = { override = { wpn_fps_extra_lock_gadgets = { forbids = {} } } }
	...
	table.insert(self.wpn_fps_smg_mac10.uses_parts, "wpn_fps_gadgets_pos_a_vg")
	...
	self.parts.wpn_fps_smg_cobray_barrel.stats = { concealment = 0, weight = 3, barrel_length = 5.5 }
	...
	self.parts.wpn_fps_smg_cobray_m_standard.stats = { concealment = 4, weight = 1, mag_amount = { 1, 2, 3 }, CLIP_AMMO_MAX = 32 }
	...
	self.parts.wpn_fps_smg_cobray_o_adapter.stats = { concealment = 0, weight = 1, sightheight = 1 }
	self.parts.wpn_fps_smg_cobray_s_m4adapter.pcs = nil
	self.parts.wpn_fps_smg_cobray_s_m4adapter.stats = {}
	self.parts.wpn_fps_smg_cobray_s_standard.stats = { concealment = 0, weight = 3, length = 9.010, shouldered = true }
...
----SHOTGUN
	self.parts.wpn_fps_sho_coach_b_short.value = 1
	...
	self.parts.wpn_fps_shot_b682_b_short.value = 1
	...
	self.parts.wpn_fps_shot_huntsman_m.visibility = { { objects = { g_long_barrel = false } } }
	self.parts.wpn_fps_shot_huntsman_m.stats = {}
	self.parts.wpn_fps_shot_huntsman_b_short.value = 1
	...
	self.parts.wpn_fps_sho_boot_b_short.value = 1
	...
	self.parts.wpn_fps_sho_boot_b_long.value = 3
	...
	self.parts.wpn_fps_shot_m37_b_short.value = 2
	...
	self.parts.wpn_fps_shot_m1897_b_short.value = 2
	...
	self.parts.wpn_fps_shot_m1897_b_long.value = 4
	...
	self.parts.wpn_fps_shot_r870_b_short.value = 3
	self.parts.wpn_fps_shot_r870_b_short.forbids = {
		"wpn_fps_shot_r870_gadget_rail",
		"wpn_fps_shot_r870_s_nostock_big", "wpn_fps_shot_r870_b_legendary", "wpn_fps_shot_r870_fg_legendary",
	}
	self.parts.wpn_fps_shot_r870_b_short.override = {}
	self.parts.wpn_fps_shot_r870_b_short.stats = { concealment = 0, weight = 5+2, barrel_length = 9, CLIP_AMMO_MAX = 3 }
	self.parts.wpn_fps_shot_r870_b_long.pcs = {}
	self.parts.wpn_fps_shot_r870_b_long.value = 6
	self.parts.wpn_fps_shot_r870_b_long.forbids = { "wpn_fps_shot_shorty_b_legendary" }
	self.parts.wpn_fps_shot_r870_b_long.stats = { concealment = 0, weight = 11+5, barrel_length = 21, CLIP_AMMO_MAX = 8 }
	self.parts.wpn_fps_shot_r870_b_legendary.is_a_unlockable = true
	self.parts.wpn_fps_shot_r870_b_legendary.pcs = {}
	self.parts.wpn_fps_shot_r870_b_legendary.type = "exclusive_set"
	self.parts.wpn_fps_shot_r870_b_legendary.sub_type = "ironsight"
	self.parts.wpn_fps_shot_r870_b_legendary.forbids = {
		"wpn_fps_shot_r870_m_extended",
		"wpn_fps_shot_shorty_b_legendary", "wpn_fps_shot_shorty_s_legendary",
		"wpn_fps_shot_shorty_s_nostock_short", "wpn_fps_shot_r870_s_nostock_big", "wpn_fps_shot_r870_s_nostock_single",
		"wpn_fps_shot_r870_fg_wood",
	}
	table.addto(self.parts.wpn_fps_shot_r870_b_legendary.forbids, self.nqr.all_bxs)
	self.parts.wpn_fps_shot_r870_b_legendary.override = {
		wpn_fps_shot_r870_b_long = { unit = fantom_unit, stats = {} },
		wpn_fps_shot_r870_s_nostock = { unit = fantom_unit },
	}
	...
	self.parts.wpn_fps_shot_r870_fg_big.forbids = { "wpn_fps_shot_r870_b_short", "wpn_fps_shot_r870_b_legendary" }
	...
	self.parts.wpn_fps_shot_r870_fg_legendary.is_a_unlockable = true
	self.parts.wpn_fps_shot_r870_fg_legendary.pcs = {}
	self.parts.wpn_fps_shot_r870_fg_legendary.forbids = { "wpn_fps_shot_r870_b_short", "wpn_nqr_extra3_rail" }
	self.parts.wpn_fps_shot_r870_fg_legendary.stats = { concealment = 0, weight = 2 }
	self.parts.wpn_fps_shot_r870_gadget_rail.pcs = {} --todo wtf is this
	self.parts.wpn_fps_shot_r870_gadget_rail.name_id = "bm_wp_r870_gadgetrail"
	self.parts.wpn_fps_shot_r870_gadget_rail.type = "extra2"
	self.parts.wpn_fps_shot_r870_gadget_rail.rails = { "side" }
	self.parts.wpn_fps_shot_r870_gadget_rail.stats = { concealment = 0, weight = 1 }
	self.parts.wpn_fps_shot_r870_m_extended.type = "magazine_ext"
	self.parts.wpn_fps_shot_r870_m_extended.parent = "barrel"
	...
	self.parts.wpn_fps_shot_r870_b_short.override.wpn_fps_shot_r870_m_extended = {
	...
	self.parts.wpn_fps_shot_r870_s_folding.stats = { concealment = 0, weight = 6, shouldered = true }
	...
	self.parts.wpn_fps_shot_r870_s_nostock_big.stats = { concealment = 10, weight = 7, sightheight = 1 }
	self.parts.wpn_fps_shot_r870_s_nostock_single.pcs = {}
	self.parts.wpn_fps_shot_r870_s_nostock_single.type = "extra"
	--self.parts.wpn_fps_shot_r870_s_nostock_single.rails = { "top" } --flimsy
	self.parts.wpn_fps_shot_r870_s_nostock_single.forbids = { "wpn_fps_shot_r870_s_folding" }
	self.parts.wpn_fps_shot_r870_s_nostock_single.override = { wpn_fps_extra_lock_sights = { forbids = deep_clone(self.nqr.all_optics) } }
	self.parts.wpn_fps_shot_r870_s_nostock_single.visibility = { { objects = { g_folding = false } } }
	self.parts.wpn_fps_shot_r870_s_nostock_single.stats = { concealment = 4, weight = 2, sightheight = 1 }
	self.parts.wpn_fps_shot_r870_s_solid.forbids = { "wpn_fps_shot_r870_s_legendary" }
	self.parts.wpn_fps_shot_r870_s_solid.stats = { concealment = 0, weight = 3, length = 11, shouldered = true }
	...
	self.parts.wpn_fps_shot_r870_s_legendary.is_a_unlockable = true
	self.parts.wpn_fps_shot_r870_s_legendary.pcs = {}
	self.parts.wpn_fps_shot_r870_s_legendary.type = "grip"
	self.parts.wpn_fps_shot_r870_s_legendary.forbids = { "wpn_fps_shot_shorty_s_legendary" }
	self.parts.wpn_fps_shot_r870_s_legendary.stats = { concealment = 0, weight = 2 }
	...
	table.insert(self.wpn_fps_shot_r870.uses_parts, "wpn_fps_shot_r870_gadget_rail")
	...
	table.insert(self.wpn_fps_shot_r870.uses_parts, "wpn_fps_shot_shorty_b_legendary")
	table.insert(self.wpn_fps_shot_r870.uses_parts, "wpn_fps_shot_shorty_fg_legendary")
	table.insert(self.wpn_fps_shot_r870.uses_parts, "wpn_fps_shot_shorty_s_legendary")
	...
	self.parts.wpn_fps_shot_shorty_b_legendary.is_a_unlockable = true
	self.parts.wpn_fps_shot_shorty_b_legendary.pcs = {}
	self.parts.wpn_fps_shot_shorty_b_legendary.type = "magazine_ext"
	self.parts.wpn_fps_shot_shorty_b_legendary.forbids = {}
	table.addto(self.parts.wpn_fps_shot_shorty_b_legendary.forbids, self.nqr.all_sps)
	self.parts.wpn_fps_shot_shorty_b_legendary.override = {
		wpn_fps_shot_r870_b_short = { unit = self.parts.wpn_fps_shot_shorty_b_legendary.unit },
	}
	self.parts.wpn_fps_shot_shorty_b_legendary.unit = fantom_unit
	self.parts.wpn_fps_shot_shorty_b_legendary.stats = { concealment = 0, weight = 2, length = 3, CLIP_AMMO_MAX = 1, overlength = { barrel = { 1, self.parts.wpn_fps_shot_r870_b_short.stats.barrel_length } }, gadget_power = { laser = 1 } }
	self.parts.wpn_fps_shot_shorty_fg_legendary.is_a_unlockable = true
	self.parts.wpn_fps_shot_shorty_fg_legendary.pcs = {}
	self.parts.wpn_fps_shot_shorty_fg_legendary.stats = { concealment = 0, weight = 2 }
	self.parts.wpn_fps_shot_shorty_m_extended_short.pcs = nil
	...
	self.parts.wpn_fps_shot_shorty_s_nostock_short.stats = { concealment = 10, weight = 4, sightheight = 1 }
	self.parts.wpn_fps_shot_shorty_s_solid_short.pcs = nil
	self.parts.wpn_fps_shot_shorty_s_solid_short.stats = {}
	self.parts.wpn_fps_shot_shorty_s_legendary.is_a_unlockable = true
	self.parts.wpn_fps_shot_shorty_s_legendary.pcs = {}
	self.parts.wpn_fps_shot_shorty_s_legendary.forbids = { "wpn_fps_shot_r870_s_legendary" }
	table.addto(self.parts.wpn_fps_shot_shorty_s_legendary.forbids, table.without(self.nqr.all_m4_grips, "wpn_fps_upg_m4_g_standard"))
	self.parts.wpn_fps_shot_shorty_s_legendary.stats = { concealment = 0, weight = 2 }
	self.wpn_fps_shot_serbu.regression = "serbu"
	self.wpn_fps_shot_serbu.unit = self.wpn_fps_shot_r870.unit
	self.wpn_fps_shot_serbu.adds = {}
	self.wpn_fps_shot_serbu.override = {
		wpn_fps_shot_r870_b_short = { forbids = table.with(self.parts.wpn_fps_shot_r870_b_short.forbids, { "wpn_fps_shot_r870_fg_big", "wpn_fps_shot_r870_fg_wood", "wpn_fps_shot_r870_fg_legendary" }) },
		wpn_fps_shot_r870_fg_wood = { forbids = table.without(self.parts.wpn_fps_shot_r870_fg_wood.forbids, { "wpn_fps_shot_r870_b_short" }) },
		wpn_fps_shot_r870_fg_big = { forbids = table.without(self.parts.wpn_fps_shot_r870_fg_big.forbids, { "wpn_fps_shot_r870_b_short" }) },
		wpn_fps_shot_r870_fg_legendary = { forbids = table.without(self.parts.wpn_fps_shot_r870_fg_legendary.forbids, { "wpn_fps_shot_r870_b_short" }) },

		wpn_nqr_extra3_rail = { parent = "foregrip" },
	}
	--self.wpn_fps_shot_serbu.override.wpn_fps_shot_r870_b_legendary = { override = deep_clone(self.parts.wpn_fps_shot_r870_b_legendary.override) }
	--self.wpn_fps_shot_serbu.override.wpn_fps_shot_r870_b_legendary.override.wpn_fps_shot_r870_b_short.forbids = 
	--table.without(self.wpn_fps_shot_serbu.override.wpn_fps_shot_r870_b_short.forbids, { "wpn_fps_shot_r870_fg_legendary" })
	...
	table.insert(self.wpn_fps_shot_serbu.uses_parts, "wpn_fps_shot_r870_gadget_rail")
	table.insert(self.wpn_fps_shot_serbu.uses_parts, "wpn_fps_shot_r870_m_extended")
	table.insert(self.wpn_fps_shot_serbu.uses_parts, "wpn_fps_shot_r870_b_legendary")
	table.insert(self.wpn_fps_shot_serbu.uses_parts, "wpn_fps_shot_r870_fg_legendary")
	table.insert(self.wpn_fps_shot_serbu.uses_parts, "wpn_fps_shot_r870_s_legendary")
	...
	self.parts.wpn_fps_sho_supernova_b_short.value = 4
	...
	self.parts.wpn_fps_sho_supernova_b_long.value = 6
	...
	self.parts.wpn_fps_sho_ksg_b_short.value = 5
	...
	self.parts.wpn_fps_sho_ksg_b_short.stats = { concealment = 0, weight = 8+8, barrel_length = 16.1, CLIP_AMMO_MAX = 10 }
	...
	self.parts.wpn_fps_sho_ksg_b_standard.stats = { concealment = 0, weight = 9+9, barrel_length = 18.5, CLIP_AMMO_MAX = 14 }
	self.parts.wpn_fps_sho_ksg_b_long.value = 8
	self.parts.wpn_fps_sho_ksg_b_long.adds = {}
	self.parts.wpn_fps_sho_ksg_b_long.forbids = { "wpn_fps_sight_pos_a_o_f_short" }
	self.parts.wpn_fps_sho_ksg_b_long.override.wpn_fps_upg_o_mbus_front = { a_obj = "a_o_f_2" }
	self.parts.wpn_fps_sho_ksg_b_long.stats = { concealment = 0, weight = 11+10, barrel_length = 21, CLIP_AMMO_MAX = 18 } --roughly
	self.parts.wpn_fps_sho_ksg_b_legendary.is_a_unlockable = true
	self.parts.wpn_fps_sho_ksg_b_legendary.pcs = {}
	self.parts.wpn_fps_sho_ksg_b_legendary.override = { wpn_fps_upg_o_dd_front = { a_obj = "a_o_f_2" }, wpn_fps_upg_o_mbus_front = { a_obj = "a_o_f_2" } }
	self.parts.wpn_fps_sho_ksg_b_legendary.stats = { concealment = 0, weight = 12+12+3, barrel_length = 24, CLIP_AMMO_MAX = 22, gadget_power = { laser = 1 } } --roughly
	...
	self.parts.wpn_fps_sho_rota_b_short.value = 5
	...
	self.parts.wpn_fps_sho_rota_b_silencer.value = 7
	self.parts.wpn_fps_sho_rota_b_silencer.forbids = {}
	table.addto(self.parts.wpn_fps_sho_rota_b_silencer.forbids, self.nqr.all_bxs)
	self.parts.wpn_fps_sho_rota_b_silencer.stats = { concealment = 0, weight = 6+6, barrel_length = 12.5, length = 10, md_code = {3,0,0,0,0} }
	...
	self.parts.wpn_fps_sho_rota_b_salvo.value = 10
	...
	self.parts.wpn_fps_sho_rota_b_salvo.override = { wpn_fps_smg_hajk_vg_moe = { unit = fantom_unit, stats = {} } }
	self.parts.wpn_fps_sho_rota_b_salvo.stats = { concealment = 0, weight = 6+10, barrel_length = 12.5, length = 8, md_code = self.parts.wpn_fps_upg_ns_sho_salvo_large.stats.md_code }
	...
	self.parts.wpn_fps_sho_striker_b_long.value = 10
	self.parts.wpn_fps_sho_striker_b_long.stats = { concealment = 0, weight = 9, barrel_length = 18.5 }
	self.parts.wpn_fps_sho_striker_b_suppressed.value = 8
	...
	self.parts.wpn_fps_sho_ultima_ns_comp.stats = { concealment = 8, weight = 4, length = 3, md_code = {0,0,1,4,0} }
	...
	self.parts.wpn_fps_sho_ben_b_short.value = 9
	...
	self.parts.wpn_fps_sho_ben_b_long.value = 11
	...
	self.parts.wpn_fps_sho_aa12_barrel_long.value = 11
	...
	self.parts.wpn_fps_sho_basset_b_short.value = 10
	...
	self.parts.wpn_fps_sho_saiga_b_short.value = 12
	self.parts.wpn_fps_sho_saiga_b_short.override = { wpn_fps_o_pos_fg = { adds = {} } }
	self.parts.wpn_fps_sho_saiga_b_short.stats = { concealment = 0, weight = 5, barrel_length = 9.2 }
	...
	self.parts.wpn_fps_shot_saiga_b_standard.stats = { concealment = 0, weight = 8, barrel_length = 16.9 }
	self.parts.wpn_upg_saiga_fg_standard.pcs = {}
	self.parts.wpn_upg_saiga_fg_standard.forbids = { "wpn_fps_ak_extra_ris", "wpn_upg_ak_fl_legend" }
	...
	self.parts.wpn_fps_sho_saiga_fg_holy.forbids = { "wpn_fps_ak_extra_ris", "wpn_upg_ak_fl_legend" }
	...
	table.swap(self.wpn_fps_shot_saiga.uses_parts, "wpn_fps_ass_akm_body_upperreceiver", "wpn_fps_ass_74_body_upperreceiver")
	...
	self.parts.wpn_fps_sho_sko12_b_short.value = 12
	self.parts.wpn_fps_sho_sko12_b_short.stats = { concealment = 0, weight = 7, barrel_length = 14.7 }
	...
	self.parts.wpn_fps_sho_sko12_m_stick.stats = { concealment = 16, weight = 3, mag_amount = { 2, 3, 4 }, CLIP_AMMO_MAX = { ["12 gauge"] = 9 } }
	self.parts.wpn_fps_sho_sko12_m_drum.pcs = {}
	self.parts.wpn_fps_sho_sko12_m_drum.stats = { concealment = 54, weight = 8, mag_amount = { 1, 1, 1 }, CLIP_AMMO_MAX = { ["12 gauge"] = 25 }, retention = false }
	...
	self.parts.wpn_fps_sho_sko12_ns_default.stats = { concealment = 1, weight = 1, length = 1, md_code = {0,2,0,0,0} }
	...
	self.parts.wpn_fps_sho_sko12_ns_stiletto.stats = { concealment = 1, weight = 1, length = 1, md_code = {0,2,0,0,0} }
	self.parts.wpn_fps_sho_sko12_s_adapter.pcs = {}
	self.parts.wpn_fps_sho_sko12_s_adapter.type = "stock"
	self.parts.wpn_fps_sho_sko12_s_adapter.stats = { concealment = 0, weight = 2, length = 6 }
	self.parts.wpn_fps_sho_sko12_s_adapter_short.name_id = self.parts.wpn_fps_remove_s.name_id
	self.parts.wpn_fps_sho_sko12_s_adapter_short.pcs = {}
	self.parts.wpn_fps_sho_sko12_s_adapter_short.stats = { concealment = 0, weight = 1 }
	self.parts.wpn_fps_sho_sko12_stock.type = "stock_addon"
	self.parts.wpn_fps_sho_sko12_stock.adds_type = nil
	self.parts.wpn_fps_sho_sko12_stock.stats = { concealment = 11, weight = 2, length = 7, shouldered = true }
	self.parts.wpn_fps_sho_sko12_stock_conversion.stats = { concealment = 0, weight = 0, shouldered = true }
	--self.wpn_fps_sho_sko12.override.wpn_fps_remove_s = { unit = self.parts.wpn_fps_sho_sko12_s_adapter_short.unit } --gets fucked up at the bottom
	...
	table.swap(self.wpn_fps_sho_sko12.default_blueprint, "wpn_fps_sho_sko12_m_drum", "wpn_fps_sho_sko12_m_stick")
	...
	table.insert(self.wpn_fps_sho_sko12.uses_parts, "wpn_fps_sho_sko12_s_adapter_short")
...
----PISTOL
	...
	self.parts.wpn_fps_pis_g17_bb_standard.value = 3
	...
	self.parts.wpn_fps_pis_g17_bb_standard.stats = { concealment = 0, weight = 2, barrel_length = 4.5 }
	...
	self.wpn_fps_pis_g17.a_fl = 1
	self.wpn_fps_pis_g17.regression = "glock_17"
	self.wpn_fps_pis_g17.unit = self.wpn_fps_pis_g18c.unit
	...
	table.insert(self.wpn_fps_pis_g17.uses_parts, "wpn_fps_upg_a_subfmj")
	...
	self.parts.wpn_fps_pis_g18c_bb_standard.value = 3
	self.parts.wpn_fps_pis_g18c_bb_standard.adds = { "wpn_fps_pis_g18c_b_dummy" }
	self.parts.wpn_fps_pis_g18c_bb_standard.forbids = { "wpn_fps_pis_g22c_b_long" }
	...
	self.parts.wpn_fps_pis_g18c_bb_standard.stats = { concealment = 0, weight = 2, barrel_length = 4.5, md_code = {0,0,1,0,0} }
	...
	self.parts.wpn_fps_pis_g18c_m_mag_17rnd.bullet_objects = { amount = 2, prefix = "g_bullet_" }
	...
	self.parts.wpn_fps_pis_g18c_m_mag_33rnd.stats = { concealment = 4, weight = 2, mag_amount = { 1, 2, 3 }, CLIP_AMMO_MAX = { ["9x19"] = 33, [".40 S&W"] = 29 } }
	self.parts.wpn_fps_pis_g18c_s_stock.stats = { concealment = 0, weight = 0, shouldered = true }
	self.wpn_fps_pis_g18c.a_fl = 1
	...
	table.insert(self.wpn_fps_pis_g18c.uses_parts, "wpn_fps_upg_a_subfmj")
	...
	self.parts.wpn_fps_pis_g22c_bb_vented.value = 3
	...
	self.parts.wpn_fps_pis_g22c_bb_vented.stats = { concealment = 0, weight = 2, barrel_length = 4.5, md_code = {0,0,1,0,0} }
	...
	self.parts.wpn_fps_pis_g22c_bb_long_vented.value = 5
	...
	self.parts.wpn_fps_pis_g22c_bb_long_vented.stats = { concealment = 0, weight = 3, barrel_length = 5.3, md_code = {0,0,1,0,0} }
	...
	self.parts.wpn_fps_pis_g22c_bb_long.value = 5
	...
	self.parts.wpn_fps_pis_g22c_bb_long.stats = { concealment = 0, weight = 3, barrel_length = 5.3 }
	...
	self.wpn_fps_pis_g22c.a_fl = 1
	self.wpn_fps_pis_g22c.regression = "g22c"
	self.wpn_fps_pis_g22c.unit = self.wpn_fps_pis_g18c.unit
	self.wpn_fps_pis_g22c.animations = nil
	table.swap(self.wpn_fps_pis_g22c.default_blueprint, "wpn_fps_pis_g22c_body_standard", "wpn_fps_pis_g18c_body_frame")
	...
	table.insert(self.wpn_fps_pis_g22c.default_blueprint, "wpn_fps_upg_blankcal_45acp")
	table.insert(self.wpn_fps_pis_g22c.uses_parts, "wpn_fps_upg_blankcal_45acp")
	table.insert(self.wpn_fps_pis_g22c.uses_parts, "wpn_fps_upg_a_subfmj")
	...
	self.parts.wpn_fps_pis_g26_bb_standard.value = 1
	...
	self.parts.wpn_fps_pis_g26_bb_standard.stats = { concealment = 0, weight = 2, barrel_length = 3.4 }
	...
	self.parts.wpn_fps_pis_g26_fl_adapter.name_id = "bm_wp_g26_gadgetrail"
	...
	self.parts.wpn_fps_pis_g26_fl_adapter.stats = { concealment = 1, weight = 1 }
	self.parts.wpn_fps_pis_g26_g_gripforce.stats = { concealment = 0, weight = 0 }
	self.parts.wpn_fps_pis_g26_g_laser.stats = { concealment = 0, weight = 0, gadget_power = { laser = 1 } }
	...
	table.insert(self.wpn_fps_pis_g26.uses_parts, "wpn_fps_upg_a_subfmj")
	table.insert(self.wpn_fps_pis_g26.uses_parts, "wpn_fps_upg_cal_40sw")

	self.wpn_fps_pis_maxim9.sightheight_mod = 0.23
	self.parts.wpn_fps_pis_maxim9_rail = deep_clone(self.parts.wpn_fps_pis_maxim9_b_standard)
	self.parts.wpn_fps_pis_maxim9_rail.pcs = {}
	self.parts.wpn_fps_pis_maxim9_rail.name_id = "bm_wp_maxim9_gadgetrail"
	self.parts.wpn_fps_pis_maxim9_rail.perks = nil
	self.parts.wpn_fps_pis_maxim9_rail.sound_switch = nil
	self.parts.wpn_fps_pis_maxim9_rail.type = "extra3"
	self.parts.wpn_fps_pis_maxim9_rail.rails = { "bottom" }
	self.parts.wpn_fps_pis_maxim9_rail.visibility = { { objects = { g_b = false, g_sight_front = false } } }
	self.parts.wpn_fps_pis_maxim9_rail.stats = { concealment = 0, weight = 1 }
	self.parts.wpn_fps_pis_maxim9_b_standard.visibility = { { objects = { g_rail = false } } }
	self.parts.wpn_fps_pis_maxim9_b_standard.stats = { concealment = 0, weight = 2, barrel_length = 4.4, length = 1, md_code = {2,0,0,0,0} }
	self.parts.wpn_fps_pis_maxim9_b_long.forbids = { "wpn_fps_pis_maxim9_b_marksman" }
	self.parts.wpn_fps_pis_maxim9_b_long.visibility = { { objects = { g_long = false } } }
	self.parts.wpn_fps_pis_maxim9_b_long.stats = { concealment = 8, weight = 2+2, barrel_length = 4.4, length = 3, md_code = {3,0,0,0,0} }
	self.parts.wpn_fps_pis_maxim9_b_marksman.type = "barrel_ext"
	self.parts.wpn_fps_pis_maxim9_b_marksman.stats = { concealment = 6, weight = 3, length = 3 }
	...
	self.wpn_fps_pis_maxim9.a_fl = 1
	...
	table.insert(self.wpn_fps_pis_maxim9.default_blueprint, 1, "wpn_fps_extra3_lock_gadgets")
	table.insert(self.wpn_fps_pis_maxim9.uses_parts, "wpn_fps_extra3_lock_gadgets")
	table.insert(self.wpn_fps_pis_maxim9.uses_parts, "wpn_fps_pis_g18c_m_mag_33rnd")
	table.insert(self.wpn_fps_pis_maxim9.uses_parts, "wpn_fps_pis_maxim9_rail")
	...
	table.insert(self.wpn_fps_pis_maxim9.uses_parts, "wpn_fps_upg_a_subfmj")

	self.parts.wpn_fps_pis_beretta_b_std.type = "barrel"
	self.parts.wpn_fps_pis_beretta_b_std.stats = { concealment = 0, weight = 2, barrel_length = 4.9 }
	...
	self.parts.wpn_fps_pis_beretta_m_std.stats = { concealment = 2, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = 15 }
	self.parts.wpn_fps_pis_beretta_m_extended.stats = { concealment = 4, weight = 2, mag_amount = { 1, 2, 3 }, CLIP_AMMO_MAX = 30 }
	...
	self.wpn_fps_pis_beretta.a_fl = 1
	...
	table.insert(self.wpn_fps_pis_beretta.uses_parts, "wpn_fps_upg_a_subfmj")

	self.parts.wpn_fps_pis_beer_b_std.stats = { concealment = 0, weight = 2, barrel_length = 4.9, md_code = {0,0,1,0,0}, md_bulk = {1,1} }
	...
	self.parts.wpn_fps_pis_beer_b_robo.value = 15
	...
	self.parts.wpn_fps_pis_beer_b_robo.stats = { concealment = 18, weight = 5+6, barrel_length = 10, md_code = {0,0,0,1,0}, sightheight = 1.5, value = 4 }
	...
	self.parts.wpn_fps_pis_beer_m_extended.stats = { concealment = 3, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = 20 }
	self.parts.wpn_fps_pis_beer_m_std.stats = { concealment = 2, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = 15 }
	...
	table.insert(self.wpn_fps_pis_beer.uses_parts, "wpn_fps_upg_a_subfmj")

	self.parts.wpn_fps_pis_holt_b_standard.stats = { concealment = 0, weight = 2, barrel_length = 4.3 }
	...
	self.parts.wpn_fps_pis_holt_m_extended.stats = { concealment = 4, weight = 2, mag_amount = { 1, 2, 3 }, CLIP_AMMO_MAX = 25 } --roughly
	self.parts.wpn_fps_pis_holt_m_standard.stats = { concealment = 2, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = 15 }
	self.wpn_fps_pis_holt.a_fl = 1
	table.insert(self.wpn_fps_pis_holt.uses_parts, "wpn_fps_upg_a_subfmj")

	self.parts.wpn_fps_pis_p226_b_barrel_standard.value = 1
	self.parts.wpn_fps_pis_p226_b_barrel_standard.forbids = { "wpn_fps_pis_p226_b_long" }
	self.parts.wpn_fps_pis_p226_b_barrel_standard.stats = { concealment = 0, weight = 2, barrel_length = 4.4 }
	self.parts.wpn_fps_pis_p226_b_barrel_equinox.pcs = {}
	self.parts.wpn_fps_pis_p226_b_barrel_equinox.value = 2
	self.parts.wpn_fps_pis_p226_b_barrel_equinox.forbids = { "wpn_fps_pis_p226_b_long" }
	self.parts.wpn_fps_pis_p226_b_barrel_equinox.stats = { concealment = 0, weight = 2, barrel_length = 4.4 }
	self.parts.wpn_fps_pis_p226_b_barrel_long.pcs = {}
	self.parts.wpn_fps_pis_p226_b_barrel_long.value = 3
	...
	self.parts.wpn_fps_pis_p226_b_barrel_long.stats = { concealment = 0, weight = 3, barrel_length = 6 }
	...
	self.parts.wpn_fps_pis_p226_m_standard.stats = { concealment = 2, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = { [".40 S&W"] = 13, ["9x19"] = 15 } }
	self.parts.wpn_fps_pis_p226_m_extended.stats = { concealment = 4, weight = 2, mag_amount = { 1, 2, 3 }, CLIP_AMMO_MAX = { [".40 S&W"] = 22, ["9x19"] = 32 } }
	...
	table.insert(self.wpn_fps_pis_p226.default_blueprint, "wpn_fps_upg_blankcal_45acp")
	table.insert(self.wpn_fps_pis_p226.uses_parts, "wpn_fps_upg_blankcal_45acp")
	table.insert(self.wpn_fps_pis_p226.uses_parts, "wpn_fps_upg_a_subfmj")
	...
	self.parts.wpn_fps_pis_hs2000_b_standard.value = 1
	...
	self.parts.wpn_fps_pis_hs2000_b_standard.stats = { concealment = 0, weight = 2, barrel_length = 4.5 }
	self.parts.wpn_fps_pis_hs2000_b_custom.pcs = {}
	self.parts.wpn_fps_pis_hs2000_b_custom.value = 2
	...
	self.parts.wpn_fps_pis_hs2000_b_custom.stats = { concealment = 0, weight = 2, barrel_length = 4, md_code = {0,0,1,0,0} }
	self.parts.wpn_fps_pis_hs2000_b_long.pcs = {}
	self.parts.wpn_fps_pis_hs2000_b_long.value = 3
	...
	self.parts.wpn_fps_pis_hs2000_b_long.stats = { concealment = 0, weight = 3, barrel_length = 5.2 }
	...
	self.parts.wpn_fps_pis_hs2000_m_extended.stats = { concealment = 4, weight = 2, mag_amount = { 1, 2, 3 }, CLIP_AMMO_MAX = { [".45 ACP"] = 25, [".40 S&W"] = 28, ["9x19"] = 32 } }
	self.parts.wpn_fps_pis_hs2000_m_standard.stats = { concealment = 2, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = { [".45 ACP"] = 13, [".40 S&W"] = 16, ["9x19"] = 19 } }
	self.wpn_fps_pis_hs2000.a_fl = 1
	...
	table.insert(self.wpn_fps_pis_hs2000.default_blueprint, "wpn_fps_upg_blankcal_45acp")
	table.insert(self.wpn_fps_pis_hs2000.uses_parts, "wpn_fps_upg_blankcal_45acp")
	table.insert(self.wpn_fps_pis_hs2000.uses_parts, "wpn_fps_upg_a_subfmj")
	...
	self.parts.wpn_fps_pis_legacy_b_standard.value = 1
	self.parts.wpn_fps_pis_legacy_b_standard.stats = { concealment = 0, weight = 2, barrel_length = 4.1 }
	self.parts.wpn_fps_pis_legacy_b_threaded.value = 2
	self.parts.wpn_fps_pis_legacy_b_threaded.stats = { concealment = 0, weight = 2, barrel_length = 4.5 }
	...
	self.wpn_fps_pis_legacy.a_fl = 1
	...
	table.insert(self.wpn_fps_pis_legacy.uses_parts, "wpn_fps_upg_a_subfmj")

	self.parts.wpn_fps_pis_packrat_b_standard.stats = { concealment = 0, weight = 2, barrel_length = 4.6 }
	...
	self.parts.wpn_fps_pis_packrat_m_standard.stats = { concealment = 2, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = { ["9x19"] = 15, [".40 S&W"] = 13 } }
	self.parts.wpn_fps_pis_packrat_m_extended.stats = { concealment = 4, weight = 2, mag_amount = { 1, 2, 3 }, CLIP_AMMO_MAX = { ["9x19"] = 30, [".40 S&W"] = 28 } }
	...
	self.wpn_fps_pis_packrat.a_fl = 1
	table.insert(self.wpn_fps_pis_packrat.uses_parts, "wpn_fps_upg_a_subfmj")
	table.insert(self.wpn_fps_pis_packrat.uses_parts, "wpn_fps_upg_cal_40sw")

	self.parts.wpn_fps_pis_pl14_b_comp.type = "barrel_ext"
	self.parts.wpn_fps_pis_pl14_b_comp.stats = { concealment = 0, weight = 2, barrel_length = 4.4, length = 2, md_code = {0,0,0,2,0} }
	self.parts.wpn_fps_pis_pl14_b_standard.stats = { concealment = 0, weight = 2, barrel_length = 4.4 }
	self.parts.wpn_fps_pis_pl14_body_standard.stats = { concealment = 0, weight = 0, length = 3 }
	self.parts.wpn_fps_pis_pl14_m_extended.stats = { concealment = 3, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = 18 } --roughly
	self.parts.wpn_fps_pis_pl14_m_standard.stats = { concealment = 2, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = 16 }
	...
	table.insert(self.wpn_fps_pis_pl14.uses_parts, "wpn_fps_upg_a_subfmj")

	self.parts.wpn_fps_pis_ppk_b_barrel_standard.value = 1
	self.parts.wpn_fps_pis_ppk_b_barrel_standard.forbids = { "wpn_fps_pis_ppk_b_long" }
	self.parts.wpn_fps_pis_ppk_b_barrel_standard.stats = { concealment = 0, weight = 2, barrel_length = 3.3 }
	...
	self.parts.wpn_fps_pis_ppk_b_barrel_long.value = 1
	self.parts.wpn_fps_pis_ppk_b_barrel_long.stats = { concealment = 0, weight = 2, barrel_length = 3.9 }
	...
	self.parts.wpn_fps_pis_ppk_g_laser.stats = { concealment = 0, weight = 0, gadget_power = { laser = 1 } }
	self.parts.wpn_fps_pis_ppk_g_standard.stats = { concealment = 0, weight = 0 }
	self.parts.wpn_fps_pis_ppk_m_standard.stats = { concealment = 1, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = 7 }
	self.wpn_fps_pis_ppk.a_fl = 1
	...
	self.parts.wpn_fps_pis_shrew_b_barrel.stats = { concealment = 0, weight = 2, barrel_length = 3 }
	...
	self.parts.wpn_fps_pis_shrew_m_extended.stats = { concealment = 2, weight = 2, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = 10 } --roughly
	self.parts.wpn_fps_pis_shrew_m_standard.stats = { concealment = 1, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = 8 }
	...
	table.insert(self.wpn_fps_pis_shrew.uses_parts, "wpn_fps_upg_a_subfmj")
	...
	self.parts.wpn_fps_pis_sparrow_b_941.value = 1
	self.parts.wpn_fps_pis_sparrow_b_941.stats = { concealment = 0, weight = 2, barrel_length = 4.5 }
	--self.parts.wpn_fps_pis_sparrow_b_comp.type = "barrel_ext"
	self.parts.wpn_fps_pis_sparrow_b_comp.value = 1
	self.parts.wpn_fps_pis_sparrow_b_comp.forbids = {}
	table.addto(self.parts.wpn_fps_pis_sparrow_b_comp.forbids, self.nqr.all_bxs)
	self.parts.wpn_fps_pis_sparrow_b_comp.stats = { concealment = 0, weight = 2+1, barrel_length = 4.5, length = 1, md_code = {0,2,0,0,0} }
	...
	self.parts.wpn_fps_pis_sparrow_b_threaded.value = 1
	self.parts.wpn_fps_pis_sparrow_b_threaded.stats = { concealment = 0, weight = 2, barrel_length = 4.5 }
	...
	self.parts.wpn_fps_pis_sparrow_m_standard.stats = { concealment = 2, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = { ["9x19"] = 15, [".40 S&W"] = 12 } }
	...
	self.wpn_fps_pis_sparrow.a_fl = 1
	...
	table.insert(self.wpn_fps_pis_sparrow.uses_parts, "wpn_fps_upg_a_subfmj")
	...
	self.parts.wpn_fps_pis_1911_bb_standard.value = 1
	...
	self.parts.wpn_fps_pis_1911_bb_standard.stats = { concealment = 0, weight = 3, barrel_length = 5 }
	...
	self.parts.wpn_fps_pis_1911_bb_vented.pcs = {}
	self.parts.wpn_fps_pis_1911_bb_vented.value = 2
	self.parts.wpn_fps_pis_1911_bb_vented.stats = { concealment = 0, weight = 3, barrel_length = 5, md_code = {0,0,1,0,0} }
	...
	self.parts.wpn_fps_pis_1911_bb_long.value = 2
	...
	self.parts.wpn_fps_pis_1911_bb_long.stats = { concealment = 0, weight = 4, barrel_length = 7 }
	...
	self.parts.wpn_fps_pis_1911_bb_long_vented.value = 3
	self.parts.wpn_fps_pis_1911_bb_long_vented.stats = { concealment = 0, weight = 4, barrel_length = 7, md_code = {0,0,1,0,0} }
	self.parts.wpn_fps_pis_1911_b_standard.visibility = { { objects = { g_barrel = false } } }
	self.parts.wpn_fps_pis_1911_b_standard.stats = { concealment = 0, weight = 2 }
	self.parts.wpn_fps_pis_1911_b_vented.visibility = { { objects = { g_barrel = false } } }
	self.parts.wpn_fps_pis_1911_b_vented.stats = { concealment = 0, weight = 2 }
	...
	self.parts.wpn_fps_pis_1911_b_long.stats = { concealment = 0, weight = 3 } --roughly
	...
	self.parts.wpn_fps_pis_1911_fl_legendary.is_a_unlockable = true
	self.parts.wpn_fps_pis_1911_fl_legendary.pcs = {}
	self.parts.wpn_fps_pis_1911_fl_legendary.type = "sight"
	self.parts.wpn_fps_pis_1911_fl_legendary.stance_mod = nil
	self.parts.wpn_fps_pis_1911_fl_legendary.stats = { concealment = 0, weight = 6, gadget_power = { laser = 2 } }
	...
	self.parts.wpn_fps_pis_1911_g_legendary.is_a_unlockable = true
	self.parts.wpn_fps_pis_1911_g_legendary.pcs = {}
	self.parts.wpn_fps_pis_1911_g_legendary.stats = { concealment = 0, weight = 1 }
	self.parts.wpn_fps_pis_1911_m_standard.stats = { concealment = 2, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = { [".45 ACP"] = 8, [".40 S&W"] = 9, ["9x19"] = 10 } }
	self.parts.wpn_fps_pis_1911_m_extended.stats = { concealment = 3, weight = 2, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = { [".45 ACP"] = 12, [".40 S&W"] = 14, ["9x19"] = 16 } }
	self.parts.wpn_fps_pis_1911_m_big.stats = { concealment = 4, weight = 3, mag_amount = { 1, 2, 3 }, CLIP_AMMO_MAX = { [".45 ACP"] = 13, [".40 S&W"] = 15, ["9x19"] = 17 } }
	...
	self.parts.wpn_fps_pis_1911_hicapa = deep_clone(self.parts.wpn_fps_pis_1911_m_extended)
	self.parts.wpn_fps_pis_1911_hicapa.name_id = "bm_wp_1911_hicapa"
	self.parts.wpn_fps_pis_1911_hicapa.type = "exclusive_set"
	self.parts.wpn_fps_pis_1911_hicapa.forbids = { "wpn_fps_pis_1911_m_extended", "wpn_fps_pis_1911_m_big", "wpn_fps_pis_1911_g_bling", "wpn_fps_pis_1911_g_engraved", "wpn_fps_pis_1911_g_ergo" }
	self.parts.wpn_fps_pis_1911_hicapa.override = {
		wpn_fps_pis_1911_m_standard = {
			unit = self.parts.wpn_fps_pis_usp_m_standard.unit,
			third_unit = self.parts.wpn_fps_pis_usp_m_standard.third_unit,
			stats = { concealment = 3, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = { [".45 ACP"] = 13, [".40 S&W"] = 17, ["9x19"] = 20 } }
		},
		wpn_fps_pis_1911_body_standard = { stats = { concealment = 0, weight = 1, length = 3 } },
		wpn_fps_pis_1911_g_standard = { unit = self.parts.wpn_fps_pis_1911_g_legendary.unit, third_unit = self.parts.wpn_fps_pis_1911_g_legendary.third_unit },
	}
	self.parts.wpn_fps_pis_1911_hicapa.unit = self.parts.wpn_fps_pis_usp_m_standard.unit
	self.parts.wpn_fps_pis_1911_hicapa.third_unit = self.parts.wpn_fps_pis_usp_m_standard.third_unit
	self.parts.wpn_fps_pis_1911_hicapa.stats = { concealment = 0, weight = 0, value = 10 }
	self.wpn_fps_pis_1911.a_fl = 1
	...
	table.insert(self.wpn_fps_pis_1911.default_blueprint, "wpn_fps_upg_blankcal_45acp")
	table.insert(self.wpn_fps_pis_1911.uses_parts, "wpn_fps_upg_blankcal_45acp")
	table.insert(self.wpn_fps_pis_1911.uses_parts, "wpn_fps_upg_a_subfmj")
	...
	table.insert(self.wpn_fps_pis_1911.uses_parts, "wpn_fps_pis_1911_hicapa")
	...
	self.parts.wpn_fps_pis_m1911_b_standard.value = 1
	self.parts.wpn_fps_pis_m1911_b_standard.forbids = { "wpn_fps_pis_m1911_sl_hardballer" }
	self.parts.wpn_fps_pis_m1911_b_standard.stats = { concealment = 0, weight = 3, barrel_length = 5 }
	self.parts.wpn_fps_pis_m1911_b_hardballer.pcs = {}
	self.parts.wpn_fps_pis_m1911_b_hardballer.value = 2
	...
	self.parts.wpn_fps_pis_m1911_b_hardballer.stats = { concealment = 0, weight = 4, barrel_length = 7 }
	...
	self.parts.wpn_fps_pis_m1911_m_standard.stats = { concealment = 2, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = { [".45 ACP"] = 8, [".40 S&W"] = 9, ["9x19"] = 10 } }
	self.parts.wpn_fps_pis_m1911_m_extended.stats = { concealment = 3, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = { [".45 ACP"] = 10, [".40 S&W"] = 11, ["9x19"] = 13 } }
	self.wpn_fps_pis_m1911.a_fl = 1
	...
	table.insert(self.wpn_fps_pis_m1911.default_blueprint, "wpn_fps_upg_blankcal_45acp")
	table.insert(self.wpn_fps_pis_m1911.uses_parts, "wpn_fps_upg_blankcal_45acp")
	table.insert(self.wpn_fps_pis_m1911.uses_parts, "wpn_fps_upg_a_subfmj")
	...
	self.parts.wpn_fps_pis_usp_bb_tactical.value = 1
	...
	self.parts.wpn_fps_pis_usp_bb_tactical.stats = { concealment = 0, weight = 2, barrel_length = 4.4 }
	...
	self.parts.wpn_fps_pis_usp_bb_expert.value = 3
	...
	self.parts.wpn_fps_pis_usp_bb_expert.stats = { concealment = 0, weight = 3, barrel_length = 5.1 }
	...
	self.parts.wpn_fps_pis_usp_bb_match.value = 4
	...
	self.parts.wpn_fps_pis_usp_bb_match.stats = { concealment = 0, weight = 3, barrel_length = 6 }
	...
	self.parts.wpn_fps_pis_usp_m_big.stats = { concealment = 6, weight = 3, mag_amount = { 1, 2, 3 }, CLIP_AMMO_MAX = 22 }
	self.parts.wpn_fps_pis_usp_m_extended.stats = { concealment = 5, weight = 2, mag_amount = { 1, 2, 3 }, CLIP_AMMO_MAX = { [".45 ACP"] = 20, [".40 S&W"] = 25, ["9x19"] = 30 } }
	self.parts.wpn_fps_pis_usp_m_standard.pcs = {}
	self.parts.wpn_fps_pis_usp_m_standard.stats = { concealment = 3, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = { [".45 ACP"] = 13, [".40 S&W"] = 16, ["9x19"] = 19 } }
	self.wpn_fps_pis_usp.a_fl = 1
	...
	table.insert(self.wpn_fps_pis_usp.default_blueprint, "wpn_fps_upg_blankcal_45acp")
	table.insert(self.wpn_fps_pis_usp.uses_parts, "wpn_fps_upg_blankcal_45acp")
	table.insert(self.wpn_fps_pis_usp.uses_parts, "wpn_fps_upg_a_subfmj")
	...
	self.parts.wpn_fps_pis_breech_b_short.value = 1
	self.parts.wpn_fps_pis_breech_b_short.stats = { concealment = 0, weight = 1, barrel_length = 2.5 }
	self.parts.wpn_fps_pis_breech_b_standard.value = 1
	self.parts.wpn_fps_pis_breech_b_standard.stats = { concealment = 0, weight = 2, barrel_length = 3.9 }
	self.parts.wpn_fps_pis_breech_b_reinforced.value = 2
	self.parts.wpn_fps_pis_breech_b_reinforced.stats = { concealment = 0, weight = 3, barrel_length = 4.7 }
	...
	table.insert(self.wpn_fps_pis_breech.uses_parts, "wpn_fps_upg_a_subfmj")

	self.parts.wpn_fps_pis_c96_b_standard.type = "barrel"
	self.parts.wpn_fps_pis_c96_b_standard.value = 1
	...
	self.parts.wpn_fps_pis_c96_b_long.value = 1
	...
	self.parts.wpn_fps_pis_c96_b_long.stats = { concealment = 0, weight = 8, barrel_length = 15 }
	...
	self.parts.wpn_fps_pis_c96_m_standard.stats = { concealment = 1, weight = 0, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = 10 }
	...
	self.parts.wpn_fps_pis_c96_s_solid.stats = { concealment = 0, weight = 0 }
	...
	table.deletefrom(self.wpn_fps_pis_c96.uses_parts, self.nqr.all_gadgets)
	...
	table.insert(self.wpn_fps_pis_c96.default_blueprint, 1, "wpn_fps_extra2_lock_gadgets")
	table.insert(self.wpn_fps_pis_c96.uses_parts, "wpn_fps_extra2_lock_gadgets")
	table.insert(self.wpn_fps_pis_c96.uses_parts, "wpn_fps_upg_a_subfmj")

	self.parts.wpn_fps_pis_type54_b_standard.stats = { concealment = 0, weight = 2, barrel_length = 4.6 }
	...
	self.parts.wpn_fps_pis_type54_m_ext.stats = { concealment = 3, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = 11 }
	self.parts.wpn_fps_pis_type54_m_standard.stats = { concealment = 2, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = 8 }
	...
	self.wpn_fps_pis_type54.a_fl = 1
	...
	self.parts.wpn_fps_pis_lemming_b_standard.value = 1
	self.parts.wpn_fps_pis_lemming_b_standard.stats = { concealment = 0, weight = 2, barrel_length = 4.8 }
	self.parts.wpn_fps_pis_lemming_b_nitride.value = 1
	self.parts.wpn_fps_pis_lemming_b_nitride.stats = { concealment = 0, weight = 2, barrel_length = 4.8 }
	self.parts.wpn_fps_pis_lemming_body.stats = { concealment = 0, weight = 0, length = 3 }
	self.parts.wpn_fps_pis_lemming_m_ext.stats = { concealment = 4, weight = 2, mag_amount = { 1, 2, 3 }, CLIP_AMMO_MAX = 30 }
	self.parts.wpn_fps_pis_lemming_m_standard.stats = { concealment = 3, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = 20 }
	...
	self.wpn_fps_pis_lemming.a_fl = 1

	local deagle_short_sps = {
		"wpn_fps_upg_ns_pis_small",
		"wpn_fps_smg_baka_b_smallsupp",
		"wpn_fps_upg_ns_pis_medium",
	...
	self.parts.wpn_fps_pis_deagle_b_standard.value = 1
	self.parts.wpn_fps_pis_deagle_b_standard.stats = { concealment = 0, weight = 7, barrel_length = 6 }
	self.parts.wpn_fps_pis_deagle_b_modern.type = "barrel"
	self.parts.wpn_fps_pis_deagle_b_modern.value = 9
	self.parts.wpn_fps_pis_deagle_b_modern.forbids = { "wpn_fps_pis_deagle_co_long", "wpn_fps_pis_deagle_co_short" }
	table.addto(self.parts.wpn_fps_pis_deagle_b_modern.forbids, self.nqr.all_bxs)
	self.parts.wpn_fps_pis_deagle_b_modern.stats = { concealment = 0, weight = 5, md_code = {0,0,0,2,0}, barrel_length = 5.5, length = 0.5 } --roughly
	self.parts.wpn_fps_pis_deagle_b_long.type = "barrel"
	self.parts.wpn_fps_pis_deagle_b_long.value = 12
	--self.parts.wpn_fps_pis_deagle_b_long.forbids = {}
	--table.addto(self.parts.wpn_fps_pis_deagle_b_long.forbids, self.nqr.all_sps)
	table.addto(self.parts.wpn_fps_pis_deagle_b_long.forbids, table.without(self.nqr.all_sps, deagle_short_sps))
	self.parts.wpn_fps_pis_deagle_b_legend.is_a_unlockable = true
	self.parts.wpn_fps_pis_deagle_b_legend.name_id = "bm_wp_deagle_b_legend"
	self.parts.wpn_fps_pis_deagle_b_legend.sort_number = 15000
	self.parts.wpn_fps_pis_deagle_b_legend.pcs = {}
	self.parts.wpn_fps_pis_deagle_b_legend.type = "barrel_ext"
	self.parts.wpn_fps_pis_deagle_b_legend.forbids = {}
	table.addto(self.parts.wpn_fps_pis_deagle_b_legend.forbids, {"wpn_fps_pis_deagle_b_modern", "wpn_fps_pis_deagle_b_long"})
	self.parts.wpn_fps_pis_deagle_b_legend.override = {}
	self.parts.wpn_fps_pis_deagle_b_legend.visibility = { { objects = { g_barrel = false } } }
	self.parts.wpn_fps_pis_deagle_b_legend.stats = { concealment = 0, weight = 9, length = 5, md_code = {0,0,3,0,0} }
	...
	self.parts.wpn_fps_pis_deagle_extra.override = { wpn_fps_gadgets_pos_a_o = { override = deep_clone(self.parts.wpn_fps_gadgets_pos_a_o.override) } }
	for i, k in pairs(table.with(self.nqr.all_reddots, self.nqr.all_piggyback_sights)) do
		self.parts.wpn_fps_pis_deagle_extra.override[k] = { a_obj = "a_quite" }
	end
	for i, k in pairs(self.nqr.all_gadgets) do
		self.parts.wpn_fps_pis_deagle_extra.override.wpn_fps_gadgets_pos_a_o.override[k] = table.copy_append(
			self.parts.wpn_fps_gadgets_pos_a_o.override[k], { a_obj = "a_quite", a_fl = (self.parts[k].a_fl or 0)+2, }
		)
	end
	...
	self.parts.wpn_fps_pis_deagle_fg_rail.forbids = { "wpn_fps_pis_deagle_b_legend" }
	...
	self.parts.wpn_fps_pis_deagle_m_standard.stats = { concealment = 3, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = { [".50 AE"] = 7, [".44 Mag"] = 8, [".357 Mag"] = 9 } }
	self.parts.wpn_fps_pis_deagle_m_extended.stats = { concealment = 4, weight = 2, mag_amount = { 1, 2, 3 }, CLIP_AMMO_MAX = { [".50 AE"] = 10, [".44 Mag"] = 11, [".357 Mag"] = 12 } }
	...
	self.wpn_fps_pis_deagle.a_fl = 1
	...
	self.wpn_fps_pis_deagle.override = {
		...
		wpn_fps_gadgets_pos_a_o = { override = deep_clone(self.parts.wpn_fps_gadgets_pos_a_o.override) },
	}
	...
	for i, k in pairs(self.nqr.all_gadgets) do
		self.wpn_fps_pis_deagle.override.wpn_fps_gadgets_pos_a_o.override[k] = {
			a_obj = "a_os", a_fl = (self.parts[k].a_fl or 0)+2,
		}
	end
	...
	table.delete(self.wpn_fps_pis_deagle.uses_parts, "wpn_fps_ass_tecci_ns_special")
	...
	table.insert(self.wpn_fps_pis_deagle.default_blueprint, "wpn_fps_gadgets_pos_pistols_blank")
	table.insert(self.wpn_fps_pis_deagle.uses_parts, "wpn_fps_gadgets_pos_pistols_blank")
	table.insert(self.wpn_fps_pis_deagle.uses_parts, "wpn_fps_gadgets_pos_a_o")
	...
	table.addto(self.wpn_fps_pis_deagle.uses_parts, self.nqr.all_gadgets)
	...
	self.parts.wpn_fps_pis_stech_b_standard.stats = { concealment = 0, weight = 3, barrel_length = 5.7 }
	...
	self.parts.wpn_fps_pis_stech_m_extended.stats = { concealment = 4, weight = 2, mag_amount = { 1, 2, 3 }, CLIP_AMMO_MAX = 40, retention = false } --roughly
	self.parts.wpn_fps_pis_stech_m_standard.stats = { concealment = 2, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = 20, retention = false }
	...
	self.wpn_fps_pis_stech.a_fl = 1
	table.delete(self.wpn_fps_pis_stech.uses_parts, "wpn_fps_pis_stech_s_standard")

	self.parts.wpn_fps_pis_pmm_lower_standard.stats = { concealment = 0, weight = 0, length = 3 }
	self.parts.wpn_fps_pis_pmm_barrel_dummy = deep_clone(self.parts.wpn_fps_pis_pmm_slide_standard)
	self.parts.wpn_fps_pis_pmm_barrel_dummy.type = "barrel"
	self.parts.wpn_fps_pis_pmm_barrel_dummy.visibility = { { objects = { g_slide = false } } }
	self.parts.wpn_fps_pis_pmm_barrel_dummy.stats = {}
	self.parts.wpn_fps_pis_pmm_slide_standard.adds = { "wpn_fps_pis_pmm_barrel_dummy" }
	self.parts.wpn_fps_pis_pmm_slide_standard.forbids = {}
	self.parts.wpn_fps_pis_pmm_slide_standard.stats = { concealment = 0, weight = 0, barrel_length = 3.7 }
	self.parts.wpn_fps_pis_pmm_slide_suppressed.type = "exclusive_set"
	self.parts.wpn_fps_pis_pmm_slide_suppressed.sub_type = "ironsight"
	self.parts.wpn_fps_pis_pmm_slide_suppressed.has_description = true
	self.parts.wpn_fps_pis_pmm_slide_suppressed.desc_id = "bm_wp_bleedoff"
	self.parts.wpn_fps_pis_pmm_slide_suppressed.forbids = {}
	table.addto(self.parts.wpn_fps_pis_pmm_slide_suppressed.forbids, self.nqr.all_bxs)
	self.parts.wpn_fps_pis_pmm_slide_suppressed.stance_mod = nil
	self.parts.wpn_fps_pis_pmm_slide_suppressed.custom_stats = nil
	self.parts.wpn_fps_pis_pmm_slide_suppressed.override = {
		wpn_fps_pis_pmm_slide_standard = {
			unit = self.parts.wpn_fps_pis_pmm_slide_suppressed.unit,
			third_unit = self.parts.wpn_fps_pis_pmm_slide_suppressed.unit,
			stats = {},
		},
	}
	self.parts.wpn_fps_pis_pmm_slide_suppressed.unit = fantom_unit
	self.parts.wpn_fps_pis_pmm_slide_suppressed.third_unit = fantom_unit
	self.parts.wpn_fps_pis_pmm_slide_suppressed.stats = { concealment = 0, weight = 2, barrel_length = 3.9, bleedoff = 0.9, sightheight = 0.5, value = 3 }
	self.parts.wpn_fps_pis_pmm_ns_suppressor.a_obj = "a_ns_alt"
	self.parts.wpn_fps_pis_pmm_ns_suppressor.depends_on = "exclusive_set"
	self.parts.wpn_fps_pis_pmm_ns_suppressor.value = 2
	self.parts.wpn_fps_pis_pmm_ns_suppressor.stats = { concealment = 8, weight = 3, length = 5, md_code = {4,0,0,0,0} }
	self.parts.wpn_fps_pis_pmm_g_standard.stats = { concealment = 0, weight = 0 }
	self.parts.wpn_fps_pis_pmm_g_black.stats = { concealment = 0, weight = 0 }
	self.parts.wpn_fps_pis_pmm_m_standard.stats = { concealment = 1, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = 8 }
	self.parts.wpn_fps_pis_pmm_flashlight_mount.pcs = {}
	self.parts.wpn_fps_pis_pmm_flashlight_mount.type = "extra3"
	self.parts.wpn_fps_pis_pmm_flashlight_mount.rails = { "bottom" }
	self.parts.wpn_fps_pis_pmm_flashlight_mount.stats = { concealment = 2, weight = 1 }
	self.wpn_fps_pis_pmm.a_fl = 1
	self.wpn_fps_pis_pmm.adds = {}
	self.wpn_fps_pis_pmm.override = {}
	for i, k in pairs(self.nqr.all_pistol_reddots) do self.wpn_fps_pis_pmm.override[k] = { parent = "slide", a_obj = "a_o_optics" } end
	for i, k in pairs(self.nqr.all_gadgets) do self.wpn_fps_pis_pmm.override[k] = { parent = "lower_body" } end
	table.insert(self.wpn_fps_pis_pmm.default_blueprint, 1, "wpn_fps_extra3_lock_gadgets")
	table.insert(self.wpn_fps_pis_pmm.uses_parts, "wpn_fps_extra3_lock_gadgets")
	table.insert(self.wpn_fps_pis_pmm.uses_parts, "wpn_fps_pis_pmm_flashlight_mount")

	self.parts.wpn_fps_pis_czech_b_standard.adds = {}
	self.parts.wpn_fps_pis_czech_b_standard.forbids = {}
	self.parts.wpn_fps_pis_czech_b_standard.stats = { concealment = 0, weight = 2, barrel_length = 4.7 }
	...
	self.parts.wpn_fps_pis_czech_m_standard.stats = { concealment = 3, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = 19 }
	self.parts.wpn_fps_pis_czech_m_extended.stats = { concealment = 4, weight = 2, mag_amount = { 1, 2, 3 }, CLIP_AMMO_MAX = 25 }
	...
	table.insert(self.wpn_fps_pis_czech.uses_parts, "wpn_fps_upg_a_subfmj")

	self.parts.wpn_fps_snp_contender_barrel_short.value = 6
	self.parts.wpn_fps_snp_contender_barrel_short.stats = { concealment = 0, weight = 5, barrel_length = 10 }
	self.parts.wpn_fps_snp_contender_barrel_standard.value = 1
	...
	self.parts.wpn_fps_snp_contender_barrel_standard.stats = { concealment = 0, weight = 7, barrel_length = 14 }
	self.parts.wpn_fps_snp_contender_barrel_long.value = 9
	...
	self.parts.wpn_fps_snp_contender_barrel_long.stats = { concealment = 0, weight = 9, barrel_length = 17 }
	...
	for i, k in pairs(table.with(self.nqr.all_reddots, self.nqr.all_gadgets)) do self.wpn_fps_snp_contender.override[k] = { parent = "barrel" } end
	...
	table.insert(self.wpn_fps_snp_contender.default_blueprint, "wpn_fps_gadgets_pos_a_o")
	table.insert(self.wpn_fps_snp_contender.uses_parts, "wpn_fps_gadgets_pos_a_o")
	table.insert(self.wpn_fps_snp_contender.uses_parts, "wpn_fps_upg_o_atibal_reddot")
	table.addto(self.wpn_fps_snp_contender.uses_parts, self.nqr.all_gadgets)
	...
	self.parts.wpn_fps_pis_welrod_body_standard.stats = { concealment = 0, weight = 2, barrel_length = 3.25, length = 3 }
	self.parts.wpn_fps_pis_welrod_b_standard.override = {}
	self.parts.wpn_fps_pis_welrod_b_standard.stats = { concealment = 0, weight = 0, length = 6, md_code = {4,0,0,0,0} }
	self.parts.wpn_fps_pis_welrod_m_standard.stats = { concealment = 2, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = { [".32 ACP"] = 8, ["9x19"] = 6 } }
	self.parts.wpn_fps_pis_welrod_g_standard.stats = { concealment = 0, weight = 0 }
	self.parts.wpn_fps_pis_welrod_o_standard_rear.type = "ironsight"
	self.parts.wpn_fps_pis_welrod_o_standard_rear.stats = { concealment = 0, weight = 0 }
	self.parts.wpn_fps_pis_welrod_o_standard_front.type = "ironsight"
	self.parts.wpn_fps_pis_welrod_gadgetrail = deep_clone(self.parts.wpn_fps_pis_legacy_fl_mount)
	self.parts.wpn_fps_pis_welrod_gadgetrail.a_obj = "a_fl_2"
	self.wpn_fps_pis_welrod.a_fl = 1
	self.wpn_fps_pis_welrod.adds = {}
	self.wpn_fps_pis_welrod.override = {}
	for i, k in pairs(self.nqr.all_gadgets) do self.wpn_fps_pis_welrod.override[k] = { a_obj = "a_fl_2" } end
	for i, k in pairs(self.nqr.all_pistol_reddots) do self.wpn_fps_pis_welrod.override[k] = { parent = "lower_receiver" } end
	table.delete(self.wpn_fps_pis_welrod.uses_parts, "wpn_upg_o_marksmansight_rear")
	table.delete(self.wpn_fps_pis_welrod.uses_parts, "wpn_fps_upg_ns_pis_ipsccomp")
	table.delete(self.wpn_fps_pis_welrod.uses_parts, "wpn_fps_upg_ns_pis_meatgrinder")
	table.deletefrom(self.wpn_fps_pis_welrod.uses_parts, self.nqr.all_bxs)
	table.insert(self.wpn_fps_pis_welrod.default_blueprint, 1, "wpn_fps_extra3_lock_gadgets")
	table.insert(self.wpn_fps_pis_welrod.uses_parts, "wpn_fps_extra3_lock_gadgets")
	table.insert(self.wpn_fps_pis_welrod.uses_parts, "wpn_fps_pis_welrod_gadgetrail")



----REVOLVER
	local default_revolver_magdrop = {
		parent = "g_bullets", effect = "effects/payday2/particles/weapons/shells/shell_revolver_dump",
	}
	self.parts.wpn_fps_pis_judge_b_standard.type = "barrel"
	self.parts.wpn_fps_pis_judge_b_standard.stats = { concealment = 0, weight = 1, barrel_length = 2.5 }
	self.parts.wpn_fps_pis_judge_b_legend.is_a_unlockable = true
	self.parts.wpn_fps_pis_judge_b_legend.pcs = {}
	self.parts.wpn_fps_pis_judge_b_legend.type = "barrel"
	self.parts.wpn_fps_pis_judge_b_legend.forbids = { "wpn_fps_pis_judge_fl_adapter" }
	self.parts.wpn_fps_pis_judge_b_legend.stats = { concealment = 0, weight = 3+3+5, barrel_length = 6, md_code = {0,0,1,0,0}, gadget_power = { laser = 1 } }
	...
	self.parts.wpn_fps_pis_judge_g_legend.is_a_unlockable = true
	self.parts.wpn_fps_pis_judge_g_legend.pcs = {}
	...
	self.parts.wpn_fps_pis_judge_body_standard.stats = { concealment = 4, weight = 1, length = 6, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = 5 }
	...
	self.parts.wpn_fps_pis_judge_body_modern.forbids = { "wpn_fps_pis_judge_fl_adapter", "wpn_fps_pis_judge_b_legend" }
	...
	self.parts.wpn_fps_pis_judge_body_modern.stats = { concealment = 0, weight = 3, value = 2 } --todo mag
	...
	table.insert(self.wpn_fps_pis_judge.uses_parts, "wpn_fps_upg_cal_45lc")
	...
	self.parts.wpn_fps_pis_chinchilla_b_standard.stats = { concealment = 0, weight = 3, barrel_length = 6 }
	self.parts.wpn_fps_pis_chinchilla_b_satan.sub_type = "ironsight"
	self.parts.wpn_fps_pis_chinchilla_b_satan.value = 4
	self.parts.wpn_fps_pis_chinchilla_b_satan.stats = { concealment = 0, weight = 4+3, barrel_length = 8.4, sightheight = 0.58 }
	...
	self.parts.wpn_fps_pis_chinchilla_m_bullets.magdrop_effect = deep_clone(default_revolver_magdrop)
	self.parts.wpn_fps_pis_chinchilla_m_bullets.magdrop_effect.parent = "g_cartridge_1"
	self.parts.wpn_fps_pis_chinchilla_m_bullets.visibility = { { objects = { g_speedloader = false } } }
	self.parts.wpn_fps_pis_chinchilla_m_bullets.stats = { concealment = 3, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = 6 }
	...
	self.parts.wpn_fps_pis_korth_b_railed.stats = { concealment = 0, weight = 4, barrel_length = 6 }
	...
	self.parts.wpn_fps_pis_korth_b_standard.stats = { concealment = 0, weight = 3, barrel_length = 6 }
	...
	self.parts.wpn_fps_pis_korth_m_6.forbids = { "wpn_fps_upg_cal_44mag" }
	self.parts.wpn_fps_pis_korth_m_6.override = { wpn_fps_pis_korth_m_body = { unit = self.parts.wpn_fps_pis_korth_m_8.unit } }
	self.parts.wpn_fps_pis_korth_m_6.magdrop_effect = deep_clone(default_revolver_magdrop)
	self.parts.wpn_fps_pis_korth_m_6.magdrop_effect.parent = "g_rod"
	self.parts.wpn_fps_pis_korth_m_6.visibility = { { objects = { g_rod = false, g_cylinder = false, g_lock = false, g_speedloader = false } } }
	self.parts.wpn_fps_pis_korth_m_6.stats = { concealment = 3, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = 8 }
	...
	self.parts.wpn_fps_pis_korth_m_8.magdrop_effect = deep_clone(default_revolver_magdrop)
	self.parts.wpn_fps_pis_korth_m_8.magdrop_effect.parent = "g_rod"
	self.parts.wpn_fps_pis_korth_m_8.visibility = { { objects = { g_rod = false, g_cylinder = false, g_lock = false, g_speedloader = false } } }
	self.parts.wpn_fps_pis_korth_m_8.stats = { concealment = 3, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = 6 }
	self.wpn_fps_pis_korth.a_fl = 1
	...
	self.parts.wpn_fps_pis_2006m_b_short.value = 3
	self.parts.wpn_fps_pis_2006m_b_short.forbids = { "wpn_fps_pis_2006m_fl_adapter" }
	self.parts.wpn_fps_pis_2006m_b_short.stats = { concealment = 0, weight = 1, barrel_length = 2 }
	self.parts.wpn_fps_pis_2006m_b_medium.value = 4
	self.parts.wpn_fps_pis_2006m_b_medium.stats = { concealment = 0, weight = 2, barrel_length = 4 }
	self.parts.wpn_fps_pis_2006m_b_standard.value = 1
	self.parts.wpn_fps_pis_2006m_b_standard.stats = { concealment = 0, weight = 3, barrel_length = 6 }
	self.parts.wpn_fps_pis_2006m_b_long.value = 6
	self.parts.wpn_fps_pis_2006m_b_long.stats = { concealment = 0, weight = 5, barrel_length = 7 } --roughly
	...
	self.parts.wpn_fps_pis_2006m_m_standard.magdrop_effect = deep_clone(default_revolver_magdrop)
	...
	self.parts.wpn_fps_pis_2006m_m_standard.stats = { concealment = 3, weight = 1, mag_amount = { 2, 4, 6 }, CLIP_AMMO_MAX = 6 }
	self.wpn_fps_pis_2006m.a_fl = 1
	...
	self.parts.wpn_fps_pis_rage_b_short.value = 2
	self.parts.wpn_fps_pis_rage_b_short.forbids = { "wpn_fps_pis_rage_extra" }
	self.parts.wpn_fps_pis_rage_b_short.stats = { concealment = 0, weight = 2, barrel_length = 4, md_code = {0,0,1,0,0} }
	self.parts.wpn_fps_pis_rage_b_standard.type = "barrel"
	self.parts.wpn_fps_pis_rage_b_standard.value = 1
	self.parts.wpn_fps_pis_rage_b_standard.stats = { concealment = 0, weight = 3, barrel_length = 6, md_code = {0,0,1,0,0} }
	self.parts.wpn_fps_pis_rage_b_comp1.type = "barrel"
	self.parts.wpn_fps_pis_rage_b_comp1.value = 4
	self.parts.wpn_fps_pis_rage_b_comp1.stats = { concealment = 0, weight = 3, barrel_length = 6, md_code = {0,0,2,0,0}, md_bulk = {2,1} }
	self.parts.wpn_fps_pis_rage_b_comp2.type = "barrel"
	self.parts.wpn_fps_pis_rage_b_comp2.value = 3
	self.parts.wpn_fps_pis_rage_b_comp2.stats = { concealment = 0, weight = 3, barrel_length = 5, md_code = {0,0,0,2,0} }
	self.parts.wpn_fps_pis_rage_b_long.type = "barrel"
	self.parts.wpn_fps_pis_rage_b_long.value = 6
	self.parts.wpn_fps_pis_rage_b_long.stats = { concealment = 0, weight = 5, barrel_length = 10, md_code = {0,0,1,0,0} }
	...
	self.parts.wpn_fps_pis_rage_body_standard.magdrop_effect = deep_clone(default_revolver_magdrop)
	...
	self.parts.wpn_fps_pis_rage_body_smooth.magdrop_effect = deep_clone(default_revolver_magdrop)
	...
	self.parts.wpn_fps_pis_rage_extra.override = { wpn_fps_extra_lock_gadgets = { forbids = {} } }
	...
	table.insert(self.wpn_fps_pis_rage.default_blueprint, 1, "wpn_fps_extra_lock_gadgets")
	table.insert(self.wpn_fps_pis_rage.uses_parts, "wpn_fps_extra_lock_gadgets")
	table.insert(self.wpn_fps_pis_rage.default_blueprint, "wpn_fps_gadgets_pos_a_o")
	table.insert(self.wpn_fps_pis_rage.uses_parts, "wpn_fps_gadgets_pos_a_o")
	...
	table.addto(self.wpn_fps_pis_rage.uses_parts, self.nqr.all_gadgets)
	...
	self.parts.wpn_fps_pis_rsh12_b_short.value = 7
	self.parts.wpn_fps_pis_rsh12_b_short.forbids = { "wpn_fps_pis_rsh12_b_comp" }
	self.parts.wpn_fps_pis_rsh12_b_short.stats = { concealment = 0, weight = 4, barrel_length = 5 }
	...
	self.parts.wpn_fps_pis_rsh12_b_standard.stats = { concealment = 0, weight = 6, barrel_length = 7.5 }
	self.parts.wpn_fps_pis_rsh12_b_comp.type = "barrel_ext"
	self.parts.wpn_fps_pis_rsh12_b_comp.forbids = {}
	self.parts.wpn_fps_pis_rsh12_b_comp.stats = { concealment = 4, weight = 2, md_code = {0,0,0,3,0} }
	...
	self.wpn_fps_pis_rsh12.a_fl = 1
	self.wpn_fps_pis_rsh12.override = { wpn_fps_gadgets_pos_a_o = { override = deep_clone(self.parts.wpn_fps_gadgets_pos_a_o.override) }, }
	for i, k in pairs(self.nqr.all_gadgets) do
		self.wpn_fps_pis_rsh12.override.wpn_fps_gadgets_pos_a_o.override[k] = {
			a_obj = "a_o", a_fl = (self.parts[k].a_fl or 0)+2, forbids = deep_clone(self.nqr.all_reddots),
		}
	end
	table.deletefrom(self.wpn_fps_pis_rsh12.uses_parts, self.nqr.all_optics)
	table.insert(self.wpn_fps_pis_rsh12.uses_parts, "wpn_fps_gadgets_pos_a_o")
	table.addto(self.wpn_fps_pis_rsh12.uses_parts, self.nqr.all_bxs_bbp)
	table.addto(self.wpn_fps_pis_rsh12.uses_parts, self.nqr.all_gadgets)

	self.parts.wpn_fps_pis_model3_b_short.value = 1
	self.parts.wpn_fps_pis_model3_b_short.stats = { concealment = 0, weight = 2, barrel_length = 4 } --roughly
	self.parts.wpn_fps_pis_model3_b_standard.value = 1
	self.parts.wpn_fps_pis_model3_b_standard.stats = { concealment = 0, weight = 3, barrel_length = 6.5 }
	self.parts.wpn_fps_pis_model3_b_long.value = 2
	self.parts.wpn_fps_pis_model3_b_long.stats = { concealment = 0, weight = 5, barrel_length = 9 } --roughly
	...
	self.parts.wpn_fps_pis_model3_m_bullets.magdrop_effect = deep_clone(default_revolver_magdrop)
	self.parts.wpn_fps_pis_model3_m_bullets.magdrop_effect.parent = "g_bullet_01"
	...
	self.parts.wpn_fps_pis_peacemaker_b_short.value = 1
	self.parts.wpn_fps_pis_peacemaker_b_short.stats = { concealment = 0, weight = 2, barrel_length = 4.6 }
	self.parts.wpn_fps_pis_peacemaker_b_standard.type = "barrel"
	self.parts.wpn_fps_pis_peacemaker_b_standard.value = 1
	self.parts.wpn_fps_pis_peacemaker_b_standard.stats = { concealment = 0, weight = 4, barrel_length = 7 } --roughly
	self.parts.wpn_fps_pis_peacemaker_b_long.type = "barrel"
	self.parts.wpn_fps_pis_peacemaker_b_long.value = 2
	self.parts.wpn_fps_pis_peacemaker_b_long.stats = { concealment = 0, weight = 6, barrel_length = 12 }
...
----AKIMBO
	aki_factory_lookup = {
		wpn_fps_pis_g17 = "wpn_fps_pis_x_g17",
		wpn_fps_pis_g18c = "wpn_fps_pis_x_g18c",
		wpn_fps_pis_g22c = "wpn_fps_pis_x_g22c",
		wpn_fps_pis_g26 = "wpn_fps_jowi",
		wpn_fps_pis_1911 = "wpn_fps_x_1911",
		wpn_fps_pis_beer = "wpn_fps_pis_x_beer",
		wpn_fps_pis_beretta = "wpn_fps_x_b92fs",
		wpn_fps_pis_breech = "wpn_fps_pis_x_breech",
		wpn_fps_pis_c96 = "wpn_fps_pis_x_c96",
		wpn_fps_pis_czech = "wpn_fps_pis_x_czech",
		wpn_fps_pis_deagle = "wpn_fps_x_deagle",
		wpn_fps_pis_holt = "wpn_fps_pis_x_holt",
		wpn_fps_pis_hs2000 = "wpn_fps_pis_x_hs2000",
		wpn_fps_pis_legacy = "wpn_fps_pis_x_legacy",
		wpn_fps_pis_m1911 = "wpn_fps_pis_x_m1911",
		wpn_fps_pis_maxim9 = "wpn_fps_pis_x_maxim9",
		wpn_fps_pis_p226 = "wpn_fps_pis_x_p226",
		wpn_fps_pis_packrat = "wpn_fps_x_packrat",
		wpn_fps_pis_pl14 = "wpn_fps_pis_x_pl14",
		wpn_fps_pis_ppk = "wpn_fps_pis_x_ppk",
		wpn_fps_pis_shrew = "wpn_fps_pis_x_shrew",
		wpn_fps_pis_sparrow = "wpn_fps_pis_x_sparrow",
		wpn_fps_pis_stech = "wpn_fps_pis_x_stech",
		wpn_fps_pis_type54 = "wpn_fps_pis_x_type54",
		wpn_fps_pis_usp = "wpn_fps_pis_x_usp",
		wpn_fps_pis_pmm = "wpn_fps_pis_x_pmm",

		wpn_fps_pis_rage = "wpn_fps_pis_x_rage",
		wpn_fps_pis_korth = "wpn_fps_pis_x_korth",
		wpn_fps_pis_chinchilla = "wpn_fps_pis_x_chinchilla",
		wpn_fps_pis_model3 = "wpn_fps_pis_x_model3",
		wpn_fps_pis_mateba = "wpn_fps_pis_x_2006m",
		wpn_fps_pis_judge = "wpn_fps_pis_x_judge",

		wpn_fps_smg_mac10 = "wpn_fps_smg_x_mac10",
		wpn_fps_smg_pm9 = "wpn_fps_smg_x_pm9",
		wpn_fps_smg_scorpion = "wpn_smg_pis_x_scorpion",
		wpn_fps_smg_baka = "wpn_fps_smg_x_baka",
		wpn_fps_smg_mp9 = "wpn_fps_smg_x_mp9",
		wpn_fps_smg_tec9 = "wpn_fps_smg_x_tec9",
		wpn_fps_smg_cobray = "wpn_fps_smg_x_cobray",
		wpn_fps_smg_sr2 = "wpn_fps_smg_x_sr2",

		wpn_fps_pis_rsh12 = "wpn_fps_pis_x_rsh12",
		wpn_fps_pis_lemming = "wpn_fps_pis_x_lemming",
		wpn_fps_smg_fmg9 = "wpn_fps_smg_x_fmg9",
	}
	for i, k in pairs(aki_factory_lookup) do
		if self[i] and self[k] then
			self[k].adds = deep_clone(self[i].adds or {})
			self[k].default_blueprint = deep_clone(self[i].default_blueprint or {})
			self[k].uses_parts = deep_clone(self[i].uses_parts or {})
			self[k].a_fl = self[i].a_fl

			local aki_anim_overrides = deep_clone(self[i].override or {})
			for u, j in pairs(self[k].override or {}) do
				if j.animations then
					if j.animations.reload then
						aki_anim_overrides[u] = aki_anim_overrides[u] or {}
						aki_anim_overrides[u].animations = j.animations
					else
						self[k].override[u].animations = nil
					end
				end
			end
			self[k].override = deep_clone(aki_anim_overrides)
		end
	end

	local aki_anim_override = {
		reload_left = "reload",
		fire = "recoil",
		reload_not_empty = "reload_not_empty",
		reload = "reload",
		reload_not_empty_left = "reload_not_empty",
		magazine_empty = "last_recoil",
	}

	table.swap(self.wpn_fps_smg_x_mac10.default_blueprint, "wpn_fps_smg_mac10_s_fold", "wpn_fps_remove_s")
	self.wpn_fps_smg_x_mac10.override.wpn_fps_smg_mac10_body_mac10 = { animations = aki_anim_override }
	self.wpn_fps_smg_x_mac10.override.wpn_fps_smg_mac10_body_ris = { animations = aki_anim_override }

	table.delete(self.wpn_fps_smg_x_scorpion.uses_parts, "wpn_fps_smg_scorpion_s_unfolded")
	self.wpn_fps_smg_x_scorpion.override.wpn_fps_smg_scorpion_body_standard = { animations = aki_anim_override }

	table.delete(self.wpn_fps_smg_x_baka.uses_parts, "wpn_fps_smg_baka_s_unfolded")
	self.wpn_fps_smg_x_baka.override.wpn_fps_smg_baka_body_standard = { animations = aki_anim_override }
	...
	self.wpn_fps_smg_x_mp9.override.wpn_fps_smg_mp9_body_mp9 = { animations = aki_anim_override }

	table.delete(self.wpn_fps_smg_x_tec9.uses_parts, "wpn_fps_smg_tec9_s_unfolded")
	self.wpn_fps_smg_x_tec9.override.wpn_fps_smg_tec9_b_long = { animations = aki_anim_override }
	self.wpn_fps_smg_x_tec9.override.wpn_fps_smg_tec9_b_standard = { animations = aki_anim_override }

	table.swap(self.wpn_fps_smg_x_cobray.default_blueprint, "wpn_fps_smg_cobray_s_standard", "wpn_fps_remove_s")
	self.wpn_fps_smg_x_cobray.animations = aki_anim_override
	...
	self.wpn_fps_smg_x_sr2.override.wpn_fps_smg_sr2_body_lower = { animations = aki_anim_override }

	self.wpn_fps_pis_x_g17.regression = "x_g17"
	self.wpn_fps_pis_x_g17.unit = self.wpn_fps_pis_x_g18c.unit

	self.wpn_fps_pis_x_g22c.regression = "x_g22c"
	self.wpn_fps_pis_x_g22c.unit = self.wpn_fps_pis_x_g18c.unit

	self.wpn_fps_pis_x_pmm.override.wpn_fps_pis_pmm_slide_standard = {
		animations = {
			reload = "reload_left",
			fire = "recoil",
			fire_steelsight = "recoil",
	...
	self.wpn_fps_pis_x_2006m.override.wpn_fps_pis_2006m_body_standard.animations = nil
	self.wpn_fps_pis_x_2006m.override.wpn_fps_pis_2006m_m_standard.animations = nil
	self.wpn_fps_pis_x_2006m.override.wpn_fps_pis_2006m_m_loader = { unit = fantom_unit }

	--self.wpn_fps_pis_x_judge.override = self.wpn_fps_pis_x_g18c.unit
...
----SPECIAL
	...
	self.parts.wpn_fps_lmg_m134_barrel_short.stats = { concealment = 0, weight = 21, barrel_length = 7 }
	self.parts.wpn_fps_lmg_m134_barrel.stats = { concealment = 0, weight = 66, barrel_length = 22 }
	self.parts.wpn_fps_lmg_m134_barrel_extreme.value = 8
	self.parts.wpn_fps_lmg_m134_barrel_extreme.stats = { concealment = 0, weight = 70, barrel_length = 22 }
	self.parts.wpn_fps_lmg_m134_barrel_legendary.is_a_unlockable = true
	self.parts.wpn_fps_lmg_m134_barrel_legendary.pcs = {}
	self.parts.wpn_fps_lmg_m134_barrel_legendary.custom_stats = { fire_rate_multiplier = 0.5 }
	self.parts.wpn_fps_lmg_m134_barrel_legendary.stats = { concealment = 0, weight = 46, barrel_length = 30 }
	...
	self.parts.wpn_fps_lmg_m134_body_upper_spikey.is_a_unlockable = true
	self.parts.wpn_fps_lmg_m134_body_upper_spikey.pcs = {}
	self.parts.wpn_fps_lmg_m134_body_upper_spikey.type = "wep_cos"
	self.parts.wpn_fps_lmg_m134_body_upper_spikey.visibility = { { objects = { g_handle = false } } }
	self.parts.wpn_fps_lmg_m134_body_upper_spikey.stats = { concealment = 5, weight = 5 }
	...
	self.parts.wpn_fps_gre_arbiter_b_standard.stats = { concealment = 0, weight = 15, barrel_length = 20 }
	self.parts.wpn_fps_gre_arbiter_b_long.forbids = { "wpn_fps_gre_arbiter_b_comp" }
	self.parts.wpn_fps_gre_arbiter_b_long.stats = { concealment = 0, weight = 18, barrel_length = 24 }
	self.parts.wpn_fps_gre_arbiter_b_comp.type = "barrel_ext"
	self.parts.wpn_fps_gre_arbiter_b_comp.stats = { concealment = 0, weight = 4, length = 3, md_code = {0,0,0,3,0} }
	self.parts.wpn_fps_gre_arbiter_body_standard.stats = { concealment = 0, weight = 0, length = 10 }
	...
	self.parts.wpn_fps_gre_china_b_standard.stats = { concealment = 0, weight = 7, barrel_length = 14.5 }
	...
	self.parts.wpn_fps_gre_china_body_standard.stats = { concealment = 0, weight = 0, length = 12 }
	...
	self.parts.wpn_fps_gre_china_s_short.stats = { concealment = 0, weight = 0 }
	self.parts.wpn_fps_gre_china_s_standard.stats = { concealment = 0, weight = 0, length = 8, shouldered = true }

	self.parts.wpn_fps_gre_m32_barrel_short.value = 18
	self.parts.wpn_fps_gre_m32_barrel_short.stats = { concealment = 0, weight = 4, barrel_length = 7.9 }
	self.parts.wpn_fps_gre_m32_barrel.stats = { concealment = 0, weight = 6, barrel_length = 11.8 }
	self.parts.wpn_fps_gre_m32_bolt.stats = { concealment = 0, weight = 0 }
	self.parts.wpn_fps_gre_m32_lower_reciever.stats = { concealment = 0, weight = 0, length = 8 }
	self.parts.wpn_fps_gre_m32_mag_body = deep_clone(self.parts.wpn_fps_gre_m32_mag)
	self.parts.wpn_fps_gre_m32_mag_body.type = "upper_reciever2"
	...
	self.parts.wpn_fps_gre_m32_stock_adapter.stats = { concealment = 0, weight = 2, length = 4 }
	self.parts.wpn_fps_gre_m32_upper_reciever.adds = { "wpn_fps_gre_m32_mag_body" }
	...
	self.parts.wpn_fps_gre_m79_barrel.stats = { concealment = 0, weight = 7, barrel_length = 14 }
	self.parts.wpn_fps_gre_m79_barrel_short.value = 14
	self.parts.wpn_fps_gre_m79_barrel_short.stats = { concealment = 0, weight = 4-1, barrel_length = 8 }
	...
	self.parts.wpn_fps_gre_m79_stock.stats = { concealment = 0, weight = 0, length = 14, shouldered = true }
	self.parts.wpn_fps_gre_m79_stock_short.stats = { concealment = 0, weight = 0, length = 5 }

	self.parts.wpn_fps_gre_ms3gl_b_standard.stats = { concealment = 0, weight = 8, barrel_length = 16 }
	self.parts.wpn_fps_gre_ms3gl_b_long.value = 20
	self.parts.wpn_fps_gre_ms3gl_b_long.stats = { concealment = 0, weight = 11, barrel_length = 22 }
	...
	self.parts.wpn_fps_gre_ms3gl_s_modern.stats = { concealment = 0, weight = 2, length = 6, shouldered = true }
	self.parts.wpn_fps_gre_ms3gl_s_standard.stats = { concealment = 0, weight = 2, length = 6, shouldered = true }
	self.parts.wpn_fps_gre_ms3gl_upper_standard.stats = { concealment = 0, weight = 0, length = 2 }
	...
	table.deletefrom(self.wpn_fps_gre_ms3gl.uses_parts, self.nqr.all_optics)
	...
	self.parts.wpn_fps_gre_slap_b_standard.stats = { concealment = 0, weight = 6, barrel_length = 11, mag_amount = { 6, 9, 12 } }
	self.parts.wpn_fps_gre_slap_body_lower.stats = { concealment = 0, weight = 0, length = 2 }
	...
	self.parts.wpn_fps_gre_slap_o_adapter.override = {
		wpn_fps_gre_slap_o_iron = { unit = fantom_unit, stats = {} },
		wpn_fps_gadgets_pos_a_o = { adds = {} },
	}
	...
	self.parts.wpn_fps_gre_slap_s_standard.stats = { concealment = 0, weight = 3, length = 10, shouldered = true, shoulderable = true }
	self.parts.wpn_fps_gre_slap_vg_standard.stats = { concealment = 0, weight = 1 }
	self.wpn_fps_gre_slap.adds = {}
	self.wpn_fps_gre_slap.override.wpn_fps_gre_m79_grenade.stats = table.copy_append(self.parts.wpn_fps_gre_m79_grenade.stats, {mag_amount = { 2, 4, 6 }})
	self.wpn_fps_gre_slap.override.wpn_fps_gadgets_pos_a_o = { adds = { "wpn_fps_extra_lock_gadgets" } }
	...
	table.insert(self.wpn_fps_gre_slap.uses_parts, "wpn_fps_gadgets_pos_a_o")
...
----
	...
	for i, k in pairs(self.parts) do
		if k.stats and k.stats.gadget_power then
			self.parts[i].stats.value = -2
			for u, j in pairs(self.parts[i].stats.gadget_power or {}) do
				self.parts[i].stats.value = self.parts[i].value or (self.parts[i].stats.value + 2 + (j+(u=="flashlight" and 3 or 0))*2)
			end
			self.parts[i].stats.value = math.max(0, self.parts[i].stats.value) --failsafe

			self.parts[i].sort_number = self.parts[i].sort_number or (
				sort_number + (self.parts[i].stats.value + (self.parts[i].stats.gadget_power.laser and 10 or 1)+ (self.parts[i].stats.gadget_power.flashlight and 20 or 1))*1000
			)
		end

		if k.stats and k.stats.barrel_length then
			k.stats.value = k.value or math.round(((k.stats.barrel_length*0.4 + k.stats.weight*1.5)-2) * (k.value_mod or 1))
			k.sort_number = (k.stats.barrel_length*10 + k.stats.weight)*100
		end

		if k.type=="magazine" and k.stats and k.stats.mag_amount and k.stats.CLIP_AMMO_MAX then
			local clip = k.stats.CLIP_AMMO_MAX
			if type(clip)=="table" then for i, k in pairs(clip) do clip = k break end end
			local weight = k.stats.weight
			local concealment = k.stats.concealment
			k.stats.value = k.value or math.round( (3+(clip * 0.1)) + (weight * 0.8) + (concealment * 0.1) )
			k.sort_number = k.stats.value*1000
		end
	end

	local sort_number = 100000
	for i, k in pairs(self.nqr.all_mds) do
		self.parts[k].stats.value = 0

		for u, j in pairs(self.parts[k].stats.md_code or {}) do
			self.parts[k].stats.value = self.parts[k].stats.value + (self.parts[k].stats.md_code[u+1] or 0)
		end
	...
	local sort_number = 10000
	for i, k in pairs(self.parts) do
		if k.stats.md_code and k.stats.md_code[1]==0 and not table.contains(self.nqr.all_mds, i) then
			k.stats.value = k.stats.value or 0

			if k.stats.barrel_length then
				k.stats.value = k.stats.value + 1
			else
				for u, j in pairs(k.stats.md_code or {}) do
					k.stats.value = k.stats.value + (k.stats.md_code[u+1] or 0)
				end
			end

			k.sort_number = k.sort_number or sort_number
			sort_number = sort_number + k.stats.value
		end
	end
	local sort_number = 200000
	for i, k in pairs(self.nqr.all_sps) do
		self.parts[k].stats.value = 4

		for u, j in pairs(self.parts[k].stats.md_code or {}) do
			self.parts[k].stats.value = self.parts[k].value or (self.parts[k].stats.value + (self.parts[k].stats.md_code[u] or 0)*2)
		end

		self.parts[k].sort_number = self.parts[k].sort_number or sort_number
		sort_number = sort_number + 1000
	end
	local sort_number = 20000
	for i, k in pairs(self.parts) do
		if k.stats.md_code and k.stats.md_code[1]~=0 and not table.contains(self.nqr.all_sps, i) then
			k.stats.value = k.stats.value or 0

			local to_add = 4
			for u, j in pairs(k.stats.md_code or {}) do
				to_add = k.value or (to_add + (k.stats.md_code[u] or 0)*2)
			end
			to_add = math.round(to_add * (0.95^(k.stats.value-1)))
			k.stats.value = k.stats.value + to_add
	...
	for i, k in pairs(self.nqr.all_sights) do
		self.parts[k].stats.value = self.parts[k].value or value
		value = value + 1

		self.parts[k].sort_number = sort_number + (self.parts[k].value or i) * 1000
	end
	local sort_number = 10000
	local value = 1
	for i, k in pairs(self.nqr.all_pistol_reddots) do
	...
	for i, k in pairs(self.parts) do
		...
		if k.stats and (k.stats.md_code or k.stats.gadget_power) then
			k.has_description = true
		end
	end
	...
	self.wpn_fps_ass_amcar.sightheight_mod = self.wpn_fps_ass_m4.sightheight_mod
	self.wpn_fps_ass_m16.sightheight_mod = self.wpn_fps_ass_m4.sightheight_mod
	self.wpn_fps_smg_olympic.sightheight_mod = self.wpn_fps_ass_m4.sightheight_mod
		...
		if k.type=="sight" and not k.blank_sight then
			...
			elseif k.stats.sightheight then
				for u, j in pairs(self.nqr.all_piggyback_sights) do
					k.override[j] = { a_obj = "a_o", parent = "sight", stats = table.copy_append(self.parts[j].stats, { sightheight = k.piggyback_height + self.parts[j].stats.sightheight }) }
				end
			end
		end
