...
--[[local function format_round(num, round_value) --todo
...
function PlayerInventoryGui:_update_player_stats()
	...
	for _, stat in ipairs(self._player_stats_shown) do
		...
		self._player_stats_texts[stat.name].total:set_text((stat.multiplier and "x" or "")..format_round(value, stat.round_value)..(stat.percent and "%" or ""))
		self._player_stats_texts[stat.name].base:set_text((stat.multiplier and "x" or "")..format_round(base, stat.round_value)..(stat.percent and "%" or ""))
...
function PlayerInventoryGui:_update_stats(name)
	...
	if name == "primary" or name == "secondary" then
		...
		--table.insert(stats, { inverted = true, name = "reload" })
...
function PlayerInventoryGui:_update_info_weapon_mod(box)
	...
	if not part_id or managers.weapon_factory:is_part_standard_issue_by_weapon_id(mod_data.name, part_id) then
		...
		for _, stat in ipairs(self._stats_shown) do
			...
			local total_value = (
				type(total_base_stats[stat.name].value)=="number"
				and math.max(total_base_stats[stat.name].value + total_mods_stats[stat.name].value + total_skill_stats[stat.name].value, 0)
				or (total_mods_stats[stat.name].value or total_base_stats[stat.name].value)
			)
...
function PlayerInventoryGui:_update_info_weapon(name)
	...
	for _, stat in ipairs(self._stats_shown) do
		...
		if stat.name=="damage" or stat.name=="spread" or stat.name=="recoil" then
			...
			value = math.max(base_stats[stat.name].value + mods_stats[stat.name].value, 0)
		else
		...
		local mags_slot = stat.name=="totalammo" and not (
			tweak_data.weapon[equipped_item.weapon_id].use_shotgun_reload or tweak_data.weapon[equipped_item.weapon_id].feed_system=="break_action"
		) and " mag" or ""

		self._stats_texts[stat.name].total:set_alpha(1)
		self._stats_texts[stat.name].total:set_text(format_round(value, stat.round_value)..mags_slot..(mags_slot~="" and value~=1 and "s" or ""))
	...
	if c_td.default_blueprint then
		...
		for _, stat in ipairs(self._stats_shown) do
			...
			--elseif stat.index then
				-- Nothing
			--elseif tweak_stats[stat.name] then
			--	local without_skill = math.round(base_stats[stat.name] + mods_stats[stat.name])
			--	local max_stat = math.max(tweak_stats[stat.name][1], tweak_stats[stat.name][#tweak_stats[stat.name]]) * tweak_data.gui.stats_present_multiplier * (modifier_stats and modifier_stats[stat.name] or 1)
...
function PlayerInventoryGui:_update_info_melee(name)
	...
	for _, stat in ipairs(self._stats_shown) do
		...
		if type(base_stats[stat.name].value)=="table" or type(mods_stats[stat.name].value)=="table" or type(skill_stats[stat.name].value)=="table" then
			value = mods_stats[stat.name].value
		else
		...
		local skill_text = skill_stats[stat.name] and format_round(skill_stats[stat.name].value, stat.round_value)
	...
	if equipped_mods then
		...
		for _, stat in pairs(self._stats_shown) do
			...
			if mods_stats[stat.name].index and tweak_stats[stat_name] then
				...
				--	mods_stats[stat.name] = max_stat - mods_stats[stat.name]
				--end
				...
				--	local ratio = mods_stats[stat.name] / max_stat
				--	mods_stats[stat.name] = ratio * 100
				--end
...
function PlayerInventoryGui:_get_melee_weapon_stats(name)
	...
	for i, stat in ipairs(self._stats_shown) do
		if not stat.index and type(base_stats[stat.name])=="number" and type(skill_stats[stat.name])=="number" then
...
function PlayerInventoryGui:_get_armor_stats(name)
	...
	for i, stat in ipairs(self._stats_shown) do
		...
		elseif stat.name == "weight" then
			...
			local category = name == "primary" and "primaries" or "secondaries"
			local equipped_item = managers.blackmarket:equipped_item("primaries")
			local equipped_slot = managers.blackmarket:equipped_weapon_slot("primaries")
			local wep_base_stats, wep_mods_stats, wep_skill_stats = WeaponDescription._get_stats(equipped_item.weapon_id, "primaries", equipped_slot)
			mod = mod + wep_base_stats.weight.value + wep_mods_stats.weight.value

			local equipped_item = managers.blackmarket:equipped_item("secondaries")
			local equipped_slot = managers.blackmarket:equipped_weapon_slot("secondaries")
			local wep_base_stats, wep_mods_stats, wep_skill_stats = WeaponDescription._get_stats(equipped_item.weapon_id, "secondaries", equipped_slot)
			mod = mod + wep_base_stats.weight.value + wep_mods_stats.weight.value
...
function PlayerInventoryGui:init(ws, fullscreen_ws, node)
	...
	local skill_box_data = {
		...
		text = managers.localization:to_upper_text("menu_wip"),
		select_anim = select_anim,
		unselect_anim = unselect_anim,
		bg_color = Color.black:with_alpha(0.05),
		clbks = {
	...
	local icon_atlas_texture = guis_catalog .. "textures/pd2/skilltree_2/icons_atlas_2"
	...
	local detection_value = detection_panel:text({
		...
		visible = true,
		font = tweak_data.menu.pd2_medium_font,
		font_size = tweak_data.menu.pd2_small_font_size --pd2_medium_font_size
	})
	...
	detection_panel:set_top(detection_text:bottom() + 0.05)
