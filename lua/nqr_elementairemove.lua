...
local ilija_pls = {
	crojob2 = "ai_remove_sniper_kill",
	crojob3 = "ai_remove_001",
	framing_frame_2 = "ai_remove_001",
	kenaz = "ai_remove_sniper_kill",
	mia_1 = "ai_remove_001",
	watchdogs_2 = "ai_remove_002",
	watchdogs_2_day = "ai_remove_002",
	born = "ai_remove_enemy_sniper",
	mex = "ai_remove_sniped",
...
function ElementAIRemove:on_executed(instigator)
	...
	if self._values.use_instigator then
		if alive(instigator) then
			...
			if self._editor_name==ilija_pls[job]
			or (self._values.instance_name and string.find(self._values.instance_name, "ilija", 1, true))
			and (
				(instigator:brain() and instigator:brain().surrendered and instigator:brain():surrendered())
				or (instigator:anim_data() and instigator:anim_data().surrender)
				or (instigator:character_damage() and instigator:character_damage().is_converted and instigator:character_damage():is_converted())
			) then log("ilija_pls") return end
