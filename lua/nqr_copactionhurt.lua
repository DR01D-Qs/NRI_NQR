...
function CopActionHurt:_upd_bleedout(t)
	...
	if not self._ext_anim.bleedout_enter and self._weapon_unit and self._unit:base()._tweak_table~="sniper" then
		if self._attention and not self._ext_anim.reload and not self._ext_anim.equip then
			...
			if self._shoot_t < t then
                autotarget = true --im watching ur ass
				...
				elseif self._common_data.allow_fire then
					...
					if autotarget then
						local new_target_pos = CopActionShoot._get_unit_shoot_pos(self, t, target_pos, target_dis, self._w_usage_tweak, falloff, i_range, autotarget)
...
--WOW YOU CAN ACTUALLY PRETTY EASILY CONTROL ANIMATION VARIANTS SO ILL REMOVE THAT MOTHERFUCKING SPINNY SHIELD KNOCK ANIM
--NO DEATH SOUND ON DENIED
function CopActionHurt:init(action_desc, common_data)
	...
	--.mission._fading_debug_output:script().log(tostring(self._unit:key()), Color.green)

	if action_type == "knock_down" then action_type = "heavy_hurt" end
	...
	elseif action_type == "bleedout" then
        ...
        self._ext_movement.bleedouted = true
	...
	elseif action_type == "shield_knock" then
		...
		if variant_count > 1 then
			variant = math.max(2, self:_pseudorandom(variant_count))
		end
		...
		if action_desc.variant ~= "bleeding" then
			...
			elseif action_type == "heavy_hurt" then
				...
				self._machine:set_speed(redir_res, 0.8)
	...
	if action_type == "bleedout" then
		...
		self._ext_inventory:drop_shield()
		...
		if self._weapon_unit then
			...
			local weapon_usage_tweak = common_data.char_tweak.weapon[self._weap_tweak.usage]
	...
	if not self._unit:base().nick_name then
		...
		elseif action_type == "death" then
			if action_desc.variant~="denied" and tweak_table~="phalanx_minion" then self._unit:sound():say("x02a_any_3p") end
