...
function CopActionShoot:update(t)
	...
	if self._shield_use_cooldown and target_vec and self._common_data.allow_fire and self._shield_use_cooldown < t and target_dis < self._shield_use_range then
		...
		if new_cooldown then self._shield_use_cooldown = new_cooldown end
	end
	...
	if not ext_anim.reload and not ext_anim.equip and not ext_anim.melee then
		...
		elseif target_vec and self._common_data.allow_fire and self._shoot_t < t and self._mod_enable_t < t then
			...
			if autotarget or self._shooting_husk_player and self._next_vis_ray_t < t then
				...
				if fire_line then
					if t - self._line_of_sight_t > 3 then
						...
						--local aim_delay = 0.3 + (target_vec:length()*0.1)
						--managers.mission._fading_debug_output:script().log(tostring(aim_delay), Color.white)
						aim_delay = (aim_delay + self:_pseudorandom() * aim_delay * 0.3) --* (math.min(3, t - self._line_of_sight_t)/3)
					...
					elseif fire_line.distance > 3000 then
						shoot = true
					end
				else
					if t - self._line_of_sight_t > 1 and not self._last_vis_check_status then
						...
						local focus_delay = self._w_usage_tweak.focus_delay * math.min(1, displacement / 10)
			...
			if shoot then
				...
				if autotarget and (not self._common_data.melee_countered_t or t - self._common_data.melee_countered_t > 15) and target_dis < 150 and self._w_usage_tweak.melee_speed and self._melee_timeout_t < t then
...
--GET FALLOFF: HARDCODED DAMAGE MULTIPLIER
function CopActionShoot:_get_shoot_falloff(target_dis, falloff)
	for i_range, range_data in ipairs(falloff) do
		if target_dis < range_data.r then
            range_data.dmg_mul = 1
			return range_data, i_range
		end
	end

    falloff[#falloff].dmg_mul = 1
	return falloff[#falloff], #falloff
end



--ENEMY MELEE COOLDOWNS
function CopActionShoot:_chk_start_melee(was_synced)
	...
	if state then
		...
		self._common_data.machine:set_speed(state, anim_speed*0.6)

		self._melee_timeout_t = TimerManager:game():time() + 0.2

		if not was_synced then self._common_data.ext_network:send("action_melee_attack", self._body_part) end
	else
...
function CopActionShoot:anim_clbk_melee_strike()
	...
	local melee_weapon = self._unit:base():melee_weapon()
	local is_weapon = melee_weapon == "weapon"
	local damage = is_weapon and 2 or tweak_data.weapon.npc_melee[melee_weapon].damage
	local dmg_mul = self._common_data.char_tweak.melee_weapon_dmg_multiplier or 1
...
function CopActionShoot:_get_unit_shoot_pos(t, pos, dis, w_tweak, falloff, i_range, shooting_local_player)
	...
	if i_range == 1 then
		...
		hit_chance = math.lerp(hit_chances[2]*0.2, hit_chances[2], focus_prog)
	else
		local prev_falloff = w_tweak.FALLOFF[i_range - 1]
		dis_lerp = math.min(1, (dis - prev_falloff.r) / (falloff.r - prev_falloff.r))
		local prev_range_hit_chance = math.lerp(prev_falloff.acc[2]*0.2, prev_falloff.acc[2], focus_prog) --math.lerp(prev_falloff.acc[1], prev_falloff.acc[2], focus_prog)
